        ...
        public function content_template()
        {
            ...
            <div class="theme" tabindex="0" aria-describedby="{{ data.section }}-{{ data.theme.id }}-action">
                ...
                <span class="more-details theme-details"
                      id="{{ data.section }}-{{ data.theme.id }}-action"
                      aria-label="<?php echo esc_attr($details_label); ?>"><?php _e('Theme Details'); ?></span>
                ...
                <div class="update-message notice inline notice-warning notice-alt" data-slug="{{ data.theme.id }}">
                    <p>
                        <?php
                            ...
                            {
                                printf(/* translators: %s: "Update now" button. */ __('New version available. %s'), '<button class="button-link update-theme" type="button">'.__('Update now').'</button>');
                            }
                        ?>
                    </p>
                </div>
                ...
                <div class="update-message notice inline notice-error notice-alt" data-slug="{{ data.theme.id }}">
                    <p>
                        ...
                        { #>
                        <?php
                            printf(/* translators: %s: Theme name. */ __('There is a new version of %s available, but it does not work with your versions of WordPress and PHP.'), '{{{ data.theme.name }}}');
                        ...
                        <?php
                            printf(/* translators: %s: Theme name. */ __('There is a new version of %s available, but it does not work with your version of WordPress.'), '{{{ data.theme.name }}}');
                        ...
                        <?php
                            printf(/* translators: %s: Theme name. */ __('There is a new version of %s available, but it does not work with your version of PHP.'), '{{{ data.theme.name }}}');
                ...
                <div class="theme-id-container">
                    ...
                    <div class="theme-actions">
                        <button type="button"
                                ...
                                aria-label="<?php echo esc_attr($customize_label); ?>"><?php _e('Customize'); ?></button>
                    </div>
                </div>
                <?php
                ...
                <div class="theme-id-container">
                    ...
                    <div class="theme-actions">
                        ...
                        ?>
                        <a href="{{{ data.theme.actions.activate }}}"
                ...
                <# if ( data.theme.actions.activate ) { #>
                <?php
                    $customizer_not_supported_message .= ' '.sprintf(/* translators: %s: URL to the themes page (also it activates the theme). */ __('However, you can still <a href="%s">activate this theme</a>, and use the Site Editor to customize it.'), '{{{ data.theme.actions.activate }}}');
                ?>
                <# } #>

                <?php
                    wp_admin_notice($customizer_not_supported_message, [
                        'type' => 'error',
                        'additional_classes' => ['notice-alt'],
                    ]);
                ?>
                ...
                <div class="theme-id-container">
                    ...
                    <div class="theme-actions">
                        ...
                        <button type="button"
                                ...
                                data-slug="{{ data.theme.id }}"><?php _e('Live Preview'); ?></button>
                        <# } else { #>
                        <button type="button"
                                class="button button-primary disabled"
                                aria-label="<?php echo esc_attr($preview_label); ?>"><?php _e('Live Preview'); ?></button>
                        <# } #>
                    </div>
                </div>
                <?php
                ...
                <div class="theme-id-container">
                    ...
                    <div class="theme-actions">
                        ...
                        <button type="button"
                                ...
                                aria-label="<?php echo esc_attr($install_label); ?>"
                                data-slug="{{ data.theme.id }}"
                                data-name="{{ data.theme.name }}"><?php _e('Install &amp; Preview'); ?></button>
                        <# } else { #>
                        <button type="button"
                                class="button button-primary disabled"
                                aria-label="<?php echo esc_attr($install_label); ?>"
                                disabled><?php _e('Install &amp; Preview'); ?></button>
                        <# } #>
                    </div>
                </div>
                <# } #>
