<?php
    ...
    class WP_Customize_Date_Time_Control extends WP_Customize_Control
    {
            ...
            <div class="date-time-fields {{ data.includeTime ? 'includes-time' : '' }}">
                <fieldset class="day-row">
                    ...
                    <div class="day-fields clear">
                        ...
                        <input id="{{ idPrefix }}date-time-day"
                               type="number"
                               size="2"
                               autocomplete="off"
                        ...
                        <input id="{{ idPrefix }}date-time-year"
                               type="number"
                               size="4"
                               autocomplete="off"
                ...
                <fieldset class="time-row clear">
                    ...
                    <div class="time-fields clear">
                        ...
                        <input id="{{ idPrefix }}date-time-hour"
                               type="number"
                               size="2"
                               autocomplete="off"
                               class="date-input hour"
                               data-component="hour"
                               min="{{ minHour }}"
                               max="{{ maxHour }}">
                        :
                        ...
                        <input id="{{ idPrefix }}date-time-minute"
                               type="number"
                               size="2"
                               autocomplete="off"
                               class="date-input minute"
                        ...
                        <select id="{{ idPrefix }}date-time-meridian"
                                class="date-input meridian"
                                data-component="meridian">
        ...
        public function get_timezone_info()
        {
            ...
            if($tz_string)
            {
                ...
                if($tz)
                {
                    ...
                    $timezone_info['description'] = sprintf(/* translators: 1: Timezone name, 2: Timezone abbreviation, 3: UTC abbreviation and offset, 4: UTC offset. */ __('Your timezone is set to %1$s (%2$s), currently %3$s (Coordinated Universal Time %4$s).'), $tz_name, '<abbr>'.$timezone_info['abbr'].'</abbr>', '<abbr>UTC</abbr>'.$formatted_gmt_offset, $formatted_gmt_offset);
                }
                else
                {
        ...
        public function format_gmt_offset($offset)
        {
            ...
            $formatted_offset = str_replace(['.25', '.5', '.75'], [':15', ':30', ':45'], $formatted_offset);
