...
// when the gutenberg plugin is active.
    if(defined('IS_GUTENBERG_PLUGIN') && IS_GUTENBERG_PLUGIN)
    {
        ...
        function gutenberg_block_core_navigation_update_interactive_view_script($metadata)
        {
            ...
            {
                $metadata['viewScript'] = ['file:./interactivity.min.js'];
            }
    ...
    function block_core_navigation_get_fallback_blocks()
    {
        ...
        $fallback_blocks = $registry->is_registered('core/page-list') ? $page_list_fallback : [];
    ...
    function render_block_core_navigation($attributes, $content, $block)
    {
        ...
        $text_decoration = _wp_array_get($attributes, ['style', 'typography', 'textDecoration'], null);
        $text_decoration_class = sprintf('has-text-decoration-%s', $text_decoration);
        $colors = block_core_navigation_build_css_colors($attributes);
        $font_sizes = block_core_navigation_build_css_font_sizes($attributes);
        $classes = array_merge($colors['css_classes'], $font_sizes['css_classes'], $is_responsive_menu ? ['is-responsive'] : [], $layout_class ? [$layout_class] : [], $is_fallback ? ['is-fallback'] : [], $text_decoration ? [$text_decoration_class] : []);
        $post_ids = block_core_navigation_get_post_ids($inner_blocks);
        if($post_ids)
        {
            _prime_post_caches($post_ids, false, false);
        }
        $list_item_nav_blocks = [
        ...
        if(! wp_script_is($view_js_file))
        {
            ...
            {
                $block->block_type->view_script_handles = array_diff($script_handles, [
                    $view_js_file,
            ...
            if($should_load_view_script && ! in_array($view_js_file, $script_handles, true))
            {
                $block->block_type->view_script_handles = array_merge($script_handles, [
                    $view_js_file,
                    'wp-block-navigation-view-2'
                ]);
            }
    ...
    function block_core_navigation_parse_blocks_from_menu_items($menu_items, $menu_items_by_parent_id)
    {
        ...
        foreach($menu_items as $menu_item)
        {
            ...
            $block['innerBlocks'] = isset($menu_items_by_parent_id[$menu_item->ID]) ? block_core_navigation_parse_blocks_from_menu_items($menu_items_by_parent_id[$menu_item->ID], $menu_items_by_parent_id) : [];
    ...
    function block_core_navigation_get_classic_menu_fallback_blocks($classic_nav_menu)
    {
        ...
        $menu_items = wp_get_nav_menu_items($classic_nav_menu->term_id, ['update_post_term_cache' => false]);
        ...
        $inner_blocks = block_core_navigation_parse_blocks_from_menu_items(isset($menu_items_by_parent_id[0]) ? $menu_items_by_parent_id[0] : [], $menu_items_by_parent_id);
