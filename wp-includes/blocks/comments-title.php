    ...
    function render_block_core_comments_title($attributes)
    {
        ...
        if($show_comments_count)
        {
            if($show_post_title)
            {
                ...
                {
                    $comments_title = sprintf(/* translators: 1: Number of comments, 2: Post title. */ _n('%1$s response to %2$s', '%1$s responses to %2$s', $comments_count), number_format_i18n($comments_count), $post_title);
                }
            }
            ...
            {
                $comments_title = sprintf(/* translators: %s: Number of comments. */ _n('%s response', '%s responses', $comments_count), number_format_i18n($comments_count));
            }
        }
