...
window.wpAjax = jQuery.extend({
    unserialize: function (s) {
        ...
        pp = s.split('&');
        for (i in pp) {
            if (typeof pp.hasOwnProperty === 'function' && !pp.hasOwnProperty(i)) {
                continue;
            }
            p = pp[i].split('=');
            r[p[0]] = p[1];
        ...
        if (x && typeof x === 'object' && x.getElementsByTagName('wp_ajax')) {
            ...
            jQuery('response', x).each(function () {
                ...
                if (!jQuery('wp_error', child).each(function () {
                    ...
                    if (e) {
                        wpAjax.invalidateForm(jQuery('#' + e + ' :input[name="' + code + '"]').parents('.form-field:first'));
                    }
    ...
    invalidateForm: function (selector) {
        return jQuery(selector).addClass('form-invalid').find('input').one('change wp-check-valid-field', function () {
            jQuery(this).closest('.form-invalid').removeClass('form-invalid');
        });
    },
    validateForm: function (selector) {
        selector = jQuery(selector);
        return !wpAjax.invalidateForm(selector.find('.form-required').filter(function () {
...
jQuery(function ($) {
    $('form.validate').on('submit', function () {
        return wpAjax.validateForm($(this));
    });
});
