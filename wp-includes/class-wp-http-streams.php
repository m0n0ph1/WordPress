<?php
    ...
    class WP_Http_Streams
    {
        ...
        public function request($url, $args = [])
        {
            ...
            $include_port_in_host_header = (($proxy->is_enabled() && $proxy->send_through_proxy($url)) || ('http' === $parsed_url['scheme'] && 80 !== $parsed_url['port']) || ('https' === $parsed_url['scheme'] && 443 !== $parsed_url['port']));
            ...
            if(! empty($processed_response['body']) && isset($processed_headers['headers']['transfer-encoding']) && 'chunked' === $processed_headers['headers']['transfer-encoding'])
            {
            ...
            if(isset($parsed_args['limit_response_size']) && strlen($processed_response['body']) > $parsed_args['limit_response_size'])
            {
        ...
        public static function verify_ssl_certificate($stream, $host)
        {
            ...
            if(! empty($cert['extensions']['subjectAltName']))
            {
                ...
                foreach($match_against as $match)
                {
                    ...
                    if(strtolower(trim($match_type)) === $host_type)
                    { // IP: or DNS:
