<?php
    ...
    function wptexturize($text, $reset = false)
    {
        ...
        if($reset || ! isset($static_characters))
        {
            ...
            $static_characters = array_merge(['...', '``', '\'\'', ' (tm)'], $cockney);
    ...
    function wp_check_invalid_utf8($text, $strip = false)
    {
        ...
        if(! isset($is_utf8))
        {
            $is_utf8 = in_array(get_option('blog_charset'), ['utf8', 'utf-8', 'UTF8', 'UTF-8'], true);
        }
        if(! $is_utf8)
        {
            return $text;
    ...
    function _make_web_ftp_clickable_cb($matches)
    {
        ...
        if(in_array($last_char, ['.', ',', ';', ':', ')'], true) === true)
        {
            $ret = $last_char;
    ...
    function _make_clickable_rel_attr($url)
    {
        ...
        $nofollow_schemes = array_intersect(wp_allowed_protocols(), ['https', 'http']);
	 ...
	 */
        if(
            ! str_contains($url, ':') && ! in_array($url[0], [
                '/',
                '#',
                '?'
...
EOF;
    ...
    function sanitize_option($option, $value)
    {
        ...
        switch($option)
        {
            ...
            case 'tag_base':
                ...
                {
                    $error = sprintf(/* translators: %s: Documentation URL. */ __('A structure tag is required when using custom permalinks. <a href="%s">Learn more</a>'), __('https://wordpress.org/documentation/article/customize-permalinks/#choosing-your-permalink-structure'));
                }
                break;
    ...
    function wp_strip_all_tags($text, $remove_breaks = false)
    {
		 ...
		 */
            trigger_error(sprintf(/* translators: 1: The function name, 2: The argument number, 3: The argument name, 4: The expected type, 5: The provided type. */ __('Warning: %1$s expects parameter %2$s (%3$s) to be a %4$s, %5$s given.'), __FUNCTION__, '#1', '$text', 'string', gettype($text)), E_USER_WARNING);
...
// phpcs:disable WordPress.WP.CapitalPDangit.Misspelled, WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid -- 8-)
    ...
    function capital_P_dangit($text)
    {
        ...
        return str_replace([
                               ...
                               '>Wordpress',
                               '(Wordpress'
                           ], [' WordPress', '&#8216;WordPress', $dblq.'WordPress', '>WordPress', '(WordPress'], $text);
    }
