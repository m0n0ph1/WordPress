<?php
    ...
    class IRI
    {
        ...
        protected function replace_invalid_with_pct_encoding($string, $extra_chars, $iprivate = false)
        {
            ...
            $string = preg_replace_callback('/(?:%[A-Fa-f0-9]{2})+/', [
                $this,
            ...
            while(($position += strspn($string, $extra_chars, $position)) < $strlen)
            {
                ...
                if(// Invalid sequences
                    ...
                    // Noncharacters
                    || ($character & 0xFFFE) === 0xFFFE || $character >= 0xFDD0 && $character <= 0xFDEF || (// Everything else not in ucschar
                        $character > 0xD7FF && $character < 0xF900 || $character < 0xA0 || $character > 0xEFFFD) && (// Everything not in iprivate, if it applies
        ...
        public function set_host($ihost)
        {
            ...
            elseif(strpos($ihost, '[') === 0 && substr($ihost, -1) === ']')
            {
        ...
        public function is_valid()
        {
            ...
            if(! $isauthority && (strpos($this->ipath, '//') === 0))
            {
                return false;
            }
        ...
        protected function remove_iunreserved_percent_encoded($match)
        {
            ...
            for($i = 1, $len = count($bytes); $i < $len; $i++)
            {
                ...
                if(! $remaining)
                {
                    ...
                    if(// Invalid sequences
                        ! $valid // Non-shortest form sequences are invalid
                        || $length > 1 && $character <= 0x7F || $length > 2 && $character <= 0x7FF || $length > 3 && $character <= 0xFFFF // Outside of range of iunreserved codepoints
                        || $character < 0x2D || $character > 0xEFFFD // Noncharacters
                        || ($character & 0xFFFE) === 0xFFFE || $character >= 0xFDD0 && $character <= 0xFDEF // Everything else not in iunreserved (this is all BMP)
