    ...
    class Parser
    {
            ...
            if(! is_string($feed_author) && isset($feed_author['properties']['photo'][0]))
            {
                $image = [
                    [
                        'child' => [
                            '' => [
                                'url' => [['data' => $feed_author['properties']['photo'][0]]]
                            ]
                        ]
                    ]
                ];
        ...
        public function tag_close($parser, $tag)
        {
            if($this->current_xhtml_construct >= 0)
            {
                ...
                if(
                    end($this->namespace) === SIMPLEPIE_NAMESPACE_XHTML && ! in_array(end($this->element), [
                        'area',
                        'base',
                        'basefont',
                        'br',
        ...
        public function tag_open($parser, $tag, $attributes)
        {
            ...
            if(isset($attribs[SIMPLEPIE_NAMESPACE_XML]['base']))
            {
                $base = $this->registry->call('Misc', 'absolutize_url', [
                    $attribs[SIMPLEPIE_NAMESPACE_XML]['base'],
                    end($this->xml_base)
                ]);
                if($base !== false)
                {
            ...
            else
            {
                ...
                if(
                    (end($this->namespace) === SIMPLEPIE_NAMESPACE_ATOM_03 && in_array(end($this->element), [
                            'title',
                            'tagline',
                            'copyright',
                            'info',
