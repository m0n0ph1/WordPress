<?php
    ...
    final class WP_Customize_Nav_Menus
    {
        ...
        public function __construct($manager)
        {
            ...
            add_filter('customize_dynamic_setting_args', [$this, 'filter_dynamic_setting_args'], 10, 2);
            add_filter('customize_dynamic_setting_class', [$this, 'filter_dynamic_setting_class'], 10, 3);
            add_action('customize_save_nav_menus_created_posts', [$this, 'save_nav_menus_created_posts']);
            ...
            add_filter('customize_refresh_nonces', [$this, 'filter_nonces']);
            add_action('wp_ajax_load-available-menu-items-customizer', [$this, 'ajax_load_available_items']);
            add_action('wp_ajax_search-available-menu-items-customizer', [$this, 'ajax_search_available_items']);
            add_action('wp_ajax_customize-nav-menus-insert-auto-draft', [$this, 'ajax_insert_auto_draft_post']);
            add_action('customize_controls_enqueue_scripts', [$this, 'enqueue_scripts']);
            add_action('customize_controls_print_footer_scripts', [$this, 'print_templates']);
            ...
            add_action('customize_preview_init', [$this, 'make_auto_draft_status_previewable']);
        ...
        public function load_available_items_query($object_type = 'post_type', $object_name = 'page', $page = 0)
        {
            ...
            if('post_type' === $object_type)
            {
                ...
                $posts = array_merge($posts, $important_pages, get_posts($args));
        ...
        public function search_available_items_query($args = [])
        {
            ...
            $post_type_objects = get_post_types(['show_in_nav_menus' => true], 'objects');
            $query = [
            ...
            $taxonomies = get_taxonomies(['show_in_nav_menus' => true], 'names');
            $terms = get_terms([
        ...
        public function customize_register()
        {
            ...
            {
                $description .= '<p>'.sprintf(/* translators: %s: URL to the Widgets panel of the Customizer. */ __('Menus can be displayed in locations defined by your theme or in <a href="%s">widget areas</a> by adding a &#8220;Navigation Menu&#8221; widget.'), "javascript:wp.customize.panel( 'widgets' ).focus();").'</p>';
            }
            else
            {
            ...
            $this->manager->add_panel(
                new WP_Customize_Nav_Menus_Panel($this->manager, 'nav_menus', [
                    'title' => __('Menus'),
            ...
            $this->manager->add_section('menu_locations', [
            ...
            {
                $theme_mods = get_option('theme_mods_'.$this->manager->get_stylesheet(), []);
            ...
            foreach($locations as $location => $description)
            {
                ...
                if($setting)
                {
                    ...
                    add_filter("customize_sanitize_{$setting_id}", [$this, 'intval_base10']);
                }
                else
                {
                    $this->manager->add_setting($setting_id, [
                        'sanitize_callback' => [$this, 'intval_base10'],
                ...
                $this->manager->add_control(
                    new WP_Customize_Nav_Menu_Location_Control($this->manager, $setting_id, [
                        'label' => $description,
            ...
            foreach($menus as $menu)
            {
                ...
                $this->manager->add_section(
                    new WP_Customize_Nav_Menu_Section($this->manager, $section_id, [
                ...
                $this->manager->add_setting(
                    new WP_Customize_Nav_Menu_Setting($this->manager, $nav_menu_setting_id, [
                        'transport' => 'postMessage',
                    ])
                );
                ...
                foreach(array_values($menu_items) as $i => $item)
                {
                    ...
                    $this->manager->add_setting(
                        new WP_Customize_Nav_Menu_Item_Setting($this->manager, $menu_item_setting_id, [
                            'value' => $value,
                            'transport' => 'postMessage',
                        ])
                    );
                    ...
                    $this->manager->add_control(
                        new WP_Customize_Nav_Menu_Item_Control($this->manager, $menu_item_setting_id, [
                            'label' => $item->title,
                            'section' => $section_id,
                            'priority' => 10 + $i,
                        ])
            ...
            $this->manager->add_section('add_menu', [
                'type' => 'new_menu',
                'title' => __('New Menu'),
                'panel' => 'nav_menus',
                'priority' => 20,
            ...
            $this->manager->add_setting(
                new WP_Customize_Filter_Setting($this->manager, 'nav_menus_created_posts', [
                    'transport' => 'postMessage',
                    'type' => 'option', // To prevent theme prefix in changeset.
                    'default' => [],
                    'sanitize_callback' => [$this, 'sanitize_nav_menus_created_posts'],
                ])
            );
        }
        ...
        public function ajax_insert_auto_draft_post()
        {
            ...
            $illegal_params = array_diff(array_keys($params), ['post_type', 'post_title']);
        ...
        public function print_templates()
        {
            ...
            <script type="text/html" id="tmpl-nav-menu-submit-new-button">
                ...
                <button id="customize-new-menu-submit"
                        type="button"
                        class="button"
                        aria-describedby="customize-new-menu-submit-description"><?php _e('Next'); ?></button>
            </script>
            ...
            <script type="text/html" id="tmpl-nav-menu-locations-header">
                ...
                <p class="customize-control-description customize-section-title-menu_locations-description">{{
                    data.l10n.locationsDescription }}</p>
            </script>
                ...
                <div id="available-menu-items-search" class="accordion-section cannot-expand">
                    <div class="accordion-section-title">
                        ...
                        <input type="text"
                               ...
                               placeholder="<?php esc_attr_e('Search menu items&hellip;'); ?>"
            ...
            <div id="<?php echo esc_attr($id); ?>" class="accordion-section">
                ...
                <div class="accordion-section-content">
                    <?php if('post_type' === $available_item_type['type']) : ?>
                        ...
                        <?php if(current_user_can($post_type_obj->cap->create_posts) && current_user_can($post_type_obj->cap->publish_posts)) : ?>
                            <div class="new-content-item">
                                <label for="<?php echo esc_attr('create-item-input-'.$available_item_type['object']); ?>"
                                       class="screen-reader-text"><?php echo esc_html($post_type_obj->labels->add_new_item); ?></label>
                                <input type="text"
                                       id="<?php echo esc_attr('create-item-input-'.$available_item_type['object']); ?>"
                    ...
                    <ul class="available-menu-items-list"
                        ...
                        data-type_label="<?php echo esc_attr(isset($available_item_type['type_label']) ? $available_item_type['type_label'] : $available_item_type['type']); ?>"></ul>
                </div>
            </div>
            <?php
        }
        ...
        public function customize_dynamic_partial_args($partial_args, $partial_id)
        {
            if(preg_match('/^nav_menu_instance\[[0-9a-f]{32}\]$/', $partial_id))
            {
                ...
                $partial_args = array_merge($partial_args, [
                    ...
                    'render_callback' => [$this, 'render_nav_menu_partial'],
        ...
        public function customize_preview_init()
        {
            ...
            add_filter('wp_nav_menu_args', [$this, 'filter_wp_nav_menu_args'], 1000);
            add_filter('wp_nav_menu', [$this, 'filter_wp_nav_menu'], 10, 2);
            add_action('wp_footer', [$this, 'export_preview_data'], 1);
        ...
        public function filter_wp_nav_menu_args($args)
        {
             ...
             */
            $can_partial_refresh = (// ...if wp_nav_menu() is directly echoing out the menu (and thus isn't manipulating the string after generated),
                ! empty($args['echo']) && // ...and if the fallback_cb can be serialized to JSON, since it will be included in the placement context data,
                (empty($args['fallback_cb']) || is_string($args['fallback_cb'])) && // ...and if the walker can also be serialized to JSON, since it will be included in the placement context data as well,
                (empty($args['walker']) || is_string($args['walker'])) // ...and if it has a theme location assigned or an assigned menu to display,
                && (! empty($args['theme_location']) || (! empty($args['menu']) && (is_numeric($args['menu']) || is_object($args['menu'])))) && // ...and if the nav menu would be rendered with a wrapper container element (upon which to attach data-* attributes).
                (! empty($args['container']) || (isset($args['items_wrap']) && str_starts_with($args['items_wrap'], '<'))));
