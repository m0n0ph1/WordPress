<?php
    ...
    function redirect_canonical($requested_url = null, $do_redirect = true)
    {
        ...
        if(is_feed() && $post_id)
        {
            ...
            if($redirect_url)
            {
                $redirect['query'] = _remove_qs_args_if_not_in_url($redirect['query'], [
                    'p',
                    'page_id',
                    'attachment_id',
                    'pagename',
        ...
        elseif(is_object($wp_rewrite) && $wp_rewrite->using_permalinks())
        {
            ...
            if(
                is_attachment() && ! array_diff(array_keys($wp->query_vars), [
                    'attachment',
                    'attachment_id'
                ]) && ! $redirect_url
            )
            ...
            { // Terms (tags/categories).
                ...
                if($term_count <= 1 && ! empty($obj->term_id))
                {
                    ...
                    if($tax_url && ! is_wp_error($tax_url) && ! empty($redirect['query']))
                    {
        ...
        if($redirect_obj instanceof WP_Post)
        {
             ...
             * throughout.
             */
            if(// Private post statuses only redirect if the user can read them.
                ! ($post_status_obj->private && current_user_can('read_post', $redirect_obj->ID)) && // For other posts, only redirect if publicly viewable.
        ...
        if(get_query_var('name'))
        {
            ...
            if(get_query_var('post_type'))
            {
                if(is_array(get_query_var('post_type')))
                {
                    ...
                    $where .= " AND post_type IN ('".implode("', '", esc_sql(get_query_var('post_type')))."')";
                }
                else
                {
            ...
            {
                $where .= " AND post_type IN ('".implode("', '", get_post_types(['public' => true]))."')";
            }
