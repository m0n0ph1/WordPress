<?php
    ...
    function wp_register_typography_support($block_type)
    {
        ...
        $has_font_family_support = _wp_array_get($typography_supports, ['__experimentalFontFamily'], false);
        $has_font_size_support = _wp_array_get($typography_supports, ['fontSize'], false);
        $has_font_style_support = _wp_array_get($typography_supports, ['__experimentalFontStyle'], false);
        $has_font_weight_support = _wp_array_get($typography_supports, ['__experimentalFontWeight'], false);
        $has_letter_spacing_support = _wp_array_get($typography_supports, ['__experimentalLetterSpacing'], false);
        $has_line_height_support = _wp_array_get($typography_supports, ['lineHeight'], false);
        $has_text_columns_support = _wp_array_get($typography_supports, ['textColumns'], false);
        $has_text_decoration_support = _wp_array_get($typography_supports, ['__experimentalTextDecoration'], false);
        $has_text_transform_support = _wp_array_get($typography_supports, ['__experimentalTextTransform'], false);
    ...
    function wp_apply_typography_support($block_type, $block_attributes)
    {
        ...
        {
            $typography_block_styles['lineHeight'] = _wp_array_get($block_attributes, [
                'style',
                'typography',
                'lineHeight'
            ]);
        ...
        {
            $typography_block_styles['textColumns'] = _wp_array_get($block_attributes, [
                'style',
                'typography',
                'textColumns'
            ],                                                      null);
        ...
        $attributes = [];
        $styles = wp_style_engine_get_styles(['typography' => $typography_block_styles], ['convert_vars_to_classnames' => true]);
    ...
    function wp_get_computed_fluid_typography_value($args = [])
    {
        ...
        $maximum_font_size = wp_get_typography_value_and_unit($maximum_font_size_raw, [
        ...
        $minimum_font_size_rem = wp_get_typography_value_and_unit($minimum_font_size_raw, [
            'coerce_to' => 'rem',
        ]);

        // Viewport widths defined for fluid typography. Normalize units.
        $maximum_viewport_width = wp_get_typography_value_and_unit($maximum_viewport_width_raw, [
            'coerce_to' => $font_size_unit,
        ]);
        $minimum_viewport_width = wp_get_typography_value_and_unit($minimum_viewport_width_raw, [
            'coerce_to' => $font_size_unit,
        ]);
    ...
    function wp_get_typography_font_size_value($preset, $should_use_fluid_typography = false)
    {
        ...
        $typography_settings = isset($global_settings['typography']) ? $global_settings['typography'] : [];
        $layout_settings = isset($global_settings['layout']) ? $global_settings['layout'] : [];

        if(isset($typography_settings['fluid']) && (true === $typography_settings['fluid'] || is_array($typography_settings['fluid'])))
        {
        ...
        $fluid_settings = isset($typography_settings['fluid']) && is_array($typography_settings['fluid']) ? $typography_settings['fluid'] : [];
         ...
         */
        $minimum_font_size_limit = wp_get_typography_value_and_unit($minimum_font_size_limit, [
...
// Register the block support.
    WP_Block_Supports::get_instance()->register('typography', [
