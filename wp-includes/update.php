<?php
    ...
    function wp_version_check($extra_stats = [], $force_check = false)
    {
        ...
        if(
            defined('WP_AUTO_UPDATE_CORE') && in_array(WP_AUTO_UPDATE_CORE, [
                'beta',
                'rc',
                'development',
                'branch-development'
            ...
            foreach($offer as $offer_key => $value)
            {
                if('packages' === $offer_key)
                {
                    $offer['packages'] = (object) array_intersect_key(
                        array_map('esc_url', $offer['packages']), array_fill_keys([
                                                                                      'full',
                                                                                      'no_content',
                                                                                      'new_bundled',
                                                                                      'partial',
    ...
    function wp_get_update_data()
    {
        ...
        if($core && function_exists('get_core_updates'))
        {
            ...
            if(
                ! empty($update_wordpress) && ! in_array($update_wordpress[0]->response, [
                    'development',
                    'latest'
                ],                                       true) && current_user_can('update_core')
            )
            {
                $counts['wordpress'] = 1;
            }
    ...
    function _wp_delete_all_temp_backups()
    {
        ...
        {
            return new WP_Error('fs_no_content_dir', /* translators: %s: Directory name. */ sprintf(__('Unable to locate WordPress content directory (%s).'), 'wp-content'));
        }
