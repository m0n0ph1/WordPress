<?php
    ...
    class ParagonIE_Sodium_Core_Ristretto255 extends ParagonIE_Sodium_Core_Ed25519
    {
        ...
        public static function ristretto255_frombytes($s, $skipCanonicalCheck = false)
        {
            ...
            $v = self::fe_mul(ParagonIE_Sodium_Core_Curve25519_Fe::fromArray(self::$d), $u1u1); /* v = d*u1^2 */
            $v = self::fe_neg($v); /* v = -d*u1^2 */
            $v = self::fe_sub($v, $u2u2); /* v = -(d*u1^2)-u2^2 */
            $v_u2u2 = self::fe_mul($v, $u2u2); /* v_u2u2 = v*u2^2 */ // fe25519_1(one);
        ...
        public static function ristretto255_sqrt_ratio_m1(
            ...
            $v3 = self::fe_mul(self::fe_sq($v), $v); /* v3 = v^3 */
            $x = self::fe_mul(self::fe_mul(self::fe_sq($v3), $u), $v); /* x = uv^7 */
            $x = self::fe_mul(self::fe_mul(self::fe_pow22523($x), /* x = (uv^7)^((q-5)/8) */ $v3), $u); /* x = uv^3(uv^7)^((q-5)/8) */
            $vxx = self::fe_mul(self::fe_sq($x), $v); /* vx^2 */
        ...
        public static function ristretto255_elligator(ParagonIE_Sodium_Core_Curve25519_Fe $t)
        {
            ...
            $v = self::fe_mul(self::fe_sub($c, self::fe_mul($r, $d)), $rpd); /* v = (c-r*d)*(r+d) */
            $result = self::ristretto255_sqrt_ratio_m1($u, $v);
            $s = $result['x'];
            $wasnt_square = 1 - $result['nonsquare'];
            $s_prime = self::fe_neg(self::fe_abs(self::fe_mul($s, $t))); /* s_prime = -|s*t| */
            ...
            $n = self::fe_sub(self::fe_mul(self::fe_mul(self::fe_sub($r, $one), $c), $sqdmone), $v); /* n =  c*(r-1)*(d-1)^2-v */
            $w0 = self::fe_mul(self::fe_add($s, $s), $v); /* w0 = 2s*v */
