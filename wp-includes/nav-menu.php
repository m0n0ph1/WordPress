<?php
    ...
    function wp_update_nav_menu_object($menu_id = 0, $menu_data = [])
    {
        ...
        {
            return new WP_Error('menu_exists', sprintf(/* translators: %s: Menu name. */ __('The menu name %s conflicts with another menu name. Please try another.'), '<strong>'.esc_html($menu_data['menu-name']).'</strong>'));
        }
    ...
    function wp_update_nav_menu_item($menu_id = 0, $menu_item_db_id = 0, $menu_item_data = [], $fire_after_hooks = true)
    {
        ...
        {
            $update_terms = wp_set_object_terms($menu_item_db_id, [$menu->term_id], 'nav_menu');
     ...
     * - object_id:        The DB ID of the original object this menu item represents, e.g. ID for posts and term_id
     * for categories.
