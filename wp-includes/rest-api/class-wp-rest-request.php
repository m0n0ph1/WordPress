<?php
    ...
    class WP_REST_Request implements ArrayAccess
    {
        ...
        public function sanitize_params()
        {
            ...
            if($invalid_params)
            {
                return new WP_Error('rest_invalid_param', /* translators: %s: List of invalid parameters. */ sprintf(__('Invalid parameter(s): %s'), implode(', ', array_keys($invalid_params))), [
                    'status' => 400,
        ...
        public function has_valid_params()
        {
            ...
            if(! empty($required))
            {
                return new WP_Error('rest_missing_callback_param', /* translators: %s: List of required parameters. */ sprintf(__('Missing parameter(s): %s'), implode(', ', $required)), [
                    'status' => 400,
                    'params' => $required,
                ]);
            }
            ...
            if(isset($attributes['validate_callback']))
            {
                ...
                if(false === $valid_check)
                {
                    ...
                    return new WP_Error('rest_invalid_params', __('Invalid parameters.'), ['status' => 400]);
                }
            }
