    ...
    class WP_REST_URL_Details_Controller extends WP_REST_Controller
    {
        ...
        public function get_item_schema()
        {
            ...
            $this->schema = [
                ...
                'properties' => [
                    'title' => [
                        'description' => sprintf(/* translators: %s: HTML title tag. */ __('The contents of the %s element from the URL.'), '<title>'),
                        'type' => 'string',
                        'context' => ['view', 'edit', 'embed'],
                        'readonly' => true,
                    ],
                    'icon' => [
                        'description' => sprintf(/* translators: %s: HTML link tag. */ __('The favicon image link of the %s element from the URL.'), '<link rel="icon">'),
                        'type' => 'string',
                        'format' => 'uri',
                        'context' => ['view', 'edit', 'embed'],
                        'readonly' => true,
                    ...
                    'description' => [
                        'description' => sprintf(/* translators: %s: HTML meta tag. */ __('The content of the %s element from the URL.'), '<meta name="description">'),
                        'type' => 'string',
                        'context' => ['view', 'edit', 'embed'],
                        'readonly' => true,
                    ],
                    'image' => [
                        'description' => sprintf(/* translators: 1: HTML meta tag, 2: HTML meta tag. */ __('The Open Graph image link of the %1$s or %2$s element from the URL.'), '<meta property="og:image">', '<meta property="og:image:url">'),
                        'type' => 'string',
                        'format' => 'uri',
                        'context' => ['view', 'edit', 'embed'],
                        'readonly' => true,
        ...
        private function get_remote_url($url)
        {
            ...
            */
            ...
            if(WP_Http::OK !== wp_remote_retrieve_response_code($response))
            {
                ...
                return new WP_Error('no_response', __('URL not found. Response returned a non-200 status code for this URL.'), ['status' => WP_Http::NOT_FOUND]);
            }
            $remote_body = wp_remote_retrieve_body($response);
            if(empty($remote_body))
            {
                return new WP_Error('no_content', __('Unable to retrieve body from response at this URL.'), ['status' => WP_Http::NOT_FOUND]);
            }
        ...
        public function permissions_check()
        {
            ...
            return new WP_Error('rest_cannot_view_url_details', __('Sorry, you are not allowed to process remote URLs.'), ['status' => rest_authorization_required_code()]);
        }
    }
