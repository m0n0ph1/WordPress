<?php
    ...
    class WP_REST_Attachments_Controller extends WP_REST_Posts_Controller
    {
        ...
        public function register_routes()
        {
            ...
            register_rest_route($this->namespace, '/'.$this->rest_base.'/(?P<id>[\d]+)/post-process', [
                'methods' => WP_REST_Server::CREATABLE,
                'callback' => [$this, 'post_process_item'],
            ...
            register_rest_route($this->namespace, '/'.$this->rest_base.'/(?P<id>[\d]+)/edit', [
                'methods' => WP_REST_Server::CREATABLE,
                'callback' => [$this, 'edit_media_item'],
        ...
        public function create_item_permissions_check($request)
        {
            ...
            {
                return new WP_Error('rest_cannot_create', __('Sorry, you are not allowed to upload media on this site.'), ['status' => 400]);
            }
            // Attaching media to a post requires ability to edit said post.
            if(! empty($request['post']) && ! current_user_can('edit_post', (int) $request['post']))
            {
                return new WP_Error('rest_cannot_edit', __('Sorry, you are not allowed to upload media to this post.'), ['status' => rest_authorization_required_code()]);
            }
        ...
        {
            if(
                ! empty($request['post']) && in_array(get_post_type($request['post']), [
                    'revision',
                    'attachment'
                ],                                    true)
            )
        ...
        protected function upload_from_file($files, $headers)
        {
            ...
            if(! empty($headers['content_md5']))
            {
                ...
                if($expected !== $actual)
                {
                    return new WP_Error('rest_upload_hash_mismatch', __('Content hash did not match expected.'), ['status' => 412]);
                }
            }
        ...
        protected function check_upload_size($file)
        {
            ...
            {
                return new WP_Error('rest_upload_limited_space', /* translators: %s: Required disk space in kilobytes. */ sprintf(__('Not enough space to upload. %s KB needed.'), number_format(($file_size - $space_left) / KB_IN_BYTES)), ['status' => 400]);
            }
            if($file_size > (KB_IN_BYTES * get_site_option('fileupload_maxk', 1500)))
            {
                return new WP_Error('rest_upload_file_too_big', /* translators: %s: Maximum allowed file size in kilobytes. */ sprintf(__('This file is too big. Files must be less than %s KB in size.'), get_site_option('fileupload_maxk', 1500)), ['status' => 400]);
            }
            // Include multisite admin functions to get access to upload_is_user_over_quota().
            require_once ABSPATH.'wp-admin/includes/ms.php';
            if(upload_is_user_over_quota(false))
            {
                return new WP_Error('rest_upload_user_quota_exceeded', __('You have used your space quota. Please delete files before uploading.'), ['status' => 400]);
            }
        ...
        protected function upload_from_data($data, $headers)
        {
            ...
            if(empty($filename))
            {
                return new WP_Error('rest_upload_invalid_disposition', __('Invalid Content-Disposition supplied. Content-Disposition needs to be formatted as `attachment; filename="image.png"` or similar.'), ['status' => 400]);
            }
            if(! empty($headers['content_md5']))
            {
                $content_md5 = array_shift($headers['content_md5']);
        ...
        public function get_item_schema()
        {
            ...
            $schema['properties']['media_details'] = [
                'description' => __('Details about the media file, specific to its type.'),
                'type' => 'object',
                'context' => ['view', 'edit', 'embed'],
                'readonly' => true,
            ];
            $schema['properties']['post'] = [
                'description' => __('The ID for the associated post of the attachment.'),
                'type' => 'integer',
                'context' => ['view', 'edit'],
            ];
            $schema['properties']['source_url'] = [
            ...
            $schema['properties']['missing_image_sizes'] = [
        ...
        public function edit_media_item_permissions_check($request)
        {
            if(! current_user_can('upload_files'))
            {
                return new WP_Error('rest_cannot_edit_image', __('Sorry, you are not allowed to upload media on this site.'), ['status' => rest_authorization_required_code()]);
            }
        ...
        public function edit_media_item($request)
        {
            ...
            {
                return new WP_Error('rest_unknown_attachment', __('Unable to get meta information for file.'), ['status' => 404]);
            }
            $supported_types = ['image/jpeg', 'image/png', 'image/gif', 'image/webp'];
            $mime_type = get_post_mime_type($attachment_id);
            if(! in_array($mime_type, $supported_types, true))
            {
                return new WP_Error('rest_cannot_edit_file_type', __('This type of file cannot be edited.'), ['status' => 400]);
            }
            ...
            else
            {
                ...
                {
                    return new WP_Error('rest_image_not_edited', __('The image was not edited. Edit the image before applying the changes.'), ['status' => 400]);
                }
            }
            /*
            ...
            {
                return new WP_Error('rest_unknown_image_file_type', __('Unable to edit this image.'), ['status' => 500]);
            }
                ...
                switch($modifier['type'])
                {
                    case 'rotate':
                        ...
                        if(0 !== $rotate)
                        {
                            ...
                            {
                                return new WP_Error('rest_image_rotation_failed', __('Unable to rotate this image.'), ['status' => 500]);
                            }
                        }
                        break;
                    case 'crop':
                        ...
                        if($size['width'] !== $width && $size['height'] !== $height)
                        {
                            ...
                            if(is_wp_error($result))
                            {
                                return new WP_Error('rest_image_crop_failed', __('Unable to crop this image.'), ['status' => 500]);
                            }
                        }
                        break;
                }
            ...
            if(is_wp_error($new_attachment_id))
            {
                ...
                {
                    $new_attachment_id->add_data(['status' => 500]);
                }
                else
                {
                    $new_attachment_id->add_data(['status' => 400]);
                }
        ...
        public function get_collection_params()
        {
            ...
            $params['status']['items']['enum'] = ['inherit', 'private', 'trash'];
