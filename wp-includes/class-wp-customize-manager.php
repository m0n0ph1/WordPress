<?php
    ...
    final class WP_Customize_Manager
    {
         ...
         *                                                 Args.
         *
         * @type null|string|false $changeset_uuid         Changeset UUID, the `post_name` for the customize_changeset post
         *                                                 containing the customized state. Defaults to `null` resulting in a UUID to be immediately generated. If
         *                                                 `false` is provided, then then the changeset UUID will be determined during `after_setup_theme`: when
         *                                                 the
         *                                                 `customize_changeset_branching` filter returns false, then
         *                                                 the default UUID will be that of the most recent `customize_changeset` post that has a status other than
         *                                                 'auto-draft',
         *                                                 'publish', or 'trash'. Otherwise, if changeset branching is
         *                                                 enabled, then a random UUID will be used.
         * @type string            $theme                  Theme to be previewed (for theme switch). Defaults to customize_theme or theme query
         *                                                 params.
         ...
         * @type bool              $autosaved              If data from a changeset's autosaved revision should be loaded if it exists. Defaults
         *                                                 to false.
         *                                                 }
		 ...
		 */
            ...
            ],         11); // Allow code to create settings first.
		 ...
		 */
            $has_post_data_nonce = (check_ajax_referer('preview-customize_'.$this->get_stylesheet(), 'nonce', false) || check_ajax_referer('save-customize_'.$this->get_stylesheet(), 'nonce', false) || check_ajax_referer('preview-customize_'.$this->get_stylesheet(), 'customize_preview_nonce', false));
         ...
         *
         * This method runs right at after_setup_theme and applies the 'customize_changeset_branching' filter to
         * determine whether concurrent changesets are allowed. Then if the Customizer is not initialized with a
         * `changeset_uuid` param, this method will determine which UUID should be used. If changeset branching is
         * disabled, then the most saved changeset will be loaded by default. Otherwise, if there are no existing saved
         ...
         * @type bool   $exclude_restore_dismissed Whether to exclude changeset auto-drafts that have been dismissed.
         *                                         Defaults to true.
         *                                         }
		 ...
		 */
            ...
            {
                $this->wp_die(-1, sprintf(/* translators: %s: customize_messenger_channel */ __('Unauthorized. You may remove the %s param to preview as frontend.'), '<code>customize_messenger_channel<code>'));
        ...
        {
            $controls = [];
            $this->controls = wp_list_sort($this->controls, [
                'priority' => 'ASC',
                'instance_number' => 'ASC',
            ],                             'ASC', true);
            ...
            foreach($this->sections as $section)
            {
                ...
                $section->controls = wp_list_sort($section->controls, [
                    'priority' => 'ASC',
                    'instance_number' => 'ASC',
                ]);
                if(! $section->panel)
            ...
            $this->containers = wp_list_sort($this->containers, [
                'priority' => 'ASC',
                'instance_number' => 'ASC',
            ],                               'ASC', true);
        }
        ...
        public function import_theme_starter_content($starter_content = [])
        {
            ...
            $all_post_slugs = array_merge(wp_list_pluck($attachments, 'post_name'), wp_list_pluck($posts, 'post_name'));
            /*
		 ...
		 */
                ...
                foreach($attachments as $symbol => $attachment)
                {
                    ...
                    if(! $attachment_id)
                    {
                        ...
                        $attachment_post_data = array_merge(
                            wp_array_slice_assoc($attachment, [
                                'post_title',
                                'post_content',
                                'post_excerpt'
                            ]), [
            ...
            foreach($nav_menus as $nav_menu_location => $nav_menu)
            {
                ...
                $this->set_post_value($nav_menu_setting_id, [
         ...
         * @type bool   $exclude_post_data Whether the post input values should also be excluded. Defaults to false when
         ...
         * @since 4.9.0 This will return the changeset's data with a user's autosave revision merged on top, if one
         *     exists and $autosaved is true.
         *
         * @since 4.7.0
         */
         ...
         *
         * @type array  $data            Optional additional changeset data. Values will be merged on top of any existing post
         *                               values.
         * @type string $status          Post status. Optional. If supplied, the save will be transactional and a post revision
         *                               will be allowed.
         ...
         * @type bool   $starter_content Whether the data is starter content. If false (default), then $starter_content
        ...
        public function save_changeset_post($args = [])
        {
            ...
            if($changeset_post_id)
            {
                ...
                {
                    return new WP_Error('changeset_already_published', __('The previous set of changes has already been published. Please try saving your current set of changes again.'), [
		 ...
		 */
            ...
            $setting_validities = $this->validate_setting_values($validated_values, [
		 ...
		 */
            ...
            foreach($args['data'] as $setting_id => $setting_params)
            {
                ...
                else
                {
                    ...
                    $data[$changeset_setting_id] = array_merge($merged_setting_params, [
                        'type' => $setting->type,
        ...
        public function add_state_query_params($url)
        {
            ...
            foreach($this->get_allowed_urls() as $allowed_url)
            {
                ...
                $is_allowed = ($parsed_allowed_url['scheme'] === $parsed_original_url['scheme'] && $parsed_allowed_url['host'] === $parsed_original_url['host'] && str_starts_with($parsed_original_url['path'], $parsed_allowed_url['path']));
                if($is_allowed)
                {
                    break;
                }
        ...
        {
            if(isset($data['post_type']) && isset($unsanitized_postarr['post_content']) && 'customize_changeset' === $data['post_type'] || ('revision' === $data['post_type'] && ! empty($data['post_parent']) && 'customize_changeset' === get_post_type($data['post_parent'])))
            {
        ...
        protected function get_lock_user_data($user_id)
        {
            ...
            return [
                ...
                'avatar' => get_avatar_url($lock_user->ID, ['size' => 128]),
            ];
        }
        ...
        public function check_changeset_lock_with_heartbeat($response, $data, $screen_id)
        {
            ...
            if(array_key_exists('check_changeset_lock', $data) && 'customize' === $screen_id && $changeset_post_id && current_user_can(get_post_type_object('customize_changeset')->cap->edit_post, $changeset_post_id))
            {
         ...
         *
         * @param int   $changeset_post_id ID for customize_changeset post. Defaults to the changeset for the current
         *                                 manager instance.
         *
		 ...
		 */
            ...
            foreach($this->_changeset_data as $raw_setting_id => $setting_params)
            {
                ...
                $is_theme_mod_setting = (isset($setting_params['value']) && isset($setting_params['type']) && 'theme_mod' === $setting_params['type'] && preg_match($namespace_pattern, $raw_setting_id, $matches));
         ...
         *
         * @since 3.4.0
         * @since 4.7.0 The semantics of this method have changed to update a changeset, optionally to also change the
        ...
        public function save()
        {
            ...
            if(isset($_POST['customize_changeset_status']))
            {
                ...
                if(
                    ! get_post_status_object($changeset_status) || ! in_array($changeset_status, [
                        'draft',
                        'pending',
                        'publish',
                        'future'
		 ...
		 */
            ...
            if($autosave && ! defined('DOING_AUTOSAVE'))
            { // Back-compat.
            ...
            {
                $r = new WP_Error('changeset_locked', __('Changeset is being edited by other user.'), [
            ...
            {
                $response['setting_validities'] = array_map([
                                                                $this,
         ...
         *
         * @return array|false Returns array of updated stashed theme mods or false if the update failed or there were
         *     no changes.
         * @since 4.7.0
         *
         */
        ...
        public function remove_panel($id)
        {
            ...
            {
                _doing_it_wrong(__METHOD__, sprintf(/* translators: 1: Panel ID, 2: Link to 'customize_loaded_components' filter reference. */ __('Removing %1$s manually will cause PHP warnings. Use the %2$s filter instead.'), $id, sprintf('<a href="%1$s">%2$s</a>', esc_url('https://developer.wordpress.org/reference/hooks/customize_loaded_components/'), '<code>customize_loaded_components</code>')), '4.5.0');
            }
        ...
        public function render_control_templates()
        {
            ...
            <script type="text/html" id="tmpl-customize-notification">
                <li class="notice notice-{{ data.type || 'info' }} {{ data.alt ? 'notice-alt' : '' }} {{ data.dismissible ? 'is-dismissible' : '' }} {{ data.containerClasses || '' }}"
                    data-code="{{ data.code }}"
                    data-type="{{ data.type }}">
            ...
            <script type="text/html" id="tmpl-customize-changeset-locked-notification">
                <li class="notice notice-{{ data.type || 'info' }} {{ data.containerClasses || '' }}"
                    data-code="{{ data.code }}"
                    data-type="{{ data.type }}">
                    <div class="notification-message customize-changeset-locked-message">
                        <img class="customize-changeset-locked-avatar"
                             src="{{ data.lockUser.avatar }}"
                             alt="{{ data.lockUser.name }}"/>
                        ...
                        <p class="action-buttons">
                            ...
                            <a class="button customize-notice-go-back-button"
                               href="{{ data.returnUrl }}"><?php _e('Go back'); ?></a>
                            <# } #>
                            <a class="button customize-notice-preview-button"
                               href="{{ data.frontendPreviewUrl }}"><?php _e('Preview'); ?></a>
            ...
            <script type="text/html" id="tmpl-customize-control-notifications">
                <ul>
                    ...
                    <li class="notice notice-{{ notification.type || 'info' }} {{ data.altNotice ? 'notice-alt' : '' }}"
                        data-code="{{ notification.code }}"
                        data-type="{{ notification.type }}">{{{ notification.message || notification.code }}}
                    </li>
                    <# } ); #>
                </ul>
            </script>
            ...
            <script type="text/html" id="tmpl-customize-preview-link-control">
                ...
                <div class="preview-link-wrapper">
                    ...
                    <input id="{{ elementPrefix }}customize-preview-link-input"
                           readonly
                           tabindex="-1"
                           class="preview-control-element"
                           data-component="input">
                    <button class="customize-copy-preview-link preview-control-element button button-secondary"
                            data-component="button"
                            data-copy-text="<?php esc_attr_e('Copy'); ?>"
                            data-copied-text="<?php esc_attr_e('Copied'); ?>"><?php esc_html_e('Copy'); ?></button>
                </div>
            </script>
        ...
        {
            $this->autofocus = array_filter(
                wp_array_slice_assoc($autofocus, [
                    'panel',
                    'section',
                    'control'
                ]), 'is_string'
        ...
        public function register_controls()
        {
            ...
            $this->add_panel(
                new WP_Customize_Themes_Panel($this, 'themes', [
                    'title' => $this->theme()->display('Name'),
                    'description' => ('<p>'.__('Looking for a theme? You can search or browse the WordPress.org theme directory, install and preview themes, then activate them right here.').'</p>'.'<p>'.__('While previewing a new theme, you can continue to tailor things like widgets and menus, and explore theme-specific options.').'</p>'),
                    'capability' => 'switch_themes',
                    'priority' => 0,
                ])
            );
            $this->add_section(
                new WP_Customize_Themes_Section($this, 'installed_themes', [
            ...
            {
                $this->add_section(
                    new WP_Customize_Themes_Section($this, 'wporg_themes', [
            ...
            $this->add_setting(
                new WP_Customize_Filter_Setting($this, 'active_theme', [
                    'capability' => 'switch_themes',
                ])
            );
            /* Site Identity */
            ...
            $this->add_control(
                new WP_Customize_Site_Icon_Control($this, 'site_icon', [
                    'label' => __('Site Icon'),
                    'description' => sprintf('<p>'.__('Site Icons are what you see in browser tabs, bookmark bars, and within the WordPress mobile apps. Upload one here!').'</p>'./* translators: %s: Site icon size in pixels. */ '<p>'.__('Site Icons should be square and at least %s pixels.').'</p>', '<strong>512 &times; 512</strong>'),
                    'section' => 'title_tagline',
                    'priority' => 60,
                    'height' => 512,
                    'width' => 512,
            ...
            $this->add_control(
                new WP_Customize_Cropped_Image_Control($this, 'custom_logo', [
                    'label' => __('Logo'),
                    'section' => 'title_tagline',
                    'priority' => 8,
            ...
            $this->add_control(
                new WP_Customize_Color_Control($this, 'header_textcolor', [
            ...
            $this->add_control(
                new WP_Customize_Color_Control($this, 'background_color', [
            ...
            if(current_theme_supports('custom-header', 'video'))
            {
                ...
                if($width && $height)
                {
                    $control_description = sprintf(/* translators: 1: .mp4, 2: Header size in pixels. */ __('Upload your video in %1$s format and minimize its file size for best results. Your theme recommends dimensions of %2$s pixels.'), '<code>.mp4</code>', sprintf('<strong>%s &times; %s</strong>', $width, $height));
                }
                elseif($width)
                {
                    $control_description = sprintf(/* translators: 1: .mp4, 2: Header width in pixels. */ __('Upload your video in %1$s format and minimize its file size for best results. Your theme recommends a width of %2$s pixels.'), '<code>.mp4</code>', sprintf('<strong>%s</strong>', $width));
                }
                else
                {
                    $control_description = sprintf(/* translators: 1: .mp4, 2: Header height in pixels. */ __('Upload your video in %1$s format and minimize its file size for best results. Your theme recommends a height of %2$s pixels.'), '<code>.mp4</code>', sprintf('<strong>%s</strong>', $height));
                }
            }
            else
            {
            ...
            $this->add_setting('external_header_video', [
                'theme_supports' => ['custom-header', 'video'],
                'transport' => 'postMessage',
		 ...
		 */
            ...
            $this->add_control(
                new WP_Customize_Media_Control($this, 'header_video', [
                    'theme_supports' => ['custom-header', 'video'],
            ...
            $this->add_control('external_header_video', [
                'theme_supports' => ['custom-header', 'video'],
                'type' => 'url',
            ...
            $this->selective_refresh->add_partial('custom_header', [
                'selector' => '#wp-custom-header',
                'render_callback' => 'the_custom_header_markup',
                'settings' => ['header_video', 'external_header_video', 'header_image'],
            ...
            $this->add_setting(
                new WP_Customize_Background_Image_Setting($this, 'background_image_thumb', [
                    'theme_supports' => 'custom-background',
                    'sanitize_callback' => [$this, '_sanitize_background_setting'],
                ])
            );
            $this->add_control(new WP_Customize_Background_Image_Control($this));
            $this->add_setting('background_preset', [
                'default' => get_theme_support('custom-background', 'default-preset'),
                'theme_supports' => 'custom-background',
                'sanitize_callback' => [$this, '_sanitize_background_setting'],
            ]);
            $this->add_control('background_preset', [
            ...
            $this->add_setting('background_position_y', [
                'default' => get_theme_support('custom-background', 'default-position-y'),
                'theme_supports' => 'custom-background',
                'sanitize_callback' => [$this, '_sanitize_background_setting'],
            ]);
            $this->add_control(
                new WP_Customize_Background_Position_Control($this, 'background_position', [
            ...
            $this->add_setting('background_size', [
		 ...
		 */
            ...
            $section_description .= sprintf(' <a href="%1$s" class="external-link" target="_blank">%2$s<span class="screen-reader-text"> %3$s</span></a>', esc_url(__('https://wordpress.org/documentation/article/css/')), __('Learn more about CSS'), /* translators: Hidden accessibility text. */ __('(opens in a new tab)'));
            $section_description .= '</p>';
            ...
            $custom_css_setting = new WP_Customize_Custom_CSS_Setting($this, sprintf('custom_css[%s]', get_stylesheet()), [
                'capability' => 'edit_css',
                'default' => '',
            ]);
            $this->add_setting($custom_css_setting);
            $this->add_control(
                new WP_Customize_Code_Editor_Control($this, 'custom_css', [
                    'label' => __('CSS code'),
                    'section' => 'custom_css',
        ...
        public function _validate_header_video($validity, $value)
        {
            ...
            if($video)
            {
                ...
                if(! str_ends_with($video, '.mp4') && ! str_ends_with($video, '.mov'))
                { // Check for .mp4 or .mov format, which (assuming h.264 encoding) are the only cross-browser-supported formats.
                    $validity->add('invalid_file_type', sprintf(/* translators: 1: .mp4, 2: .mov */ __('Only %1$s or %2$s files may be used for header video. Please convert your video file and try again, or, upload your video to YouTube and link it with the option below.'), '<code>.mp4</code>', '<code>.mov</code>'));
                }
            }
