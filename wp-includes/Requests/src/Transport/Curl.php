            ...
            do
            {
                ...
                foreach($to_process as $key => $done)
                {
                    ...
                    if($done['result'] !== CURLE_OK)
                    {
                        ...
                        $exception = new CurlException($reason, CurlException::EASY, $done['handle'], $done['result']);
                        $responses[$key] = $exception;
                        $options['hooks']->dispatch('transport.internal.parse_error', [
                            &$responses[$key],
                            $requests[$key]
                        ]);
                    }
                    else
                    {
                        ...
                        $options['hooks']->dispatch('transport.internal.parse_response', [
                            &$responses[$key],
                            $requests[$key]
                        ]);
                    }
