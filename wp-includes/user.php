<?php
    ...
    function wp_authenticate_username_password($user, $username, $password)
    {
        ...
        if(! $user)
        {
            return new WP_Error('invalid_username', sprintf(/* translators: %s: User name. */ __('<strong>Error:</strong> The username <strong>%s</strong> is not registered on this site. If you are unsure of your username, try your email address instead.'), $username));
        }
        ...
        {
            return new WP_Error('incorrect_password', sprintf(/* translators: %s: User name. */ __('<strong>Error:</strong> The password you entered for the username %s is incorrect.'), '<strong>'.$username.'</strong>').' <a href="'.wp_lostpassword_url().'">'.__('Lost your password?').'</a>');
        }
    ...
    function wp_authenticate_email_password($user, $email, $password)
    {
        ...
        if(! wp_check_password($password, $user->user_pass, $user->ID))
        {
            return new WP_Error('incorrect_password', sprintf(/* translators: %s: Email address. */ __('<strong>Error:</strong> The password you entered for the email address %s is incorrect.'), '<strong>'.$email.'</strong>').' <a href="'.wp_lostpassword_url().'">'.__('Lost your password?').'</a>');
        }
...
//
    ...
    function wp_list_users($args = [])
    {
        ...
        foreach($users as $user_id)
        {
            ...
            {
                $name = sprintf(/* translators: 1: User's first name, 2: Last name. */ _x('%1$s %2$s', 'Display name based on first name and last name'), $user->first_name, $user->last_name);
            }
            else
            {
    ...
    function get_user_count($network_id = null)
    {
        ...
        {
            _doing_it_wrong(__FUNCTION__, sprintf(/* translators: %s: $network_id */ __('Unable to pass %s if not using multisite.'), '<code>$network_id</code>'), '6.0.0');
        }
...
//
    ...
    function wp_dropdown_users($args = '')
    {
        ...
        $query_args = wp_array_slice_assoc($parsed_args, [
            'blog_id',
            'include',
            'exclude',
            'orderby',
	 ...
	 */
        ...
        if(empty($userdata['display_name']))
        {
            ...
            {
                $display_name = sprintf(/* translators: 1: User's first name, 2: Last name. */ _x('%1$s %2$s', 'Display name based on first name and last name'), $meta['first_name'], $meta['last_name']);
            }
...
###SITEURL###'
    ...
    function retrieve_password($user_login = null)
    {
        ...
        if(! is_user_logged_in())
        {
            ...
            if($requester_ip)
            {
                $message .= sprintf(/* translators: %s: IP address of password reset requester. */ __('This password reset request originated from the IP address %s.'), $requester_ip)."\r\n";
            }
        }
	 ...
	 */
        ...
        {
            $errors->add('retrieve_password_email_failure', sprintf(/* translators: %s: Documentation URL. */ __('<strong>Error:</strong> The email could not be sent. Your site may not be correctly configured to send emails. <a href="%s">Get support for resetting your password</a>.'), esc_url(__('https://wordpress.org/documentation/article/reset-your-password/'))));
    ...
    function register_new_user($user_login, $user_email)
    {
        ...
        {
            $errors->add('email_exists', sprintf(/* translators: %s: Link to the login page. */ __('<strong>Error:</strong> This email address is already registered. <a href="%s">Log in</a> with this address or choose another one.'), wp_login_url()));
        }
        ...
        {
            $errors->add('registerfail', sprintf(/* translators: %s: Admin email address. */ __('<strong>Error:</strong> Could not register you&hellip; please contact the <a href="mailto:%s">site admin</a>!'), get_option('admin_email')));
...
###SITEURL###'
    ...
    function new_user_email_admin_notice()
    {
        ...
        if('profile.php' === $pagenow && isset($_GET['updated']))
        {
            ...
            if($email)
            {
                $message = sprintf(/* translators: %s: New email address. */ __('Your email address has not been updated yet. Please check your inbox at %s for a confirmation email.'), '<code>'.esc_html($email['newemail']).'</code>');
    ...
    function wp_user_personal_data_exporter($email_address)
    {
        ...
        if(is_array($_extra_data) && ! empty($_extra_data))
        {
            ...
            {
                _doing_it_wrong(__FUNCTION__, sprintf(/* translators: %s: wp_privacy_additional_user_profile_data */ __('Filter %s returned items with reserved names.'), '<code>wp_privacy_additional_user_profile_data</code>'), '5.4.0');
            }
    ...
    function _wp_privacy_send_request_confirmation_notification($request_id)
    {
        ...
        $subject = sprintf(/* translators: Privacy data request confirmed notification email subject. 1: Site title, 2: Name of the confirmed action. */ __('[%1$s] Action Confirmed: %2$s'), $email_data['sitename'], $action_description);
...
###SITEURL###'
         ...
         *
         */
        $content = apply_filters_deprecated('user_confirmed_action_email_content', [
            $content,
            $email_data
        ],                                  '5.8.0', sprintf(/* translators: 1 & 2: Deprecation replacement options. */ __('%1$s or %2$s'), 'user_request_confirmed_email_content', 'user_erasure_fulfillment_email_content'));
    ...
    function _wp_privacy_send_erasure_fulfillment_notification($request_id)
    {
        ...
        $subject = sprintf(/* translators: Erasure request fulfilled notification email subject. %s: Site title. */ __('[%s] Erasure Request Fulfilled'), $email_data['sitename']);
         ...
         * @since      4.9.8
         */
        $subject = apply_filters_deprecated('user_erasure_complete_email_subject', [
            $subject,
            $email_data['sitename'],
            $email_data
        ],                                  '5.8.0', 'user_erasure_fulfillment_email_subject');
...
###SITEURL###'
        ...
        $content = apply_filters_deprecated('user_confirmed_action_email_content', [
            $content,
            $email_data
        ],                                  '5.8.0', sprintf(/* translators: 1 & 2: Deprecation replacement options. */ __('%1$s or %2$s'), 'user_erasure_fulfillment_email_content', 'user_request_confirmed_email_content'));
        ...
        $headers = apply_filters_deprecated('user_erasure_complete_email_headers', [
            ...
            $request_id,
            $email_data
        ],                                  '5.8.0', 'user_erasure_fulfillment_email_headers');
    ...
    function wp_user_request_action_description($action_name)
    {
        switch($action_name)
        {
            ...
            case 'remove_personal_data':
                ...
                break;
            default:
                /* translators: %s: Action name. */ $description = sprintf(__('Confirm the "%s" action'), $action_name);
                break;
        }
