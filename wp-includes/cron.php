<?php
    ...
    function wp_clear_scheduled_hook($hook, $args = [], $wp_error = false)
    {
        ...
        if($errors)
        {
            if($wp_error)
            {
                array_walk($errors, [$error, 'merge_from']);
     ...
     *                               event, so they should be the same as those used when originally scheduling the event.
     *                               Default empty array.
     * @param int|null    $timestamp Optional. Unix timestamp (UTC) of the event. If not specified, the next scheduled
     *                               event is returned. Default null.
     *
     * @return object|false {
