<?php
    ...
    function wp_default_packages_vendor($scripts)
    {
        ...
        $vendor_scripts = [
            ...
            'wp-polyfill-inert',
            'wp-polyfill' => ['wp-polyfill-inert', 'regenerator-runtime'],
        ];
        ...
        did_action('init') && $scripts->add_inline_script(
            'moment', sprintf(
            "moment.updateLocale( '%s', %s );", get_user_locale(), wp_json_encode([
    ...
    function wp_get_script_polyfill($scripts, $tests)
    {
        ...
        foreach($tests as $test => $handle)
        {
            ...
            $polyfill .= (// Test presence of feature...
                '( '.$test.' ) || './*
    ...
    function wp_default_packages_inline_scripts($scripts)
    {
        ...
        $scripts->add_inline_script(
            'wp-api-fetch', implode("\n", [
            sprintf('wp.apiFetch.nonceMiddleware = wp.apiFetch.createNonceMiddleware( "%s" );', wp_installing() ? '' : wp_create_nonce('wp_rest')),
        ...
        $scripts->add_inline_script(
            'wp-date', sprintf(
            'wp.date.setSettings( %s );', wp_json_encode([
                                                             'l10n' => [
    ...
    function wp_default_scripts($scripts)
    {
        ...
        $scripts->add('common', "/wp-admin/js/common$suffix.js", ['jquery', 'hoverIntent', 'utils'], false, 1);
        $scripts->set_translations('common');

        $scripts->add('wp-sanitize', "/wp-includes/js/wp-sanitize$suffix.js", [], false, 1);
        ...
        $scripts->add('editor', "/wp-admin/js/editor$suffix.js", ['utils', 'jquery'], false, 1);
        ...
        $scripts->add('wp-lists', "/wp-includes/js/wp-lists$suffix.js", ['wp-ajax-response', 'jquery-color'], false, 1);
        ...
        $scripts->add('cropper', '/wp-includes/js/crop/cropper.js', ['scriptaculous-dragdrop']);
	 ...
	 */
        ...
        $scripts->add('jquery-core', "/wp-includes/js/jquery/jquery$suffix.js", [], '3.7.1');
        $scripts->add('jquery-migrate', "/wp-includes/js/jquery/jquery-migrate$suffix.js", [], '3.4.1');
	 ...
	 */
        $scripts->add('jquery-ui-core', "/wp-includes/js/jquery/ui/core$suffix.js", ['jquery'], '1.13.2', 1);
        $scripts->add('jquery-effects-core', "/wp-includes/js/jquery/ui/effect$suffix.js", ['jquery'], '1.13.2', 1);
	 ...
	 */
        $scripts->add('imagesloaded', '/wp-includes/js/imagesloaded.min.js', [], '5.0.0', 1);
        $scripts->add('masonry', '/wp-includes/js/masonry.min.js', ['imagesloaded'], '4.2.2', 1);
        $scripts->add('jquery-masonry', '/wp-includes/js/jquery/jquery.masonry.min.js', [
            'jquery',
            'masonry'
        ],            '3.1.2b', 1);
        ...
        // Back compat handles:
        foreach(['all', 'html5', 'flash', 'silverlight', 'html4'] as $handle)
        {
        ...
        $scripts->add('swfupload-handlers', "/wp-includes/js/swfupload/handlers$suffix.js", [
            'swfupload-all',
            'jquery'
        ],            '2201-20110524');
        ...
        $scripts->add('json2', "/wp-includes/js/json2$suffix.js", [], '2015-05-03');
        did_action('init') && $scripts->add_data('json2', 'conditional', 'lt IE 8');

        $scripts->add('underscore', "/wp-includes/js/underscore$dev_suffix.js", [], '1.13.4', 1);
        $scripts->add('backbone', "/wp-includes/js/backbone$dev_suffix.js", ['underscore', 'jquery'], '1.5.0', 1);
        ...
        $scripts->add('revisions', "/wp-admin/js/revisions$suffix.js", [
            'wp-backbone',
            'jquery-ui-slider',
            'hoverIntent'
        ],            false, 1);
        ...
        $scripts->add('auth-app', "/wp-admin/js/auth-app$suffix.js", [
            'jquery',
            'wp-api-request',
            'wp-i18n',
            'wp-hooks'
        ...
        $scripts->add('word-count', "/wp-admin/js/word-count$suffix.js", [], false, 1);
        ...
        $scripts->add('customize-preview', "/wp-includes/js/customize-preview$suffix.js", [
            'wp-a11y',
            'customize-base'
        ],            false, 1);
        ...
        did_action('init') && $scripts->localize('customize-controls', '_wpCustomizeControlsL10n', [
            ...
            'themeInstallUnavailable' => sprintf(/* translators: %s: URL to Add Themes admin screen. */ __('You will not be able to install new themes from here yet since your install requires SFTP credentials. For now, please <a href="%s">add themes in the admin</a>.'), esc_url(admin_url('theme-install.php'))),
            'publishSettings' => __('Publish Settings'),
            'invalidDate' => __('Invalid date.'),
            'invalidValue' => __('Invalid value.'),
            'blockThemeNotification' => sprintf(/* translators: 1: Link to Site Editor documentation on HelpHub, 2: HTML button. */ __('Hurray! Your theme supports site editing with blocks. <a href="%1$s">Tell me more</a>. %2$s'), __('https://wordpress.org/documentation/article/site-editor/'), sprintf('<button type="button" data-action="%1$s" class="button switch-to-editor">%2$s</button>', esc_url(admin_url('site-editor.php')), __('Use Site Editor'))),
        ]);
	 ...
	 */
        ...
        $scripts->add('wp-api', "/wp-includes/js/wp-api$suffix.js", [
            'jquery',
            'backbone',
            'underscore',
            'wp-api-request'
        ...
        if(is_admin())
        {
            ...
            did_action('init') && $scripts->localize('admin-comments', 'adminCommentsSettings', [
            ...
            $scripts->add('theme', "/wp-admin/js/theme$suffix.js", [
                'wp-backbone',
                'wp-a11y',
                'customize-base'
            ],            false, 1);
            ...
            $scripts->add('media', "/wp-admin/js/media$suffix.js", [
                'jquery',
                'clipboard',
                'wp-i18n',
                'wp-a11y'
	 ...
	 */
        ...
        $styles->add('colors', true, ['wp-admin', 'buttons']);
        ...
        $styles->add('mediaelement', '/wp-includes/js/mediaelement/mediaelementplayer-legacy.min.css', [], '4.2.17');
        ...
        $styles->add('farbtastic', "/wp-admin/css/farbtastic$suffix.css", [], '1.3u1');
        $styles->add('jcrop', '/wp-includes/js/jcrop/jquery.Jcrop.min.css', [], '0.9.15');
        $styles->add('colors-fresh', false, ['wp-admin', 'buttons']); // Old handle.
	 ...
	 */
        ...
        $styles->add('wp-reset-editor-styles', "/wp-includes/css/dist/block-library/reset$suffix.css", [
            'common',
            'forms'
        ] // Make sure the reset is loaded after the default WP Admin styles.
        );

        $styles->add('wp-editor-classic-layout-styles', "/wp-includes/css/dist/edit-post/classic$suffix.css", []);

        $styles->add('wp-block-editor-content', "/wp-includes/css/dist/block-editor/content$suffix.css", ['wp-components']);
	 ...
	 */
        ...
        wp_localize_script('dashboard', 'communityEventsData', [
    ...
    function wp_enqueue_global_styles_css_custom_properties()
    {
        ...
        wp_add_inline_style('global-styles-css-custom-properties', wp_get_global_stylesheet(['variables']));
