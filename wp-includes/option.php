<?php
     ...
     *
     * Autoloading too many options can lead to performance problems, especially if the options are not frequently
     * used.
     * This function allows modifying the autoload value for multiple options without changing the actual option value.
     * This is for example recommended for plugin activation and deactivation hooks, to ensure any options exclusively
     * used by the plugin which are generally autoloaded can be set to not autoload when the plugin is inactive.
     *
     * @param array $options Associative array of option names and their autoload values to set. The option names are
     *                       expected to not be SQL-escaped. The autoload values accept 'yes'|true to enable or
     *                       'no'|false to disable.
     *
     * @return array Associative array of all provided $options as keys and boolean values for whether their autoload
     *     value was updated.
     * @since 6.4.0
     *
     * @global wpdb $wpdb    WordPress database abstraction object.
     ...
     *
     * This is a wrapper for {@see wp_set_option_autoload_values()}, which can be used to set different autoload values
     * for each option at once.
     *
     ...
     *
     * @param int   $network_id Optional. Network ID of network for which to prime network options cache. Defaults to
     *                          current network.
     *
     ...
     * @param string|bool $autoload Optional. Whether to load the option when WordPress starts up. For existing
     *                              options,
     *                              `$autoload` can only be updated using `update_option()` if `$value` is also
     *                              changed.
     *                              Accepts 'yes'|true to enable or 'no'|false to disable.
     *                              Autoloading too many options can lead to performance problems, especially if the
     *                              options are not frequently used. For options which are accessed across several
     *                              places in the frontend, it is recommended to autoload them, by using 'yes'|true. For options which are
     *                              accessed only on few specific URLs, it is recommended to not autoload them, by using 'no'|false. For
     *                              non-existent options, the default value is 'yes'. Default null.
     *
     ...
     *                                options are not frequently used. For options which are accessed across several
     *                                places in the frontend, it is recommended to autoload them, by using 'yes'|true. For options which are
     *                                accessed only on few specific URLs, it is recommended to not autoload them, by using 'no'|false. Default
     *                                'yes'.
     *
     ...
     *
     * @param bool  $force_db Optional. Force cleanup to run against the database even when an external object cache is
     *                        used.
     *
     * @since 4.9.0
     *
     ...
     *                                         Valid values are 'string', 'boolean', 'integer', 'number', 'array', and
     *                                         'object'.
