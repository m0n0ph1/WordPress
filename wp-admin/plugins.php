    ...
    if ($action) {
        switch ($action) {
            case 'activate':
                ...
                if (is_wp_error($result)) {
                    if ('unexpected_output' === $result->get_error_code()) {
                        ...
                        wp_redirect(add_query_arg('_error_nonce', wp_create_nonce('plugin-activation-error_' . $plugin),
                            $redirect));
                        exit;
                    } else {
                        wp_die($result);
            ...
            case 'update-selected':
                ...
                $url = self_admin_url('update.php?action=update-selected&amp;plugins=' . urlencode(implode(',',
                        $plugins)));
            ...
            case 'deactivate':
                ...
                if (!is_network_admin()) {
                    ...
                    update_site_option('recently_activated',
                        [$plugin => time()] + (array) get_site_option('recently_activated'));
                }
            ...
            case 'deactivate-selected':
                ...
                if (!is_network_admin()) {
                    ...
                    update_site_option('recently_activated',
                        $deactivated + (array) get_site_option('recently_activated'));
                }
                ...
                if (!isset($_REQUEST['verify-delete'])) {
                    ...
                    <div class="wrap">
                                ...
                                if ('.' === $plugin_slug) {
                                    ...
                                    if ($folder_plugins) {
                                        foreach ($folder_plugins as $plugin_file => $data) {
                                            $plugin_info[$plugin_file] = _get_plugin_data_markup_translate($plugin_file,
                                                $data);
                            ...
                            <?php
                                ...
                                foreach ($plugin_info as $plugin) {
                                    if ($plugin['is_uninstallable']) {
                                        ...
                                        echo '<li>', sprintf(__('%1$s by %2$s (will also <strong>delete its data</strong>)'),
                                            '<strong>' . $plugin['Name'] . '</strong>',
                                            '<em>' . $plugin['AuthorName'] . '</em>'), '</li>';
                                        $data_to_delete = true;
                                    } else {
                                        /* translators: 1: Plugin name, 2: Plugin author. */
                                        echo '<li>', sprintf(_x('%1$s by %2$s', 'plugin'),
                                                '<strong>' . $plugin['Name'] . '</strong>',
                                                '<em>' . $plugin['AuthorName']) . '</em>', '</li>';
                                    }
                                }
                        ...
                        <form method="post"
                            ...
                            <?php submit_button($data_to_delete ? __('Yes, delete these files and data') : __('Yes, delete these files'),
                                '', 'submit', false); ?>
                        </form>
                        <?php
            ...
            case 'resume':
                ...
                $result = resume_plugin($plugin,
                    self_admin_url("plugins.php?error=resuming&plugin_status=$status&paged=$page&s=$s"));
            ...
            default:
                if (isset($_POST['checked'])) {
                    ...
                    $sendback = apply_filters("handle_bulk_actions-{$screen}", $sendback, $action,
                        $plugins); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
...
// Used by wp_admin_notice() updated notices.
    ...
    if (isset($_GET['error'])) {
        ...
        if (!isset($_GET['main']) && !isset($_GET['charsout'])
            && isset($_GET['_error_nonce']) && wp_verify_nonce($_GET['_error_nonce'],
