...
// Clean up request URI from temporary args for screen options/paging uri's to work as expected.
                    ...
                    <div class="wrap">
                        ...
                        <?php if(1 === $plugins_to_delete) : ?>
                            ...
                            <?php if($have_non_network_plugins && is_network_admin()) : ?>
                                <div class="error"><p>
                                        <strong><?php _e('Caution:'); ?></strong> <?php _e('This plugin may be active on other sites in the network.'); ?>
                                    </p></div>
                            <?php endif; ?>
                        ...
                        <?php else : ?>
                            ...
                            <?php if($have_non_network_plugins && is_network_admin()) : ?>
                                <div class="error"><p>
                                        <strong><?php _e('Caution:'); ?></strong> <?php _e('These plugins may be active on other sites in the network.'); ?>
                                    </p></div>
                            <?php endif; ?>
                        ...
                        ?>
                        <form method="post"
                              action="<?php echo $referer ? esc_url($referer) : ''; ?>"
                              style="display:inline;">
    ...
    get_current_screen()->set_help_sidebar('<p><strong>'.__('For more information:').'</strong></p>'.'<p>'.__('<a href="https://wordpress.org/documentation/article/manage-plugins/">Documentation on Managing Plugins</a>').'</p>'.$help_sidebar_autoupdates.'<p>'.__('<a href="https://wordpress.org/support/forums/">Support forums</a>').'</p>');
...
// Used in the HTML title tag.
    ...
    if(! empty($invalid))
    {
        foreach($invalid as $plugin_file => $error)
        {
            ...
            printf(/* translators: 1: Plugin file, 2: Error message. */ __('The plugin %1$s has been deactivated due to an error: %2$s'), '<code>'.esc_html($plugin_file).'</code>', esc_html($error->get_error_message()));
            echo '</p></div>';
        }
    }
// Used by wp_admin_notice() updated notices.
    ...
    if(isset($_GET['error']))
    {
        ...
        {
            $errmsg = sprintf(/* translators: %d: Number of characters. */ _n('The plugin generated %d character of <strong>unexpected output</strong> during activation.', 'The plugin generated %d characters of <strong>unexpected output</strong> during activation.', $_GET['charsout']), $_GET['charsout']);
        ...
        if(! isset($_GET['main']) && ! isset($_GET['charsout']) && isset($_GET['_error_nonce']) && wp_verify_nonce($_GET['_error_nonce'], 'plugin-activation-error_'.$plugin))
        {
    ...
    elseif(isset($_GET['deleted']))
    {
        ...
        {
            $plugin_not_deleted_message = sprintf(/* translators: %s: Error message. */ __('Plugin could not be deleted due to an error: %s'), esc_html($delete_result->get_error_message()));
            wp_admin_notice($plugin_not_deleted_message, [
                'id' => 'message',
                'additional_classes' => ['error'],
                'dismissible' => true,
            ]);
...
?>
    ...
    <div class="wrap">
        ...
        <?php
            ...
            {
                ?>
                <a href="<?php echo esc_url(self_admin_url('plugin-install.php')); ?>"
                   class="page-title-action"><?php echo esc_html__('Add New Plugin'); ?></a>
                <?php
            }
            if(strlen($s))
            {
                echo '<span class="subtitle">';
                printf(/* translators: %s: Search query. */ __('Search results for: %s'), '<strong>'.esc_html(urldecode($s)).'</strong>');
                echo '</span>';
            }
        ?>
