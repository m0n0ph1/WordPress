    ...
    if ($doaction) {
        ...
        switch ($doaction) {
            ...
            default:
                 ...
                 *
                 */
                $sendback = apply_filters("handle_bulk_actions-{$screen}", $sendback, $doaction,
...
// Used in the HTML title tag.
    ...
    $bulk_messages['wp_block'] = [
        ...
        'locked' => (1 === $bulk_counts['locked']) ? __('1 block not updated, somebody is editing it.') :
            /* translators: %s: Number of blocks. */
            _n('%s block not updated, somebody is editing it.', '%s blocks not updated, somebody is editing them.',
                $bulk_counts['locked']),
        /* translators: %s: Number of blocks. */
        ...
        /* translators: %s: Number of blocks. */
        'untrashed' => _n('%s block restored from the Trash.', '%s blocks restored from the Trash.',
            $bulk_counts['untrashed']),
    ];
        ...
        <?php
            ...
            foreach ($bulk_counts as $message => $count) {
                ...
                if ('trashed' === $message && isset($_REQUEST['ids'])) {
                    ...
                    $messages[] = sprintf(
                        '<a href="%1$s">%2$s</a>',
                        esc_url(wp_nonce_url("edit.php?post_type=$post_type&doaction=undo&action=untrash&ids=$ids",
                            'bulk-posts')),
                        __('Undo')
                    );
                }
