<?php
    ...
    switch($action)
    {
        ...
        case 'move-down-menu-item':
            ...
            {
                $menus = isset($_REQUEST['menu']) ? [(int) $_REQUEST['menu']] : wp_get_object_terms($menu_item_id, 'nav_menu', ['fields' => 'ids']);
                ...
                if(! is_wp_error($menus) && ! empty($menus[0]))
                {
                    ...
                    if(isset($orders_to_dbids[$dbids_to_orders[$menu_item_id] + 1]))
                    {
                        ...
                        if(! empty($menu_item_data['menu_item_parent']) && (empty($next_item_data['menu_item_parent']) || (int) $next_item_data['menu_item_parent'] !== (int) $menu_item_data['menu_item_parent']))
                        {
                    ...
                    elseif(! empty($menu_item_data['menu_item_parent']) && in_array((int) $menu_item_data['menu_item_parent'], $orders_to_dbids, true))
                    {
        ...
        case 'move-up-menu-item':
            ...
            if(is_nav_menu_item($menu_item_id))
            {
                ...
                {
                    $menus = [(int) $_REQUEST['menu']];
                }
                else
                {
                    $menus = wp_get_object_terms($menu_item_id, 'nav_menu', ['fields' => 'ids']);
                }
        ...
        case 'delete-menu-item':
            ...
            {
                $messages[] = wp_get_admin_notice(__('The menu item has been successfully deleted.'), [
                    'id' => 'message',
        ...
        case 'delete':
            ...
            {
                $messages[] = wp_get_admin_notice($deletion->get_error_message(), [
                    'id' => 'message',
            ...
            {
                $messages[] = wp_get_admin_notice(__('The menu has been successfully deleted.'), [
                    'id' => 'message',
                    'additional_classes' => ['updated'],
                    'dismissible' => true,
                ]);
        ...
        case 'delete_menus':
            ...
            {
                $messages[] = wp_get_admin_notice(__('Selected menus have been successfully deleted.'), [
                    'id' => 'message',
                    'additional_classes' => ['updated'],
                    'dismissible' => true,
                ]);
        ...
        case 'update':
            ...
            if(0 === $nav_menu_selected_id)
            {
                ...
                if($new_menu_title)
                {
                    $_nav_menu_selected_id = wp_update_nav_menu_object(0, ['menu-name' => $new_menu_title]);
            ...
            else
            {
                ...
                {
                    $_nav_menu_selected_id = wp_update_nav_menu_object($nav_menu_selected_id, ['menu-name' => $menu_title]);

                    if(is_wp_error($_nav_menu_selected_id))
                    {
                        $_menu_object = $_nav_menu_selected_id;
                        $messages[] = wp_get_admin_notice($_nav_menu_selected_id->get_error_message(), [
                            'id' => 'message',
                            'additional_classes' => ['error'],
                            'dismissible' => true,
                        ]);
     ...
     */
    if(1 === count(get_registered_nav_menus()) && ! $add_new_screen && empty($nav_menus) && ! empty($page_count->publish))
    {
    ...
    if(is_nav_menu($nav_menu_selected_id))
    {
        $menu_items = wp_get_nav_menu_items($nav_menu_selected_id, ['post_status' => 'any']);
    ...
    {
        $message_no_theme_support = sprintf(/* translators: %s: URL to Widgets screen. */ __('Your theme does not natively support menus, but you can use them in sidebars by adding a &#8220;Navigation Menu&#8221; widget on the <a href="%s">Widgets</a> screen.'), admin_url('widgets.php'));
        $messages[] = wp_get_admin_notice($message_no_theme_support, [
            'id' => 'message',
            'additional_classes' => ['updated'],
            'dismissible' => true,
        ]);
    ...
    get_current_screen()->set_help_sidebar('<p><strong>'.__('For more information:').'</strong></p>'.'<p>'.__('<a href="https://wordpress.org/documentation/article/appearance-menus-screen/">Documentation on Menus</a>').'</p>'.'<p>'.__('<a href="https://wordpress.org/support/forums/">Support forums</a>').'</p>');
...
<div class="wrap">
    ...
    <?php
        if(current_user_can('customize')) :
            $focus = $locations_screen ? ['section' => 'menu_locations'] : ['panel' => 'nav_menus'];
            printf(
                ' <a class="page-title-action hide-if-no-customize" href="%1$s">%2$s</a>', esc_url(
                add_query_arg([
                                  ['autofocus' => $focus],
    ...
    <nav class="nav-tab-wrapper wp-clearfix" aria-label="<?php esc_attr_e('Secondary menu'); ?>">
        <a href="<?php echo esc_url(admin_url('nav-menus.php')); ?>"
           class="nav-tab<?php echo $nav_tab_active_class; ?>"<?php echo $nav_aria_current; ?>><?php esc_html_e('Edit Menus'); ?></a>
        <?php
            if($num_locations && $menu_count)
            {
                ...
                <a href="<?php echo esc_url(add_query_arg(['action' => 'locations'], admin_url('nav-menus.php'))); ?>"
                   class="nav-tab<?php echo $active_tab_class; ?>"<?php echo $aria_current; ?>><?php esc_html_e('Manage Locations'); ?></a>
                <?php
            }
        ?>
    </nav>
    ...
    <?php
        if($locations_screen) :
            ...
            {
                echo '<p>'.sprintf(/* translators: %s: Number of menus. */ _n('Your theme supports %s menu. Select which menu appears in each location.', 'Your theme supports %s menus. Select which menu appears in each location.', $num_locations), number_format_i18n($num_locations)).'</p>';
            }
            ?>
            <div id="menu-locations-wrap">
                <form method="post"
                      action="<?php echo esc_url(add_query_arg(['action' => 'locations'], admin_url('nav-menus.php'))); ?>">
                        ...
                        <?php foreach($locations as $_location => $_name) { ?>
                            <tr class="menu-locations-row">
                                <td class="menu-location-title">
                                    <label for="locations-<?php echo $_location; ?>"><?php echo $_name; ?></label></td>
                                <td class="menu-location-menus">
                                    <select name="menu-locations[<?php echo $_location; ?>]"
                                            id="locations-<?php echo $_location; ?>">
                                        ...
                                        <?php
                                            foreach($nav_menus as $menu) :
                                                ...
                                                ?>
                                                <option <?php echo $data_orig; ?> <?php selected($selected); ?>
			...
			<span class="screen-reader-text">
                ...
                <?php else : ?>
                    <form method="get" action="<?php echo esc_url(admin_url('nav-menus.php')); ?>">
                        ...
                        <label for="select-menu-to-edit"
                               class="selected-menu"><?php _e('Select a menu to edit:'); ?></label>
                        ...
                        <span class="submit-btn"><input type="submit"
                                                        class="button"
                                                        value="<?php esc_attr_e('Select'); ?>"></span>
                            ...
                            <div class="menu-edit">
                                ...
                                <div id="nav-menu-header">
                                    <div class="major-publishing-actions wp-clearfix">
                                        ...
                                        <input name="menu-name"
                                               ...
                                               required="required" <?php echo $menu_name_val.$menu_name_aria_desc; ?> />
                                        <div class="publishing-action">
                                            <?php submit_button(empty($nav_menu_selected_id) ? __('Create Menu') : __('Save Menu'), 'primary large menu-save', 'save_menu', false, ['id' => 'save_menu_header']); ?>
                                ...
                                <div id="post-body">
                                    <div id="post-body-content" class="wp-clearfix">
                                        ...
                                        <?php if($add_new_screen) : ?>
                                            <p class="post-body-plain"
                                               id="menu-name-desc"><?php _e('Give your menu a name, then click Create Menu.'); ?></p>
                                            <?php if(isset($_GET['use-location'])) : ?>
                                                <input type="hidden"
                                                       name="use-location"
                                                       value="<?php echo esc_attr($_GET['use-location']); ?>"/>
                                            <?php endif; ?>
                                        ...
                                        <?php if(! $add_new_screen) : ?>
                                            <div id="nav-menu-bulk-actions-bottom"
                                                 class="bulk-actions" <?php echo $hide_style; ?>>
                                                ...
                                                <input type="button"
                                                       class="deletion menu-items-delete disabled"
                                                       value="<?php esc_attr_e('Remove Selected Items'); ?>">
                                        ...
                                        <div class="menu-settings" <?php echo $no_menus_style; ?>>
                                            ...
                                            <fieldset class="menu-settings-group auto-add-pages">
                                                ...
                                                <div class="menu-settings-input checkbox-input">
                                                    <input type="checkbox"<?php checked($auto_add); ?>
                                                           name="auto-add-pages"
                                                           id="auto-add-pages"
                                                           value="1"/>
                                                    <label for="auto-add-pages"><?php printf(__('Automatically add new top-level pages to this menu'), esc_url(admin_url('edit.php?post_type=page'))); ?></label>
                                                </div>
                                            </fieldset>
                                                ...
                                                <fieldset class="menu-settings-group menu-theme-locations">
                                                    ...
                                                    <?php
                                                        foreach($locations as $location => $description) :
                                                            ...
                                                            if(isset($menu_locations[$location]) && 0 !== $nav_menu_selected_id && $menu_locations[$location] === $nav_menu_selected_id)
                                                            {
                                                                $checked = true;
                                                            }
                                                            ?>
													...
													<?php
                                                        printf(/* translators: %s: Menu name. */ _x('(Currently set to: %s)', 'menu location'), wp_get_nav_menu_object($menu_locations[$location])->name);
                                                    ?>
													</span>
                                                                <?php endif; ?>
                                                            </div>
                                ...
                                <div id="nav-menu-footer">
                                    <div class="major-publishing-actions">
                                        <div class="publishing-action">
                                            <?php submit_button(empty($nav_menu_selected_id) ? __('Create Menu') : __('Save Menu'), 'primary large menu-save', 'save_menu', false, ['id' => 'save_menu_footer']); ?>
                                        </div><!-- END .publishing-action -->
                                        <?php if($menu_count > 0) : ?>

                                            <?php if($add_new_screen) : ?>
                                                <span class="cancel-action">
									<a class="submitcancel cancellation menu-cancel"
                                       href="<?php echo esc_url(admin_url('nav-menus.php')); ?>"><?php _e('Cancel'); ?></a>
                                        ...
                                        echo esc_url(
                                            wp_nonce_url(
                                                add_query_arg([
                                                              ...
                                                              ], admin_url('nav-menus.php')), 'delete-nav_menu-'.$nav_menu_selected_id
                                            )
                                        );
                                    ?>
