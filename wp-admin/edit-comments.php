<?php
    ...
    if($doaction)
    {
        ...
        if('delete_all' === $doaction && ! empty($_REQUEST['pagegen_timestamp']))
        {
            ...
            $comment_ids = $wpdb->get_col(
				...
				WHERE comment_approved = %s AND %s > comment_date_gmt", $comment_status, $delete_time
                )
            );
            $doaction = 'delete';
        }
    ...
    if($post_id)
    {
        ...
        if($comments_count->moderated > 0)
        {
            ...
            $title = sprintf(/* translators: 1: Comments count, 2: Post title. */ __('Comments (%1$s) on &#8220;%2$s&#8221;'), number_format_i18n($comments_count->moderated), $draft_or_post_title);
        }
        else
        {
            // Used in the HTML title tag.
            $title = sprintf(/* translators: %s: Post title. */ __('Comments on &#8220;%s&#8221;'), $draft_or_post_title);
        }
    }
    else
    {
        ...
        {
            // Used in the HTML title tag.
            $title = sprintf(/* translators: %s: Comments count. */ __('Comments (%s)'), number_format_i18n($comments_count->moderated));
        }
        else
        {
            // Used in the HTML title tag.
    ...
    get_current_screen()->add_help_tab([
                                           'id' => 'overview',
                                           'title' => __('Overview'),
                                           'content' => '<p>'.__('You can manage comments made on your site similar to the way you manage posts and other content. This screen is customizable in the same ways as other management screens, and you can act on comments using the on-hover action links or the bulk actions.').'</p>',
                                       ]);
    get_current_screen()->add_help_tab([
...
<div class="wrap">
    <h1 class="wp-heading-inline">
        <?php
            if($post_id)
            {
                printf(/* translators: %s: Link to post. */ __('Comments on &#8220;%s&#8221;'), sprintf('<a href="%1$s">%2$s</a>', get_edit_post_link($post_id), wp_html_excerpt(_draft_or_post_title($post_id), 50, '&hellip;')));
            }
            else
            {
                _e('Comments');
    ...
    <?php
        ...
        if(isset($_REQUEST['s']) && strlen($_REQUEST['s']))
        {
            ...
            printf(/* translators: %s: Search query. */ __('Search results for: %s'), '<strong>'.esc_html(wp_unslash($_REQUEST['s'])).'</strong>');
            echo '</span>';
        }
    ?>
            ...
            if($approved > 0 || $deleted > 0 || $trashed > 0 || $untrashed > 0 || $spammed > 0 || $unspammed > 0 || $same > 0)
            {
                ...
                if($same > 0)
                {
                    ...
                    if($comment)
                    {
                        ...
                        {
                            case '1':
                                $messages[] = __('This comment is already approved.').sprintf(' <a href="%1$s">%2$s</a>', esc_url(admin_url("comment.php?action=editcomment&c=$same")), __('Edit comment'));
                                break;
                            case 'trash':
                                $messages[] = __('This comment is already in the Trash.').sprintf(' <a href="%1$s">%2$s</a>', esc_url(admin_url('edit-comments.php?comment_status=trash')), __('View Trash'));
                                break;
                            case 'spam':
                                $messages[] = __('This comment is already marked as spam.').sprintf(' <a href="%1$s">%2$s</a>', esc_url(admin_url("comment.php?action=editcomment&c=$same")), __('Edit comment'));
                                break;
                        }
                    }
                }
