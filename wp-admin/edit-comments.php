<?php
    ...
    if ($doaction) {
        ...
        if (!in_array($doaction, ['approve', 'unapprove', 'spam', 'unspam', 'trash', 'delete'], true)) {
            ...
            $redirect_to = apply_filters("handle_bulk_actions-{$screen}", $redirect_to, $doaction,
                $comment_ids); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
        }
                ...
                if ($spammed > 0) {
                    ...
                    $messages[] = sprintf(
                        /* translators: %s: Number of comments. */
                            ...
                            ' <a href="%1$s">%2$s</a><br />',
                            esc_url(wp_nonce_url("edit-comments.php?doaction=undo&action=unspam&ids=$ids",
                                'bulk-comments')),
                            __('Undo')
                        );
                }
                ...
                if ($trashed > 0) {
                    ...
                    $messages[] = sprintf(
                        /* translators: %s: Number of comments. */
                            ...
                            ' <a href="%1$s">%2$s</a><br />',
                            esc_url(wp_nonce_url("edit-comments.php?doaction=undo&action=untrash&ids=$ids",
                                'bulk-comments')),
                            __('Undo')
                        );
                }
