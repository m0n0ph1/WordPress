<?php
                ...
                if ($updater->is_vcs_checkout(ABSPATH)) {
                    ...
                    if ($can_set_update_option) {
                        ...
                        printf(
                            ...
                            wp_nonce_url(add_query_arg('value', 'disable', $action_url),
                                'core-major-auto-updates-nonce'),
                    ...
                    if ($can_set_update_option) {
                        ...
                        printf(
                            ...
                            wp_nonce_url(add_query_arg('value', 'enable', $action_url),
                                'core-major-auto-updates-nonce'),
    ...
    function list_plugin_updates()
    {
        ...
        if (!isset($core_updates[0]->response) || 'latest' === $core_updates[0]->response || 'development' === $core_updates[0]->response || version_compare($core_updates[0]->current,
                $cur_wp_version, '=')) {
            ...
            <table class="widefat updates-table" id="update-plugins-table">
                ...
                <?php
                    ...
                    foreach ((array) $plugins as $plugin_file => $plugin_data) {
                        $plugin_data = (object) _get_plugin_data_markup_translate($plugin_file, (array) $plugin_data,
                            false,
                            true);
                        ...
                        if (isset($plugin_data->update->tested) && version_compare($plugin_data->update->tested,
                                $cur_wp_version, '>=')) {
                            /* translators: %s: WordPress version. */
                            $compat = '<br />' . sprintf(__('Compatibility with WordPress %s: 100%% (according to its author)'),
                                    $cur_wp_version);
                        } else {
                            /* translators: %s: WordPress version. */
                            $compat = '<br />' . sprintf(__('Compatibility with WordPress %s: Unknown'),
                                    $cur_wp_version);
                        }
                        ...
                        $details = sprintf(
                            ...
                            esc_attr(sprintf(__('View %1$s version %2$s details'), $plugin_data->Name,
    ...
    function do_core_upgrade($reinstall = false)
    {
        ...
        <div class="wrap">
            ...
            <?php
                ...
                $credentials = request_filesystem_credentials($url, '', false, ABSPATH, ['version', 'locale'],
                ...
                if (!WP_Filesystem($credentials, ABSPATH, $allow_relaxed_file_ownership)) {
                    ...
                    request_filesystem_credentials($url, '', true, ABSPATH, ['version', 'locale'],
                        $allow_relaxed_file_ownership);
                    echo '</div>';
