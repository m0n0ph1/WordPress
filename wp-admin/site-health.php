    ...
    <div class="health-check-header">
        ...
        <?php
            if(isset($_GET['https_updated']))
            {
                ...
                {
                    wp_admin_notice(__('Site URLs switched to HTTPS.'), [
                        'type' => 'success',
                        'id' => 'message',
                        'dismissible' => true,
                    ]);
                ...
                {
                    wp_admin_notice(__('Site URLs could not be switched to HTTPS.'), [
                        'type' => 'error',
                        'id' => 'message',
                        'dismissible' => true,
                    ]);
        ...
        <nav class="<?php echo implode(' ', $wrapper_classes); ?>" aria-label="<?php esc_attr_e('Secondary menu'); ?>">
            <?php
                ...
                {
                    printf(
                                                                 '<a href="%s" class="health-check-tab %s">%s</a>', esc_url(
                        add_query_arg([
                                          'tab' => $slug,
                                      ], admin_url('site-health.php'))
                    ), ($current_tab === $slug ? 'active' : ''), esc_html($label)
                    );
                }
            ?>
                    ...
                    <div class="health-check-offscreen-nav">
                        <?php
                            ...
                            foreach($tabs_slice as $slug => $label)
                            {
                                ...
                                ), (isset($_GET['tab']) && $_GET['tab'] === $slug ? 'active' : ''), esc_html($label)
                                );
                            }
                        ?>
                    </div>
        ...
        <script id="tmpl-health-check-issue" type="text/template">
            <h4 class="health-check-accordion-heading">
                <button aria-expanded="false"
                        ...
                        aria-controls="health-check-accordion-block-{{ data.test }}"
                        type="button">
