...
// This is the no-js fallback script. Generally this will all be handled by `auth-app.js`.
    if(isset($_POST['action']) && 'authorize_application_password' === $_POST['action'])
    {
        ...
        {
            $created = WP_Application_Passwords::create_new_application_password(get_current_user_id(), [
                'name' => $app_name,
                'app_id' => $app_id,
            ]);
...
?>
    <div class="wrap">
        ...
        <?php
            ...
            {
                wp_admin_notice($error->get_error_message(), [
                    'type' => 'error',
                ]);
            }
        ?>
        ...
        <div class="card auth-app-card">
            ...
            <?php if($app_name) : ?>
                <p>
                    <?php
                        printf(/* translators: %s: Application name. */ __('Would you like to give the application identifying itself as %s access to your account? You should only do this if you trust the application in question.'), '<strong>'.esc_html($app_name).'</strong>');
                    ?>
                </p>
            <?php else : ?>
            ...
            <?php
				...
				<label for="new-application-password-value">'.sprintf(/* translators: %s: Application name. */ esc_html__('Your new password for %s is:'), '<strong>'.esc_html($app_name).'</strong>').'
				</label>
			...
			<p>'.__('Be sure to save this in a safe location. You will not be able to retrieve it.').'</p>';
                    ...
                    ?>
                    <form action="<?php echo esc_url(admin_url('authorize-application.php')); ?>"
                          method="post"
                          class="form-wrap">
                        ...
                        <p class="description" id="description-approve">
                            <?php
                                if($success_url)
                                {
                                    printf(/* translators: %s: The URL the user is being redirected to. */ __('You will be sent to %s'), '<strong><code>'.esc_html(
                                                                                                                                           add_query_arg([
                                                                                                                                                             'site_url' => site_url(),
                                                                                                                                                             'user_login' => $user->user_login,
                                                                                                                                                             'password' => '[------]',
                        ...
                        <p class="description" id="description-reject">
                            <?php
                                if($reject_url)
                                {
                                    printf(/* translators: %s: The URL the user is being redirected to. */ __('You will be sent to %s'), '<strong><code>'.esc_html($reject_url).'</code></strong>');
                                }
                                else
                                {
