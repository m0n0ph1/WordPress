<?php
    ...
    wp_add_inline_script('wp-blocks', sprintf('wp.blocks.setCategories( %s );', wp_json_encode(get_block_categories($post))), 'after');
    /*
    ...
    if($is_new_post && ! isset($editor_settings['template']) && 'post' === $post->post_type)
    {
        ...
        if(in_array($post_format, ['audio', 'gallery', 'image', 'quote', 'video'], true))
        {
            $editor_settings['template'] = [["core/$post_format"]];
        }
    }
...
JS;
    $script = sprintf($init_script, $post->post_type, $post->ID, wp_json_encode($editor_settings), wp_json_encode($initial_edits));
