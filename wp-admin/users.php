...
// Contextual help - choose Help on the top right of admin panel to preview this.
    ...
    get_current_screen()->set_help_sidebar('<p><strong>'.__('For more information:').'</strong></p>'.'<p>'.__('<a href="https://wordpress.org/documentation/article/users-screen/">Documentation on Managing Users</a>').'</p>'.'<p>'.__('<a href="https://wordpress.org/documentation/article/roles-and-capabilities/">Descriptions of Roles and Capabilities</a>').'</p>'.'<p>'.__('<a href="https://wordpress.org/support/forums/">Support forums</a>').'</p>');
    ...
    switch($wp_list_table->current_action())
    {
        ...
        case 'promote':
            ...
            foreach($user_ids as $id)
            {
                ...
                if($id === $current_user->ID && ! $wp_roles->role_objects[$role]->has_cap('promote_users') && ! (is_multisite() && current_user_can('manage_network_users')))
                {
                    ...
                    <ul>
                        <?php
                            ...
                            foreach($all_user_ids as $id)
                            {
                                ...
                                {
                                    echo '<li>';
                                    printf(/* translators: 1: User ID, 2: User login. */ __('ID #%1$s: %2$s <strong>The current user will not be deleted.</strong>'), $id, $user->user_login);
                                    echo "</li>\n";
                                }
                                else
                                {
                                    ...
                                    printf(/* translators: 1: User ID, 2: User login. */ __('ID #%1$s: %2$s'), $id, $user->user_login);
                                    echo "</li>\n";
            ...
            if(isset($_GET['update'])) :
                switch($_GET['update'])
                {
                    ...
                    case 'del_many':
                        ...
                        $messages[] = wp_get_admin_notice($message, [
                            'id' => 'message',
                    ...
                    case 'add':
                        ...
                        {
                            $message .= sprintf(' <a href="%1$s">%2$s</a>', esc_url(add_query_arg('wp_http_referer', urlencode(wp_unslash($_SERVER['REQUEST_URI'])), self_admin_url('user-edit.php?user_id='.$user_id))), __('Edit user'));
                        }
                    ...
                    case 'resetpassword':
                        ...
                        break;
                    case 'promote':
                        $messages[] = wp_get_admin_notice(__('Changed roles.'), [
                            'id' => 'message',
                            'additional_classes' => ['updated'],
                            'dismissible' => true,
                        ]);
                        break;
                    case 'err_admin_role':
                        $messages[] = wp_get_admin_notice(__('The current user&#8217;s role must have user editing capabilities.'), [
                            'id' => 'message',
                            'additional_classes' => ['error'],
                            'dismissible' => true,
                        ]);
                        $messages[] = wp_get_admin_notice(__('Other user roles have been changed.'), [
                            'id' => 'message',
                            'additional_classes' => ['updated'],
                            'dismissible' => true,
                        ]);
                        break;
                    case 'err_admin_del':
                        $messages[] = wp_get_admin_notice(__('You cannot delete the current user.'), [
                            'id' => 'message',
                            'additional_classes' => ['error'],
                            'dismissible' => true,
                        ]);
                        $messages[] = wp_get_admin_notice(__('Other users have been deleted.'), [
                            'id' => 'message',
                            'additional_classes' => ['updated'],
                            'dismissible' => true,
                        ]);
                        break;
                    case 'remove':
                        $messages[] = wp_get_admin_notice(__('User removed from this site.'), [
                            'id' => 'message',
                    ...
                    case 'err_admin_remove':
                        $messages[] = wp_get_admin_notice(__('You cannot remove the current user.'), [
                            'id' => 'message',
                            'additional_classes' => ['error'],
                            'dismissible' => true,
                        ]);
                        $messages[] = wp_get_admin_notice(__('Other users have been removed.'), [
                            'id' => 'message',
                            'additional_classes' => ['updated', 'fade'],
                            'dismissible' => true,
                        ]);
        ...
        <?php
            ...
            <div class="wrap">
                ...
                <?php
                    ...
                    {
                        printf('<a href="%1$s" class="page-title-action">%2$s</a>', esc_url(admin_url('user-new.php')), esc_html__('Add New User'));
                    }
                    elseif(is_multisite() && current_user_can('promote_users'))
                    {
                        printf('<a href="%1$s" class="page-title-action">%2$s</a>', esc_url(admin_url('user-new.php')), esc_html__('Add Existing User'));
                    }

                    if(strlen($usersearch))
                    {
                        echo '<span class="subtitle">';
                        printf(/* translators: %s: Search query. */ __('Search results for: %s'), '<strong>'.esc_html($usersearch).'</strong>');
                        echo '</span>';
                    }
                ?>
