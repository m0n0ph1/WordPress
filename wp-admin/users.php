    ...
    switch ($wp_list_table->current_action()) {
        ...
        case 'dodelete':
            ...
            if (empty($_REQUEST['delete_option'])) {
                $url = self_admin_url('users.php?action=delete&users[]=' . implode('&users[]=',
                        $user_ids) . '&error=true');
        ...
        default:
            ...
            if ($wp_list_table->current_action() && !empty($_REQUEST['users'])) {
                ...
                $sendback = apply_filters("handle_bulk_actions-{$screen}", $sendback, $wp_list_table->current_action(),
                    $user_ids); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
                ...
                switch ($_GET['update']) {
                    ...
                    case 'resetpassword':
                        ...
                        if (1 === $reset_count) {
                            ...
                            /* translators: %s: Number of users. */
                            $message = _n('Password reset links sent to %s user.',
