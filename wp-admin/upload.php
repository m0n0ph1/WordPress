<?php
    ...
    if (!empty($_GET['trashed']) && absint($_GET['trashed'])) {
        ...
        $message .= sprintf(
            ' <a href="%1$s">%2$s</a>',
            esc_url(wp_nonce_url('upload.php?doaction=undo&action=untrash&ids=' . (isset($_GET['ids']) ? $_GET['ids'] : ''),
                'bulk-media')),
            __('Undo')
        );
    ...
    if ($doaction) {
        ...
        switch ($doaction) {
            ...
            default:
                ...
                $location = apply_filters("handle_bulk_actions-{$screen}", $location, $doaction,
                    $post_ids); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
        }
