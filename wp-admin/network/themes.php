                ...
                if(! isset($_REQUEST['verify-delete']))
                {
                    ...
                    <div class="wrap">
                        <?php if(1 === $themes_to_delete) : ?>
                            ...
                            <div class="error"><p>
                                    <strong><?php _e('Caution:'); ?></strong> <?php _e('This theme may be active on other sites in the network.'); ?>
                                </p></div>
                            <p><?php _e('You are about to remove the following theme:'); ?></p>
                        <?php else : ?>
                            <h1><?php _e('Delete Themes'); ?></h1>
                            <div class="error"><p>
                                    <strong><?php _e('Caution:'); ?></strong> <?php _e('These themes may be active on other sites in the network.'); ?>
                                </p></div>
                        ...
                        <ul class="ul-disc">
                            <?php
                                ...
                                {
                                    echo '<li>'.sprintf(/* translators: 1: Theme name, 2: Theme author. */ _x('%1$s by %2$s', 'theme'), '<strong>'.$theme->display('Name').'</strong>', '<em>'.$theme->display('Author').'</em>').'</li>';
                                }
                            ?>
                        </ul>
                        <?php if(1 === $themes_to_delete) : ?>
    ...
    <div class="wrap">
        ...
        <?php
            if(isset($_REQUEST['s']) && strlen($_REQUEST['s']))
            {
                ...
                printf(/* translators: %s: Search query. */ __('Search results for: %s'), '<strong>'.esc_html($s).'</strong>');
                echo '</span>';
            }
        ?>
        ...
        <?php
            ...
            if(isset($_GET['enabled']))
            {
                ...
                {
                    $message = sprintf(/* translators: %s: Number of themes. */ _n('%s theme enabled.', '%s themes enabled.', $enabled), number_format_i18n($enabled));
                }
            }
            elseif(isset($_GET['disabled']))
            {
                ...
                {
                    $message = sprintf(/* translators: %s: Number of themes. */ _n('%s theme disabled.', '%s themes disabled.', $disabled), number_format_i18n($disabled));
                }
            }
            elseif(isset($_GET['deleted']))
            {
                ...
                {
                    $message = sprintf(/* translators: %s: Number of themes. */ _n('%s theme deleted.', '%s themes deleted.', $deleted), number_format_i18n($deleted));
                }
            }
            elseif(isset($_GET['enabled-auto-update']))
            {
                ...
                {
                    $message = sprintf(/* translators: %s: Number of themes. */ _n('%s theme will be auto-updated.', '%s themes will be auto-updated.', $enabled), number_format_i18n($enabled));
                }
            }
            elseif(isset($_GET['disabled-auto-update']))
            {
                ...
                {
                    $message = sprintf(/* translators: %s: Number of themes. */ _n('%s theme will no longer be auto-updated.', '%s themes will no longer be auto-updated.', $disabled), number_format_i18n($disabled));
                }
            }
