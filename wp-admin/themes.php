...
// Help tab: Auto-updates.
    ...
    wp_localize_script('theme', '_wpThemeSettings', [
        ...
        'l10n' => [
            ...
            /* translators: %d: Number of themes. */ 'themesFound' => __('Number of Themes found: %d'),
    ...
    <div class="wrap">
        ...
        <?php if(! is_multisite() && current_user_can('install_themes')) : ?>
            <a href="<?php echo esc_url(admin_url('theme-install.php')); ?>"
               class="hide-if-no-js page-title-action"><?php echo esc_html__('Add New Theme'); ?></a>
        <?php endif; ?>
        ...
        <?php
            ...
            {
                wp_admin_notice(__('The active theme is broken. Reverting to the default theme.'), [
                    'id' => 'message1',
            ...
            elseif(isset($_GET['activated']))
            {
                ...
                {
                    wp_admin_notice(__('Settings saved and theme activated.').'<a href="'.esc_url(home_url('/')).'">'.__('Visit site').'</a>', [
                        'id' => 'message2',
                        'additional_classes' => ['updated'],
                        'dismissible' => true,
                    ]);
                ...
                {
                    wp_admin_notice(__('New theme activated.').'<a href="'.esc_url(home_url('/')).'">'.__('Visit site').'</a>', [
                        'id' => 'message2',
                        'additional_classes' => ['updated'],
                        'dismissible' => true,
                    ]);
            ...
            {
                wp_admin_notice(__('Theme deleted.'), [
                    'id' => 'message3',
                    'additional_classes' => ['updated'],
                    'dismissible' => true,
                ]);
            ...
            {
                wp_admin_notice(__('You cannot delete a theme while it has an active child theme.'), [
                    'id' => 'message4',
            ...
            {
                wp_admin_notice(__('Theme resumed.'), [
                    'id' => 'message5',
                    'additional_classes' => ['updated'],
                    'dismissible' => true,
                ]);
            ...
            {
                wp_admin_notice(__('Theme could not be resumed because it triggered a <strong>fatal error</strong>.'), [
                    'id' => 'message6',
                    'additional_classes' => ['error'],
                ]);
            }
            ...
            {
                wp_admin_notice(__('Theme will be auto-updated.'), [
                    'id' => 'message7',
                    'additional_classes' => ['updated'],
                    'dismissible' => true,
                ]);
            ...
            {
                wp_admin_notice(__('Theme will no longer be auto-updated.'), [
                    'id' => 'message8',
                    'additional_classes' => ['updated'],
                    'dismissible' => true,
                ]);
                        ...
                        <div class="theme<?php echo $active_class; ?>">
                            <?php if(! empty($theme['screenshot'][0])) { ?>
                                <div class="theme-screenshot">
                                    <img src="<?php echo esc_url($theme['screenshot'][0].'?ver='.$theme['version']); ?>"
                                         alt=""/>
                                </div>
                            <?php } else { ?>
                            ...
                            <?php
                                if(! $theme['compatibleWP'] || ! $theme['compatiblePHP'])
                                {
                                    ...
                                    if(! $theme['compatibleWP'] && ! $theme['compatiblePHP'])
                                    {
                                        ...
                                        if(current_user_can('update_core') && current_user_can('update_php'))
                                        {
                                            $message .= sprintf(/* translators: 1: URL to WordPress Updates screen, 2: URL to Update PHP page. */ ' '.__('<a href="%1$s">Please update WordPress</a>, and then <a href="%2$s">learn more about updating PHP</a>.'), self_admin_url('update-core.php'), esc_url(wp_get_update_php_url()));
                                            $message .= wp_update_php_annotation('</p><p><em>', '</em>', false);
                                        }
                                        elseif(current_user_can('update_core'))
                                        {
                                            $message .= sprintf(/* translators: %s: URL to WordPress Updates screen. */ ' '.__('<a href="%s">Please update WordPress</a>.'), self_admin_url('update-core.php'));
                                        }
                                        elseif(current_user_can('update_php'))
                                        {
                                            $message .= sprintf(/* translators: %s: URL to Update PHP page. */ ' '.__('<a href="%s">Learn more about updating PHP</a>.'), esc_url(wp_get_update_php_url()));
                                            $message .= wp_update_php_annotation('</p><p><em>', '</em>', false);
                                        }
                                    }
                                ...
                                <div class="theme-actions">
                                    ...
                                    <?php } elseif($theme['compatibleWP'] && $theme['compatiblePHP']) { ?>
                                        ...
                                        <a class="button activate"
                                           href="<?php echo $theme['actions']['activate']; ?>"
                                ...
                                <?php
                                    if($can_resume)
                                    {
                                        if('theme_paused' === $broken_theme->errors()->get_error_code())
                                        {
                                            ...
                                            ?>
                                            <td><a href="<?php echo esc_url($resume_url); ?>"
                                                   class="button resume-theme"><?php _e('Resume'); ?></a></td>
                                            <?php
                                        }
                                        else
                                        {
                                    ...
                                    if($can_delete)
                                    {
                                        ...
                                        ?>
                                        <td><a href="<?php echo esc_url($delete_url); ?>"
                                    ...
                                    if($can_install && 'theme_no_parent' === $broken_theme->errors()->get_error_code())
                                    {
                                        ...
                                        if(! is_wp_error($parent_theme))
                                        {
                                            ...
                                            ?>
                                            <td><a href="<?php echo esc_url($install_url); ?>"
                                                   class="button install-theme"><?php _e('Install Parent Theme'); ?></a>
                                            </td>
                                            <?php
                                        }
                                    }
...
?>
    <script id="tmpl-theme" type="text/template">
        ...
        <div class="update-message notice inline notice-error notice-alt"><p>
                <# if ( ! data.updateResponse.compatibleWP && ! data.updateResponse.compatiblePHP ) { #>
                <?php
                    printf(/* translators: %s: Theme name. */ __('There is a new version of %s available, but it does not work with your versions of WordPress and PHP.'), '{{{ data.name }}}');
                    if(current_user_can('update_core') && current_user_can('update_php'))
                    {
                        printf(/* translators: 1: URL to WordPress Updates screen, 2: URL to Update PHP page. */ ' '.__('<a href="%1$s">Please update WordPress</a>, and then <a href="%2$s">learn more about updating PHP</a>.'), self_admin_url('update-core.php'), esc_url(wp_get_update_php_url()));
                        wp_update_php_annotation('</p><p><em>', '</em>');
                ...
                <?php
                    printf(/* translators: %s: Theme name. */ __('There is a new version of %s available, but it does not work with your version of WordPress.'), '{{{ data.name }}}');
                    if(current_user_can('update_core'))
                    {
                        printf(/* translators: %s: URL to WordPress Updates screen. */ ' '.__('<a href="%s">Please update WordPress</a>.'), self_admin_url('update-core.php'));
                    }
                ...
                <?php
                    printf(/* translators: %s: Theme name. */ __('There is a new version of %s available, but it does not work with your version of PHP.'), '{{{ data.name }}}');
                    if(current_user_can('update_php'))
                    {
                        printf(/* translators: %s: URL to Update PHP page. */ ' '.__('<a href="%s">Learn more about updating PHP</a>.'), esc_url(wp_get_update_php_url()));
                        wp_update_php_annotation('</p><p><em>', '</em>');
        ...
        <button type="button"
                aria-label="<?php echo esc_attr($details_aria_label); ?>"
                class="more-details"
                id="{{ data.id }}-action"><?php _e('Theme Details'); ?></button>
        <div class="theme-author">
            <?php
                /* translators: %s: Theme author name. */
        ...
        <div class="theme-id-container">
            ...
            <div class="theme-actions">
                ...
                <a aria-label="<?php echo esc_attr($customize_aria_label); ?>"
                   class="button button-primary customize load-customize hide-if-no-customize"
                   href="{{{ data.actions.customize }}}"><?php _e('Customize'); ?></a>
                <# } #>
                <# } else { #>
                <# if ( data.compatibleWP && data.compatiblePHP ) { #>
                <?php
                ...
                <a aria-label="<?php echo esc_attr($live_preview_aria_label); ?>"
                   class="button button-primary load-customize hide-if-no-customize"
                   href="{{{ data.actions.customize }}}"><?php _e('Live Preview'); ?></a>
                <# } else { #>
                <?php
                    /* translators: %s: Theme name. */
                    $aria_label = sprintf(_x('Cannot Activate %s', 'theme'), '{{ data.name }}');
