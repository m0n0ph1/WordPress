<?php
    ...
    function _wp_translate_postdata($update = false, $post_data = null)
    {
        ...
        if ($update && !current_user_can('edit_post', $post_data['ID'])) {
            ...
            if ('page' === $post_data['post_type']) {
                return new WP_Error('edit_others_pages',
                    __('Sorry, you are not allowed to create pages as this user.'));
            } else {
                return new WP_Error('edit_others_posts',
        ...
        if (isset($post_data['user_ID']) && ($post_data['post_author'] != $post_data['user_ID'])
            ...
            if ($update) {
                if ('page' === $post_data['post_type']) {
                    return new WP_Error('edit_others_pages',
                        __('Sorry, you are not allowed to edit pages as this user.'));
                } else {
                    return new WP_Error('edit_others_posts',
                        __('Sorry, you are not allowed to edit posts as this user.'));
                }
            } else {
                if ('page' === $post_data['post_type']) {
                    return new WP_Error('edit_others_pages',
        ...
        if (isset($post_data['tax_input'])) {
            foreach ((array) $post_data['tax_input'] as $taxonomy => $terms) {
                ...
                if ($tax_object && isset($tax_object->meta_box_sanitize_cb)) {
                    $translated['tax_input'][$taxonomy] = call_user_func_array($tax_object->meta_box_sanitize_cb,
                        [$taxonomy, $terms]);
                }
            }
        }
        ...
        if (!$success && is_callable([$wpdb, 'strip_invalid_text_for_column'])) {
            ...
            foreach ($fields as $field) {
                if (isset($translated[$field])) {
                    $translated[$field] = $wpdb->strip_invalid_text_for_column($wpdb->posts, $field,
                        $translated[$field]);
                }
            }
    ...
    function bulk_edit_posts($post_data = null)
    {
        ...
        foreach ($post_ids as $post_id) {
            ...
            if (
                in_array($post->post_status, ['future', 'draft'], true) &&
    ...
    function get_default_post_to_edit($post_type = 'post', $create_in_db = false)
    {
        ...
        if ($create_in_db) {
            ...
            $post = get_post($post_id);
            if (current_theme_supports('post-formats') && post_type_supports($post->post_type,
                    'post-formats') && get_option('default_post_format')) {
        ...
        if (!current_user_can($ptype->cap->edit_posts)) {
            if ('page' === $ptype->name) {
                ...
                return new WP_Error('edit_posts',
                    __('Sorry, you are not allowed to create posts or drafts on this site.'));
            }
        }
...
//
    ...
    function _fix_attachment_links($post)
    {
        ...
        if (!get_option('permalink_structure') || !in_array($post['post_status'], ['publish', 'future', 'private'],
                true)) {
            return;
        }
        
        // Short if there aren't any links or no '?attachment_id=' strings (strpos cannot be zero).
        if (!strpos($content, '?attachment_id=') || !preg_match_all('/<a ([^>]+)>[\s\S]+?<\/a>/', $content,
                $link_matches)) {
            return;
        }
    ...
    function wp_edit_attachments_query_vars($q = false)
    {
        ...
        if (isset($q['post_mime_type']) && !array_intersect((array) $q['post_mime_type'],
                array_keys($post_mime_types))) {
    ...
    function get_sample_permalink($post, $title = null, $name = null)
    {
        ...
        $post->post_name = wp_unique_post_slug($post->post_name, $post->ID, $post->post_status, $post->post_type,
            $post->post_parent);
                    ...
                    if ($locked) {
                        ...
                        <div class="post-locked-message">
                            ...
                            <p class="currently-editing wp-tab-first" tabindex="0">
                                <?php
                                    if ($override) {
                                        ...
                                        printf(__('%s is currently editing this post. Do you want to take over?'),
                            ...
                            <p>
                                ...
                                <?php if ($preview_link) { ?>
                                    <a class="button<?php echo $tab_last; ?>"
                                       href="<?php echo esc_url($preview_link); ?>"><?php _e('Preview'); ?></a>
                                    <?php
                                }
                                    ...
                                    if ($override) {
                                        ?>
                                        <a class="button button-primary wp-tab-last"
                                           href="<?php echo esc_url(add_query_arg('get-post-lock', '1',
                                               wp_nonce_url(get_edit_post_link($post->ID, 'url'),
                                                   'lock-post_' . $post->ID))); ?>"><?php _e('Take over'); ?></a>
                                        <?php
                                    }
                        ...
                        <div class="post-taken-over">
                            ...
                            <p class="wp-tab-first" tabindex="0">
                                ...
                                <span class="locked-saving hidden"><img src="<?php echo esc_url(admin_url('images/spinner-2x.gif')); ?>"
                                                                        width="16"
                                                                        height="16"
                                                                        alt=""/> <?php _e('Saving revision&hellip;'); ?></span>
                            ...
                            <p><a class="button button-primary wp-tab-last"
                                  href="<?php echo esc_url($sendback); ?>"><?php echo $sendback_text; ?></a></p>
                        </div>
                        <?php
                    }
    ...
    function wp_create_post_autosave($post_data)
    {
        ...
        if ($old_autosave) {
            ...
            foreach (array_intersect(array_keys($new_autosave),
                array_keys(_wp_post_revision_fields($post))) as $field) {
