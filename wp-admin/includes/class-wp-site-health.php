    ...
    class WP_Site_Health
    {
        ...
        public function get_test_php_extensions()
        {
            ...
            foreach ($modules as $library => $module) {
                ...
                if (!$this->test_php_extension_availability($extension_name, $function_name, $constant_name,
                        $class_name)
        ...
        private function prepare_sql_data()
        {
            ...
            $this->is_acceptable_mysql_version = version_compare($this->mysql_required_version,
                $this->mysql_server_version,
                '<=');
            $this->is_recommended_mysql_version = version_compare($this->mysql_recommended_version,
                $this->mysql_server_version, '<=');
        }
			 ...
			 */
                ...
                if (wp_is_https_supported()) {
                    ...
                    if (defined('WP_HOME') || defined('WP_SITEURL')) {
                        ...
                        $default_direct_update_url = add_query_arg('action', 'update_https',
                            wp_nonce_url(admin_url('site-health.php'), 'wp_update_https'));
        ...
        public function get_test_page_cache()
        {
            ...
            $description .= '<code>' . implode('</code>, <code>',
                    array_keys($this->get_page_cache_headers())) . '.</code>';
            ...
            if (empty($page_cache_detail['response_time'])) {
                ...
                if (empty($page_cache_detail['headers'])) {
                    ...
                    $headers_summary .= ' <code>' . implode('</code>, <code>',
                            $page_cache_detail['headers']) . '</code>.';
		 ...
		 */
            ...
            for ($i = 1; $i <= 3; $i++) {
                ...
                foreach ($caching_headers as $header => $callback) {
                    ...
                    if (empty($callback) || (is_callable($callback) && count(array_filter($header_values,
                                $callback)) > 0)) {
