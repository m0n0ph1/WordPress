<?php
    ...
    function wp_iframe($content_func, ...$args)
    {
        ...
        if (
            (is_array($content_func) && !empty($content_func[1]) && str_starts_with((string) $content_func[1],
                    'media')) ||
    ...
    function media_upload_form_handler()
    {
        ...
        if (isset($send_id)) {
            ...
            if (!empty($attachment['url'])) {
                ...
                if (str_contains($attachment['url'],
                        'attachment_id') || get_attachment_link($send_id) === $attachment['url']) {
    ...
    function image_size_input_fields($post, $check = '')
    {
        ...
        foreach ($size_names as $size => $label) {
            ...
            $html = "<div class='image-size-item'><input type='radio' " . disabled($enabled, false,
                    false) . "name='attachments[$post->ID][image-size]' id='{$css_id}' value='{$size}'$checked />";
...
';
    ...
    function image_media_send_to_editor($html, $attachment_id, $attachment)
    {
        ...
        if (str_starts_with($post->post_mime_type, 'image')) {
            ...
            return get_image_send_to_editor($attachment_id, $attachment['post_excerpt'], $attachment['post_title'],
                $align,
                $url, $rel, $size, $alt);
        }
    ...
    function get_media_item($attachment_id, $args = null)
    {
        ...
        $display_title = $parsed_args['show_title'] ? "<div class='filename new'><span class='title'>" . wp_html_excerpt($display_title,
                60, '&hellip;') . '</span></div>' : '';
        ...
        if ($delete && current_user_can('delete_post', $attachment_id)) {
            if (!EMPTY_TRASH_DAYS) {
                $delete = "<a href='" . wp_nonce_url("post.php?action=delete&amp;post=$attachment_id",
                        'delete-post_' . $attachment_id) . "' id='del[$attachment_id]' class='delete-permanently'>" . __('Delete Permanently') . '</a>';
            } elseif (!MEDIA_TRASH) {
				...
				<a href='" . wp_nonce_url("post.php?action=delete&amp;post=$attachment_id",
                        'delete-post_' . $attachment_id) . "' id='del[$attachment_id]' class='button'>" . __('Continue') . "</a>
				<a href='#' class='button' onclick=\"this.parentNode.style.display='none';return false;\">" . __('Cancel') . '</a>
				</div>';
            } else {
                $delete = "<a href='" . wp_nonce_url("post.php?action=trash&amp;post=$attachment_id",
                        'trash-post_' . $attachment_id) . "' id='del[$attachment_id]' class='delete'>" . __('Move to Trash') . "</a>
			<a href='" . wp_nonce_url("post.php?action=untrash&amp;post=$attachment_id",
                        'untrash-post_' . $attachment_id) . "' id='undo[$attachment_id]' class='undo hidden'>" . __('Undo') . '</a>';
            }
        } else {
            $delete = '';
        }
        ...
        foreach ($form_fields as $id => $field) {
            ...
            if (!empty($field['helps'])) {
                $item .= "<p class='help'>" . implode("</p>\n<p class='help'>",
                        array_unique((array) $field['helps'])) . '</p>';
            }
            $item .= "</td>\n\t\t</tr>\n";
	...
	*/
        ...
        <div id="plupload-upload-ui" class="hide-if-no-js">
            ...
            <div id="drag-drop-area">
                <div class="drag-drop-inside">
                    ...
                    <p class="drag-drop-buttons"><input id="plupload-browse-button"
                                                        type="button"
            ...
            <div id="gallery-settings" style="display:none;">
                ...
                <p class="ml-submit">
                    <input type="button"
                           class="button"
                           style="display:none;"
                           onMouseDown="wpgallery.update();"
                           name="insert-gallery"
                           id="insert-gallery"
                           value="<?php esc_attr_e('Insert gallery'); ?>"/>
                    <input type="button"
                           class="button"
                           style="display:none;"
                           onMouseDown="wpgallery.update();"
                ...
                <div class="alignleft actions">
                    <?php
                        ...
                        if ($month_count && !(1 == $month_count && 0 == $arc_result[0]->mmonth)) {
                            ?>
                            <select name='m'>
                                <option<?php selected($selected_month,
                                    0); ?> value='0'><?php _e('All dates'); ?></option>
                                <?php
    ...
    function wp_media_insert_url_form($default_view = 'image')
    {
        ...
        return '
	<p class="media-types"><label><input type="radio" name="media_type" value="image" id="image-only"' . checked('image-only',
                $view,
                false) . ' /> ' . __('Image') . '</label> &nbsp; &nbsp; <label><input type="radio" name="media_type" value="generic" id="not-image"' . checked('not-image',
                $view, false) . ' /> ' . __('Audio, Video, or Other File') . '</label></p>
        ...
        <div class="wp_attachment_holder wp-clearfix">
            <?php
                ...
                if (wp_attachment_is_image($post->ID)) :
                    ...
                    <div<?php echo $not_open_style; ?> class="image-editor"
                                                       id="image-editor-<?php echo $attachment_id; ?>">
                        <?php
    ...
    function attachment_submitbox_metadata()
    {
        ...
        <div class="misc-pub-section misc-pub-uploadedby">
            <?php if ($uploaded_by_link) { ?>
                <?php _e('Uploaded by:'); ?>
                <a href="<?php echo $uploaded_by_link; ?>"><strong><?php echo $uploaded_by_name; ?></strong></a>
            <?php } else { ?>
            ...
            if ($post_parent) {
                ...
                <div class="misc-pub-section misc-pub-uploadedto">
                    <?php if ($uploaded_to_link) { ?>
                        <?php _e('Uploaded to:'); ?>
                        <a href="<?php echo $uploaded_to_link; ?>"><strong><?php echo $uploaded_to_title; ?></strong></a>
                    <?php } else { ?>
        ...
        <div class="misc-pub-section misc-pub-attachment">
			...
			<button type="button"
                    class="button copy-attachment-url edit-media"
