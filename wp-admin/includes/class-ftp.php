...
// <!-- --------------------------------------------------------------------------------------- -->
        ...
        function features()
        {
            ...
            $f = preg_split("/[" . CRLF . "]+/", preg_replace("/[0-9]{3}[ -].*[" . CRLF . "]+/", "", $this->_message),
                -1,
                PREG_SPLIT_NO_EMPTY);
            $this->_features = [];
            foreach ($f as $k => $v) {
...
//			$this->SendMSG(implode($this->_eol_code[$this->OS_local], $out));
        ...
        function mget($remote, $local = ".", $continious = false)
        {
            ...
            if ($list === false) {
                $this->PushError("mget", "cannot read remote folder list",
                    "Cannot read remote folder \"" . $remote . "\" contents");
            ...
            foreach ($list as $el) {
                if ($el["type"] == "d") {
                    if (!$this->mget($remote . "/" . $el["name"], $local . "/" . $el["name"], $continious)) {
                        $this->PushError("mget", "cannot copy folder",
                            "Cannot copy remote folder \"" . $remote . "/" . $el["name"] . "\" to local \"" . $local . "/" . $el["name"] . "\"");
                        $ret = false;
                        if (!$continious) {
                            break;
                        }
                    ...
                    if (!$this->get($remote . "/" . $el["name"], $local . "/" . $el["name"])) {
                        $this->PushError("mget", "cannot copy file",
                            "Cannot copy remote file \"" . $remote . "/" . $el["name"] . "\" to local \"" . $local . "/" . $el["name"] . "\"");
                        $ret = false;
                        if (!$continious) {
                            break;
                        }
            ...
            if ($is_windows && preg_match("/([0-9]{2})-([0-9]{2})-([0-9]{2}) +([0-9]{2}):([0-9]{2})(AM|PM) +([0-9]+|<DIR>) +(.+)/",
                ...
                if (!$is_windows && $lucifer = preg_split("/[ ]/", $line, 9, PREG_SPLIT_NO_EMPTY)) {
                    ...
                    if ($lcount == 8) {
                        ...
                        $b['time'] = strtotime(sprintf("%d %s %d %02d:%02d", $b['day'], $b['month'], $b['year'],
                            $b['hour'],
                            $b['minute']));
                        $b['name'] = $lucifer[8];
                    }
        ...
        function mdel($remote, $continious = false)
        {
            $list = $this->rawlist($remote, "-la");
            if ($list === false) {
                $this->PushError("mdel", "cannot read remote folder list",
                    "Cannot read remote folder \"" . $remote . "\" contents");
            ...
            foreach ($list as $el) {
                ...
                if ($el["type"] == "d") {
                    ...
                    if (!$this->delete($remote . "/" . $el["name"])) {
                        $this->PushError("mdel", "cannot delete file",
                            "Cannot delete remote file \"" . $remote . "/" . $el["name"] . "\"");
                        $ret = false;
                        if (!$continious) {
                            break;
                        }
            ...
            if (!$this->rmdir($remote)) {
                $this->PushError("mdel", "cannot delete folder",
                    "Cannot delete remote folder \"" . $remote . "/" . $el["name"] . "\"");
                $ret = false;
            }
...
// Gnre une erreur pour traitement externe  la classe
        ...
        function dirlist($remote)
        {
            $list = $this->rawlist($remote, "-la");
            if ($list === false) {
                $this->PushError("dirlist", "cannot read remote folder list",
                    "Cannot read remote folder \"" . $remote . "\" contents");
