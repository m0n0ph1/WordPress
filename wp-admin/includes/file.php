<?php
    ...
    function wp_edit_theme_plugin_file($args)
    {
        ...
        if (!is_file($real_file)) {
            return new WP_Error('file_does_not_exist',
        ...
        if ($test_type) {
            ...
            if ((!$type || !$ext) && !current_user_can('unfiltered_upload')) {
                return call_user_func_array($upload_error_handler,
                    [&$file, __('Sorry, you are not allowed to upload this file type.')]);
            }
    ...
    function download_url($url, $timeout = 300, $signature_verification = false)
    {
        ...
        if ($signature_verification) {
            ...
            $signed_hostnames = apply_filters('wp_signature_hosts',
                ['wordpress.org', 'downloads.wordpress.org', 's.w.org']);
    ...
    function verify_file_signature($filename, $signatures, $filename_for_errors = false)
    {
        ...
        if (!function_exists('sodium_crypto_sign_verify_detached') || !in_array('sha384',
                array_map('strtolower', hash_algos()), true)) {
            return new WP_Error(
    ...
    function _unzip_file_ziparchive($file, $to, $needed_dirs = [])
    {
        ...
        for ($i = 0; $i < $z->numFiles; $i++) {
            ...
            if (str_starts_with($info['name'],
                '__MACOSX/')) { // Don't extract the OS X-created __MACOSX directory files.
                continue;
            }
            ...
            if (false === $contents) {
                return new WP_Error('extract_failed_ziparchive', __('Could not extract file from archive.'),
                    $info['name']);
            }
            ...
            if ('f' === $fileinfo['type']) {
                ...
                if (!$wp_filesystem->is_dir($to . $filename)) {
                    if (!$wp_filesystem->mkdir($to . $filename, FS_CHMOD_DIR)) {
                        return new WP_Error('mkdir_failed_copy_dir', __('Could not create directory.'),
                            $to . $filename);
                    }
                }
    ...
    function move_dir($from, $to, $overwrite = false)
    {
        ...
        if ($wp_filesystem->exists($to)) {
            if (!$overwrite) {
                return new WP_Error('destination_already_exists_move_dir', __('The destination folder already exists.'),
                    $to);
            } elseif (!$wp_filesystem->delete($to, true)) {
                // Can't overwrite if the destination couldn't be deleted.
                return new WP_Error('destination_not_deleted_move_dir',
        ...
        <form action="<?php echo esc_url($form_post); ?>" method="post">
            <div id="request-filesystem-credentials-form" class="request-filesystem-credentials-form">
                ...
                <fieldset>
                    ...
                    <?php
                        $disabled = disabled((defined('FTP_SSL') && FTP_SSL) || (defined('FTP_SSH') && FTP_SSH), true,
                            false);
                        foreach ($types as $name => $text) :
                            ...
                            <label for="<?php echo esc_attr($name); ?>">
                                <input type="radio"
                                       ...
                                       value="<?php echo esc_attr($name); ?>" <?php checked($name,
                                    $connection_type); ?> <?php echo $disabled; ?> />
                                <?php echo $text; ?>
                            </label>
                        <?php
                        endforeach;
