<?php /** @noinspection ALL */
    ...
    function wp_print_file_editor_templates()
    {
        ...
        <script type="text/html" id="tmpl-wp-file-editor-notice">
            <div class="notice inline notice-{{ data.type || 'info' }} {{ data.alt ? 'notice-alt' : '' }} {{ data.dismissible ? 'is-dismissible' : '' }} {{ data.classes || '' }}">
                ...
                <p>
                    <?php
                        printf(/* translators: 1: Line number, 2: File path. */ __('Your PHP code changes were not applied due to an error on line %1$s of file %2$s. Please fix and try saving again.'), '{{ data.line }}', '{{ data.file }}');
                    ?>
                </p>
                ...
                <p>
                    <?php
                        printf(/* translators: %s: Documentation URL. */ __('You need to make this file writable before you can save your changes. See <a href="%s">Changing File Permissions</a> for more information.'), __('https://wordpress.org/documentation/article/changing-file-permissions/'));
                    ?>
                </p>
                <# } else { #>
    ...
    function wp_edit_theme_plugin_file($args)
    {
        ...
        if(! empty($args['plugin']))
        {
            ...
            $is_active = in_array($plugin, (array) get_option('active_plugins', []), true);
        }
        elseif(! empty($args['theme']))
        {
            ...
            {
                return new WP_Error('theme_no_stylesheet', __('The requested theme does not exist.').' '.$theme->errors()->get_error_message());
            }
    ...
    function _wp_handle_upload(&$file, $overrides, $time, $action)
    {
        ...
        else
        {
            ...
            $upload_error_strings = [
                false,
                sprintf(/* translators: 1: upload_max_filesize, 2: php.ini */ __('The uploaded file exceeds the %1$s directive in %2$s.'), 'upload_max_filesize', 'php.ini'),
                sprintf(/* translators: %s: MAX_FILE_SIZE */ __('The uploaded file exceeds the %s directive that was specified in the HTML form.'), 'MAX_FILE_SIZE'),
        ...
        if($test_size && ! ($test_file_size > 0))
        {
            ...
            {
                $error_msg = sprintf(/* translators: 1: php.ini, 2: post_max_size, 3: upload_max_filesize */ __('File is empty. Please upload something more substantial. This error could also be caused by uploads being disabled in your %1$s file or by %2$s being defined as smaller than %3$s in %1$s.'), 'php.ini', 'post_max_size', 'upload_max_filesize');
            }
        ...
        if($test_type)
        {
            ...
            {
                return call_user_func_array($upload_error_handler, [
                    &$file,
        ...
        if(null === $move_new_file)
        {
            ...
            if(false === $move_new_file)
            {
                ...
                return $upload_error_handler($file, sprintf(/* translators: %s: Destination file path. */ __('The uploaded file could not be moved to %s.'), $error_path));
            }
        }
    ...
    function download_url($url, $timeout = 300, $signature_verification = false)
    {
        ...
        {
            $signed_hostnames = apply_filters('wp_signature_hosts', [
                'wordpress.org',
    ...
    function verify_file_md5($filename, $expected_md5)
    {
        ...
        return new WP_Error('md5_mismatch', sprintf(/* translators: 1: File checksum, 2: Expected checksum value. */ __('The checksum of the file (%1$s) does not match the expected checksum value (%2$s).'), bin2hex($file_md5), bin2hex($expected_raw_md5)));
    }
    ...
    function verify_file_signature($filename, $signatures, $filename_for_errors = false)
    {
        ...
        )
        {
             ...
             */
            return new WP_Error('signature_verification_unsupported', sprintf(/* translators: %s: The filename of the package. */ __('The authenticity of %s could not be verified as signature verification is unavailable on this system.'), '<span class="code">'.esc_html($filename_for_errors).'</span>'), [
                'php' => PHP_VERSION,
        ...
        if(! $signatures)
        {
            return new WP_Error('signature_verification_no_signature', sprintf(/* translators: %s: The filename of the package. */ __('The authenticity of %s could not be verified as no signature was found.'), '<span class="code">'.esc_html($filename_for_errors).'</span>'), [
        ...
        return new WP_Error(
            'signature_verification_failed', sprintf(/* translators: %s: The filename of the package. */ __('The authenticity of %s could not be verified.'), '<span class="code">'.esc_html($filename_for_errors).'</span>'), // Error data helpful for debugging:
            [
                'filename' => $filename_for_errors,
                'keys' => $trusted_keys,
            ...
            <div id="request-filesystem-credentials-form" class="request-filesystem-credentials-form">
                ...
                <label for="hostname">
                    ...
                    <input name="hostname"
                           ...
                           class="code"
                           placeholder="<?php esc_attr_e('example: www.wordpress.org'); ?>"
                           value="<?php echo $hostname_value; ?>"<?php disabled(defined('FTP_HOST')); ?> />
                </label>
                <div class="ftp-username">
                    <label for="username">
                        ...
                        <input name="username"
                               type="text"
                               id="username"
                               value="<?php echo esc_attr($username); ?>"<?php disabled(defined('FTP_USER')); ?> />
                    </label>
                </div>
                <div class="ftp-password">
                    <label for="password">
                        ...
                        <input name="password"
                               type="password"
                               id="password"
                               value="<?php echo $password_value; ?>"<?php disabled(defined('FTP_PASS')); ?>
                               spellcheck="false"/>
                        <?php
                            if(! defined('FTP_PASS'))
                            {
                        ...
                        foreach($types as $name => $text) :
                            ...
                            <label for="<?php echo esc_attr($name); ?>">
                                <input type="radio"
                                       ...
                                       value="<?php echo esc_attr($name); ?>" <?php checked($name, $connection_type); ?> <?php echo $disabled; ?> />
                                <?php echo $text; ?>
                            </label>
                        <?php
                        endforeach;
                        ...
                        <fieldset id="ssh-keys"<?php echo $hidden_class; ?>>
                            ...
                            <label for="public_key">
                                ...
                                <input name="public_key"
                                       type="text"
                                       id="public_key"
                                       aria-describedby="auth-keys-desc"
                                       value="<?php echo esc_attr($public_key); ?>"<?php disabled(defined('FTP_PUBKEY')); ?> />
                            </label>
                            <label for="private_key">
                                ...
                                <input name="private_key"
                                       type="text"
                                       id="private_key"
                                       value="<?php echo esc_attr($private_key); ?>"<?php disabled(defined('FTP_PRIKEY')); ?> />
                            </label>
             ...
             */
                ...
                <p class="request-filesystem-credentials-action-buttons">
                    ...
                    <button class="button cancel-button"
                            ...
                            type="button"><?php _e('Cancel'); ?></button>
    ...
    function wp_opcache_invalidate_directory($dir)
    {
        ...
        if(! is_string($dir) || '' === trim($dir))
        {
            if(WP_DEBUG)
            {
                $error_message = sprintf(/* translators: %s: The function name. */ __('%s expects a non-empty string.'), '<code>wp_opcache_invalidate_directory()</code>');
