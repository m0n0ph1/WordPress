<?php
    ...
    class WP_Comments_List_Table extends WP_List_Table
    {
        ...
        public function __construct($args = [])
        {
            ...
            {
                add_filter('comment_author', [$this, 'floated_admin_avatar'], 10, 2);
            }
        ...
        public function prepare_items()
        {
            ...
            if(! in_array($comment_status, ['all', 'mine', 'moderated', 'approved', 'spam', 'trash'], true))
            {
                $comment_status = 'all';
            }
        ...
        public function column_cb($item)
        {
            ...
            ?>
                ...
                <input id="cb-select-<?php echo $comment->comment_ID; ?>"
                       type="checkbox"
        ...
        public function column_comment($comment)
        {
            ...
            if($comment->comment_parent)
            {
                ...
                if($parent)
                {
                    ...
                    printf(/* translators: %s: Comment link. */ __('In reply to %s.'), '<a href="'.$parent_link.'">'.$name.'</a>');
                }
            }
        ...
        public function column_author($comment)
        {
            ...
            if(! empty($author_url_display))
            {
                ...
                printf('<a href="%s" rel="noopener noreferrer">%s</a><br />', esc_url($author_url), esc_html($author_url_display));
            }
        ...
        public function column_response($comment)
        {
            ...
            {
                $thumb = wp_get_attachment_image($post->ID, [80, 60], true);
                if($thumb)
                {
                    echo $thumb;
                }
        ...
        protected function get_views()
        {
            ...
            $stati = [
                ...
                'all' => _nx_noop('All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', 'comments'),
                // Singular not used.

                /* translators: %s: Number of comments. */
                'mine' => _nx_noop('Mine <span class="count">(%s)</span>', 'Mine <span class="count">(%s)</span>', 'comments'),

                /* translators: %s: Number of comments. */
                'moderated' => _nx_noop('Pending <span class="count">(%s)</span>', 'Pending <span class="count">(%s)</span>', 'comments'),

                /* translators: %s: Number of comments. */
                'approved' => _nx_noop('Approved <span class="count">(%s)</span>', 'Approved <span class="count">(%s)</span>', 'comments'),

                /* translators: %s: Number of comments. */
                'spam' => _nx_noop('Spam <span class="count">(%s)</span>', 'Spam <span class="count">(%s)</span>', 'comments'),

                /* translators: %s: Number of comments. */
                'trash' => _nx_noop('Trash <span class="count">(%s)</span>', 'Trash <span class="count">(%s)</span>', 'comments'),
            ];
            ...
            foreach($stati as $status => $label)
            {
                ...
                */
                ...
                $status_links[$status] = [
                    'url' => esc_url($link),
                    'label' => sprintf(translate_nooped_plural($label, $num_comments->$status), sprintf('<span class="%s-count">%s</span>', ('moderated' === $status) ? 'pending' : $status, number_format_i18n($num_comments->$status))),
        ...
        protected function comment_type_dropdown($comment_type)
        {
            ...
            {
                printf('<label class="screen-reader-text" for="filter-by-comment-type">%s</label>', /* translators: Hidden accessibility text. */ __('Filter by comment type'));
                ...
                foreach($comment_types as $type => $label)
                {
                    ...
                    {
                        printf("\t<option value='%s'%s>%s</option>\n", esc_attr($type), selected($comment_type, $type, false), esc_html($label));
                    }
                }
        ...
        {
            return [
                'author' => ['comment_author', false, __('Author'), __('Table ordered by Comment Author.')],
                'response' => [
                    'comment_post_ID',
                    false,
                    _x('In Response To', 'column name'),
                    __('Table ordered by Post Replied To.'),
                ],
                'date' => 'comment_date',
            ];
        }
        ...
        protected function handle_row_actions($item, $column_name, $primary)
        {
            ...
            if($comment_status && 'all' !== $comment_status)
            {
                ...
                {
                    $actions['unapprove'] = sprintf('<a href="%s" data-wp-lists="%s" class="vim-u vim-destructive aria-button-if-js" aria-label="%s">%s</a>', $unapprove_url, "delete:the-comment-list:comment-{$comment->comment_ID}:e7e7d3:action=dim-comment&amp;new=unapproved", esc_attr__('Unapprove this comment'), __('Unapprove'));
                }
                elseif('unapproved' === $the_comment_status)
                {
                    $actions['approve'] = sprintf('<a href="%s" data-wp-lists="%s" class="vim-a vim-destructive aria-button-if-js" aria-label="%s">%s</a>', $approve_url, "delete:the-comment-list:comment-{$comment->comment_ID}:e7e7d3:action=dim-comment&amp;new=approved", esc_attr__('Approve this comment'), __('Approve'));
                }
            }
            else
            {
            ...
            {
                $actions['unspam'] = sprintf('<a href="%s" data-wp-lists="%s" class="vim-z vim-destructive aria-button-if-js" aria-label="%s">%s</a>', $unspam_url, "delete:the-comment-list:comment-{$comment->comment_ID}:66cc66:unspam=1", esc_attr__('Restore this comment from the spam'), _x('Not Spam', 'comment'));
            }

            if('trash' === $the_comment_status)
            {
                $actions['untrash'] = sprintf('<a href="%s" data-wp-lists="%s" class="vim-z vim-destructive aria-button-if-js" aria-label="%s">%s</a>', $untrash_url, "delete:the-comment-list:comment-{$comment->comment_ID}:66cc66:untrash=1", esc_attr__('Restore this comment from the Trash'), __('Restore'));
            }

            if('spam' === $the_comment_status || 'trash' === $the_comment_status || ! EMPTY_TRASH_DAYS)
            {
                $actions['delete'] = sprintf('<a href="%s" data-wp-lists="%s" class="delete vim-d vim-destructive aria-button-if-js" aria-label="%s">%s</a>', $delete_url, "delete:the-comment-list:comment-{$comment->comment_ID}::delete=1", esc_attr__('Delete this comment permanently'), __('Delete Permanently'));
            }
            else
            {
