<?php
        ...
        if('get-post-item' === $type)
        {
            if(post_type_exists($object_type))
            {
                if(isset($request['ID']))
                {
                    ...
                    if('markup' === $response_format)
                    {
                        echo walk_nav_menu_tree(array_map('wp_setup_nav_menu_item', [get_post($object_id)]), 0, (object) $args);
                    }
        ...
        elseif(preg_match('/quick-search-(posttype|taxonomy)-([a-zA-Z_-]*\b)/', $type, $matches))
        {
            if('posttype' === $matches[1] && get_post_type_object($matches[2]))
            {
                ...
                while($search_results_query->have_posts())
                {
                    ...
                    {
                        $var_by_ref = $post->ID;
                        echo walk_nav_menu_tree(array_map('wp_setup_nav_menu_item', [get_post($var_by_ref)]), 0, (object) $args);
                    }
                    elseif('json' === $response_format)
                    {
                        echo wp_json_encode([
            ...
            elseif('taxonomy' === $matches[1])
            {
                ...
                foreach((array) $terms as $term)
                {
                    if('markup' === $response_format)
                    {
                        echo walk_nav_menu_tree(array_map('wp_setup_nav_menu_item', [$term]), 0, (object) $args);
                    }
                    elseif('json' === $response_format)
                    {
                        echo wp_json_encode([
    ...
    function wp_nav_menu_setup()
    {
        ...
        if(false === get_user_option('managenav-menuscolumnshidden'))
        {
            ...
            update_user_meta($user->ID, 'managenav-menuscolumnshidden', [
                0 => 'link-target',
                1 => 'css-classes',
                2 => 'xfn',
                3 => 'description',
    ...
    function wp_initial_nav_menu_meta_boxes()
    {
        ...
        $initial_meta_boxes = ['add-post-type-page', 'add-post-type-post', 'add-custom-links', 'add-category'];
        $hidden_meta_boxes = [];
        ...
        <div id="<?php echo esc_attr("posttype-{$post_type_name}"); ?>" class="posttypediv">
            ...
            >
                ...
                <p class="quick-search-wrap">
                    <label for="<?php echo esc_attr("quick-search-posttype-{$post_type_name}"); ?>"
                    ...
                    <span class="spinner"></span>
                    <?php
                        submit_button(__('Search'), 'small quick-search-submit hide-if-js', 'submit', false, ['id' => "submit-quick-search-posttype-{$post_type_name}"]);
                    ?>
                </p>
            ...
            <p class="button-controls wp-clearfix"
               data-items-type="<?php echo esc_attr("posttype-{$post_type_name}"); ?>">
    ...
    function wp_nav_menu_item_taxonomy_meta_box($data_object, $box)
    {
        ...
        <div id="<?php echo esc_attr("taxonomy-{$taxonomy_name}"); ?>" class="taxonomydiv">
            ...
            <div id="<?php echo esc_attr("tabs-panel-search-taxonomy-{$taxonomy_name}"); ?>"
                ...
                ?>
                <p class="quick-search-wrap">
                    <label for="<?php echo esc_attr("quick-search-taxonomy-{$taxonomy_name}"); ?>"
                           class="screen-reader-text">
                        <?php
                            /* translators: Hidden accessibility text. */
                            _e('Search');
                    ...
                    <span class="spinner"></span>
                    <?php
                        submit_button(__('Search'), 'small quick-search-submit hide-if-js', 'submit', false, ['id' => "submit-quick-search-taxonomy-{$taxonomy_name}"]);
                    ?>
                </p>
            ...
            <p class="button-controls wp-clearfix"
               data-items-type="<?php echo esc_attr("taxonomy-{$taxonomy_name}"); ?>">
			<span class="list-controls hide-if-no-js">
				<input type="checkbox"<?php wp_nav_menu_disabled_check($nav_menu_selected_id); ?>
					id="<?php echo esc_attr($tab_name); ?>" class="select-all"
                />
        ...
        if(0 === $menu_id || is_nav_menu($menu_id))
        {
            ...
            foreach((array) $menu_data as $_possible_db_id => $_item_object_data)
            {
                if(// Checkbox is not checked.
                    empty($_item_object_data['menu-item-object-id']) && (// And item type either isn't set.
                        ! isset($_item_object_data['menu-item-type']) || // Or URL is the default.
                        in_array($_item_object_data['menu-item-url'], [
                            'https://',
                            'http://',
                            '',
                        ],       true) || // Or it's not a custom menu item (but not the custom home page).
                        ! ('custom' === $_item_object_data['menu-item-type'] && ! isset($_item_object_data['menu-item-db-id'])) || // Or it *is* a custom menu item that already exists.
    ...
    function wp_get_nav_menu_to_edit($menu_id = 0)
    {
        ...
        if(is_nav_menu($menu))
        {
            ...
            {
                return new WP_Error('menu_walker_not_exist', sprintf(/* translators: %s: Walker class name. */ __('The Walker class named %s does not exist.'), '<strong>'.$walker_class_name.'</strong>'));
            }
            ...
            $result .= walk_nav_menu_tree(array_map('wp_setup_nav_menu_item', $menu_items), 0, (object) ['walker' => $walker]);
            $result .= ' </ul> ';
    ...
    function _wp_delete_orphaned_draft_menu_items()
    {
        ...
        $menu_items_to_delete = $wpdb->get_col(
			...
			AND meta_key = '_menu_item_orphaned' AND meta_value < %d", $delete_timestamp
            )
        );
    ...
    function wp_nav_menu_update_menu_items($nav_menu_selected_id, $nav_menu_selected_title)
    {
        ...
        $nav_menu_option['auto_add'] = array_intersect($nav_menu_option['auto_add'], wp_get_nav_menus(['fields' => 'ids']));
