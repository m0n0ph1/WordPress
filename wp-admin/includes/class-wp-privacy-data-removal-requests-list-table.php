    ...
    class WP_Privacy_Data_Removal_Requests_List_Table extends WP_Privacy_Requests_Table
    {
        ...
        public function column_email($item)
        {
            ...
            if('request-confirmed' !== $status)
            {
                ...
                $remove_data_markup = '<span class="remove-personal-data force-remove-personal-data" '.'data-erasers-count="'.esc_attr($erasers_count).'" '.'data-request-id="'.esc_attr($request_id).'" '.'data-nonce="'.esc_attr($nonce).'">';
                ...
                $complete_request_markup .= sprintf(
                    ...
                    wp_nonce_url(
                        add_query_arg([
                                      ...
                                      ], admin_url('erase-personal-data.php')), 'bulk-privacy_requests'
                    )
        ...
        public function column_next_steps($item)
        {
            ...
            switch($status)
            {
                ...
                case 'request-confirmed':
                    /** This filter is documented in wp-admin/includes/ajax-actions.php */ $erasers = apply_filters('wp_privacy_personal_data_erasers', []);
                    $erasers_count = count($erasers);
                    $request_id = $item->ID;
                    $nonce = wp_create_nonce('wp-privacy-erase-personal-data-'.$request_id);

                    echo '<div class="remove-personal-data" '.'data-force-erase="1" '.'data-erasers-count="'.esc_attr($erasers_count).'" '.'data-request-id="'.esc_attr($request_id).'" '.'data-nonce="'.esc_attr($nonce).'">';
