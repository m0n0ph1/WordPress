...
//
        ...
        foreach ((array) $_POST['menu-item'] as $menu_item_data) {
            if (
                ...
                switch ($menu_item_data['menu-item-type']) {
                    ...
                    case 'taxonomy':
                        $_object = get_term($menu_item_data['menu-item-object-id'],
                            $menu_item_data['menu-item-object']);
                        break;
                }
    ...
    function wp_ajax_add_meta()
    {
        ...
        if (isset($_POST['metakeyselect']) || isset($_POST['metakeyinput'])) {
            ...
            if ('auto-draft' === $post->post_status) {
                ...
                $post_data['post_title'] = sprintf(__('Draft created on %1$s at %2$s'), gmdate(__('F j, Y'), $now),
                    gmdate(__('g:i a'), $now));
	 ...
	 */
        ...
        $post_data = !empty($_REQUEST['post_data']) ? _wp_get_allowed_postdata(_wp_translate_postdata(false,
        ...
        if (isset($post_data['context']) && in_array($post_data['context'], ['custom-header', 'custom-background'],
                true)) {
            $wp_filetype = wp_check_filetype_and_ext($_FILES['async-upload']['tmp_name'],
    ...
    function wp_ajax_parse_embed()
    {
        ...
        if (!empty($no_ssl_support) || (is_ssl() && (preg_match('%<(iframe|script|embed) [^>]*src="http://%',
                        $parsed) ||
    ...
    function wp_ajax_delete_theme()
    {
        ...
        $url = wp_nonce_url('themes.php?action=delete&stylesheet=' . urlencode($stylesheet),
    ...
    function wp_ajax_install_plugin()
    {
        ...
        if (current_user_can('activate_plugin',
                $install_status['file']) && is_plugin_inactive($install_status['file'])) {
            $status['activateUrl'] = add_query_arg(
                [
    ...
    function wp_ajax_wp_privacy_export_personal_data()
    {
        ...
        if (0 < count($exporters)) {
            ...
            if (!is_array($response['data'])) {
                wp_send_json_error(
                /* translators: %s: Exporter friendly name. */
                    sprintf(__('Expected data array in response array from exporter: %s.'),
                        esc_html($exporter_friendly_name))
                );
            }
            ...
            if (!array_key_exists('done', $response)) {
                wp_send_json_error(
                /* translators: %s: Exporter friendly name. */
                    sprintf(__('Expected done (boolean) in response array from exporter: %s.'),
                        esc_html($exporter_friendly_name))
                );
            }
        } else {
         ...
         *
         */
        $response = apply_filters('wp_privacy_personal_data_export_page', $response, $exporter_index, $email_address,
            $page,
            $request_id, $send_as_email, $exporter_key);
    ...
    function wp_ajax_wp_privacy_erase_personal_data()
    {
        ...
        if (0 < count($erasers)) {
            ...
            if (!array_key_exists('eraser_friendly_name', $eraser)) {
                /* translators: %d: Eraser array index. */
                wp_send_json_error(sprintf(__('Eraser array at index %d does not include a friendly name.'),
                    $eraser_index));
            }
         ...
         *
         */
        $response = apply_filters('wp_privacy_personal_data_erasure_page', $response, $eraser_index, $email_address,
            $page,
            $request_id, $eraser_key);
