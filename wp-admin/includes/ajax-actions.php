...
//
    ...
    function wp_ajax_fetch_list()
    {
        ...
        $wp_list_table = _get_list_table($list_class, ['screen' => $_GET['list_args']['screen']['id']]);
        if(! $wp_list_table)
        {
            wp_die(0);
        }
        ...
        if(! $total || ! $per_page || ! $page || ! $url)
        {
            ...
            $x = new WP_Ajax_Response([
                                          ...
                                          'supplemental' => [
                                              ...
                                              'i18n_comments_text' => sprintf(/* translators: %s: Number of comments. */ _n('%s Comment', '%s Comments', $counts->approved), number_format_i18n($counts->approved)),
                                              'i18n_moderation_text' => sprintf(/* translators: %s: Number of comments. */ _n('%s Comment in moderation', '%s Comments in moderation', $counts->moderated), number_format_i18n($counts->moderated)),
...
//
    ...
    function _wp_ajax_add_hierarchical_term()
    {
        ...
        foreach($names as $cat_name)
        {
            ...
            $cat_id = wp_insert_term($cat_name, $taxonomy->name, ['parent' => $parent]);
        ...
        $add['supplemental'] = ['newcat_parent' => $sup];
    ...
    function wp_ajax_dim_comment()
    {
        ...
        if(! $comment)
        {
            $x = new WP_Ajax_Response([
                                          'what' => 'comment',
                                          'id' => new WP_Error('invalid_comment', /* translators: %d: Comment ID. */ sprintf(__('Comment %d does not exist'), $id)),
                                      ]);
            $x->send();
        }
        ...
        if(in_array($current, ['unapproved', 'spam'], true))
        {
    ...
    function wp_ajax_get_comments($action)
    {
        ...
        $wp_list_table = _get_list_table('WP_Post_Comments_List_Table', ['screen' => 'edit-comments']);
    ...
    function wp_ajax_replyto_comment($action)
    {
        ...
        else
        {
            ...
            {
                $wp_list_table = _get_list_table('WP_Comments_List_Table', ['screen' => 'edit-comments']);
            }
            $wp_list_table->single_row($comment);
        }
        $comment_list_item = ob_get_clean();
    ...
    function wp_ajax_edit_comment()
    {
        ...
        $wp_list_table = _get_list_table($checkbox ? 'WP_Comments_List_Table' : 'WP_Post_Comments_List_Table', ['screen' => 'edit-comments']);
    ...
    function wp_ajax_add_meta()
    {
        ...
        { // Update?
            ...
            $x = new WP_Ajax_Response([
                                          ...
                                          'position' => 0,
                                          'supplemental' => ['postid' => $meta->post_id],
                                      ]);
        }
        $x->send();
    }
    ...
    function wp_ajax_add_user($action)
    {
        ...
        $x = new WP_Ajax_Response([
                                      ...
                                      'supplemental' => [
                                          'show-link' => sprintf(/* translators: %s: The new user. */ __('User %s added'), '<a href="#user-'.$user_id.'">'.$user_object->user_login.'</a>'),
                                          'role' => $role,
                                      ],
                                  ]);
        $x->send();
    ...
    function wp_ajax_closed_postboxes()
    {
        ...
        if(is_array($hidden))
        {
            ...
            $hidden = array_diff($hidden, ['submitdiv', 'linksubmitdiv', 'manage-menu', 'create-menu']);
    ...
    function wp_ajax_inline_save()
    {
        ...
        if(! empty($data['post_name']) && in_array($post['post_status'], ['draft', 'pending'], true))
        {
        ...
        {
            $request_post = [get_post($_POST['post_ID'])];
        ...
        $wp_list_table->display_rows([get_post($_POST['post_ID'])], $level);
    ...
    function wp_ajax_media_create_image_subsizes()
    {
        ...
        {
            wp_send_json_error(['message' => __('Sorry, you are not allowed to upload files.')]);
        }

        if(empty($_POST['attachment_id']))
        {
            wp_send_json_error(['message' => __('Upload failed. Please reload and try again.')]);
        }
	 ...
	 */
        ...
        else
        {
            ...
            if(! $response)
            {
                wp_send_json_error(['message' => __('Upload failed.')]);
            }
        }
	 ...
	 */
        ...
        $post_data = ! empty($_REQUEST['post_data']) ? _wp_get_allowed_postdata(_wp_translate_postdata(false, (array) $_REQUEST['post_data'])) : [];
        ...
        if(
            isset($post_data['context']) && in_array($post_data['context'], [
                'custom-header',
                'custom-background'
            ],                                       true)
        )
    ...
    function wp_ajax_wp_fullscreen_save_post()
    {
        ...
        wp_send_json_success(['last_edited' => $last_edited]);
    }
    ...
    function wp_ajax_query_themes()
    {
        ...
        $args = wp_parse_args(wp_unslash($_REQUEST['request']), [
            'per_page' => 20,
            'fields' => array_merge((array) $theme_field_defaults, [
                'reviews_url' => true,
    ...
    function wp_ajax_crop_image()
    {
        ...
        {
            wp_send_json_error(['message' => __('Image could not be processed.')]);
        }
        ...
        switch($context)
        {
            case 'site-icon':
                ...
                if(get_post_meta($attachment_id, '_wp_attachment_context', true) == $context)
                {
                    ...
                    add_filter('image_size_names_choose', [$wp_site_icon, 'additional_sizes']);
                    break;
                }
            ...
            default:
                ...
                $use_original_title = (('' !== trim($original_attachment->post_title)) && /*
				 * Check if the original image has a title other than the "filename" default,
				 * meaning the image had a title when originally uploaded or its title was edited.
				 */
                    ($parent_basename !== $sanitized_post_title) && (pathinfo($parent_basename, PATHINFO_FILENAME) !== $sanitized_post_title));
    ...
    function wp_ajax_wp_privacy_export_personal_data()
    {
        ...
        if(0 < count($exporters))
        {
            ...
            {
                wp_send_json_error(/* translators: %s: Exporter array index. */ sprintf(__('Expected an array describing the exporter at index %s.'), $exporter_key));
            }

            if(! array_key_exists('exporter_friendly_name', $exporter))
            {
                wp_send_json_error(/* translators: %s: Exporter array index. */ sprintf(__('Exporter array at index %s does not include a friendly name.'), $exporter_key));
            }
            ...
            {
                wp_send_json_error(/* translators: %s: Exporter friendly name. */ sprintf(__('Exporter does not include a callback: %s.'), esc_html($exporter_friendly_name)));
            }

            if(! is_callable($exporter['callback']))
            {
                wp_send_json_error(/* translators: %s: Exporter friendly name. */ sprintf(__('Exporter callback is not a valid callback: %s.'), esc_html($exporter_friendly_name)));
            }
            ...
            {
                wp_send_json_error(/* translators: %s: Exporter friendly name. */ sprintf(__('Expected response as an array from exporter: %s.'), esc_html($exporter_friendly_name)));
            }

            if(! array_key_exists('data', $response))
            {
                wp_send_json_error(/* translators: %s: Exporter friendly name. */ sprintf(__('Expected data in response array from exporter: %s.'), esc_html($exporter_friendly_name)));
            }

            if(! is_array($response['data']))
            {
                wp_send_json_error(/* translators: %s: Exporter friendly name. */ sprintf(__('Expected data array in response array from exporter: %s.'), esc_html($exporter_friendly_name)));
            }

            if(! array_key_exists('done', $response))
            {
                wp_send_json_error(/* translators: %s: Exporter friendly name. */ sprintf(__('Expected done (boolean) in response array from exporter: %s.'), esc_html($exporter_friendly_name)));
            }
        }
        else
        {
    ...
    function wp_ajax_wp_privacy_erase_personal_data()
    {
        ...
        if(0 < count($erasers))
        {
            ...
            {
                wp_send_json_error(sprintf(/* translators: %s: Eraser friendly name. */ __('Eraser does not include a callback: %s.'), esc_html($eraser_friendly_name)));
            }

            if(! is_callable($eraser['callback']))
            {
                wp_send_json_error(sprintf(/* translators: %s: Eraser friendly name. */ __('Eraser callback is not valid: %s.'), esc_html($eraser_friendly_name)));
            }
            ...
            if(! is_array($response))
            {
                wp_send_json_error(sprintf(/* translators: 1: Eraser friendly name, 2: Eraser array index. */ __('Did not receive array from %1$s eraser (index %2$d).'), esc_html($eraser_friendly_name), $eraser_index));
            }

            if(! array_key_exists('items_removed', $response))
            {
                wp_send_json_error(sprintf(/* translators: 1: Eraser friendly name, 2: Eraser array index. */ __('Expected items_removed key in response array from %1$s eraser (index %2$d).'), esc_html($eraser_friendly_name), $eraser_index));
            }

            if(! array_key_exists('items_retained', $response))
            {
                wp_send_json_error(sprintf(/* translators: 1: Eraser friendly name, 2: Eraser array index. */ __('Expected items_retained key in response array from %1$s eraser (index %2$d).'), esc_html($eraser_friendly_name), $eraser_index));
            }

            if(! array_key_exists('messages', $response))
            {
                wp_send_json_error(sprintf(/* translators: 1: Eraser friendly name, 2: Eraser array index. */ __('Expected messages key in response array from %1$s eraser (index %2$d).'), esc_html($eraser_friendly_name), $eraser_index));
            }

            if(! is_array($response['messages']))
            {
                wp_send_json_error(sprintf(/* translators: 1: Eraser friendly name, 2: Eraser array index. */ __('Expected messages key to reference an array in response array from %1$s eraser (index %2$d).'), esc_html($eraser_friendly_name), $eraser_index));
            }

            if(! array_key_exists('done', $response))
            {
                wp_send_json_error(sprintf(/* translators: 1: Eraser friendly name, 2: Eraser array index. */ __('Expected done flag in response array from %1$s eraser (index %2$d).'), esc_html($eraser_friendly_name), $eraser_index));
            }
        }
        else
        {
    ...
    {
        _doing_it_wrong(
            'wp_ajax_health_check_dotorg_communication', sprintf(// translators: 1: The Site Health action that is no longer used by core. 2: The new function that replaces it.
            __('The Site Health check for %1$s has been replaced with %2$s.'), 'wp_ajax_health_check_dotorg_communication', 'WP_REST_Site_Health_Controller::test_dotorg_communication'
        ),  '5.6.0'
        );
    ...
    {
        _doing_it_wrong(
            'wp_ajax_health_check_background_updates', sprintf(// translators: 1: The Site Health action that is no longer used by core. 2: The new function that replaces it.
            __('The Site Health check for %1$s has been replaced with %2$s.'), 'wp_ajax_health_check_background_updates', 'WP_REST_Site_Health_Controller::test_background_updates'
        ),  '5.6.0'
        );
    ...
    {
        _doing_it_wrong(
            'wp_ajax_health_check_loopback_requests', sprintf(// translators: 1: The Site Health action that is no longer used by core. 2: The new function that replaces it.
            __('The Site Health check for %1$s has been replaced with %2$s.'), 'wp_ajax_health_check_loopback_requests', 'WP_REST_Site_Health_Controller::test_loopback_requests'
        ),  '5.6.0'
        );
    ...
    {
        _doing_it_wrong(
            'wp_ajax_health_check_get_sizes', sprintf(// translators: 1: The Site Health action that is no longer used by core. 2: The new function that replaces it.
            __('The Site Health check for %1$s has been replaced with %2$s.'), 'wp_ajax_health_check_get_sizes', 'WP_REST_Site_Health_Controller::get_directory_sizes'
        ),  '5.6.0'
        );
    ...
    function wp_ajax_toggle_auto_updates()
    {
        ...
        {
            wp_send_json_error(['error' => __('Invalid data. No selected item.')]);
        }
        ...
        {
            wp_send_json_error(['error' => __('Invalid data. Unknown state.')]);
        }
        $state = $_POST['state'];

        if('plugin' !== $_POST['type'] && 'theme' !== $_POST['type'])
        {
            wp_send_json_error(['error' => __('Invalid data. Unknown type.')]);
        }
        $type = $_POST['type'];
        ...
        if('disable' === $state)
        {
            $auto_updates = array_diff($auto_updates, [$asset]);
        }
        else
        {
            $auto_updates[] = $asset;
    ...
    function wp_ajax_send_password_reset()
    {
        ...
        if(true === $results)
        {
            wp_send_json_success(/* translators: %s: User's display name. */ sprintf(__('A password reset link was emailed to %s.'), $user->display_name));
        }
        else
        {
