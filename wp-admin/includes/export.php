    ...
    function export_wp($args = [])
    {
        ...
        if ($args['category'] && 'post' === $args['content']) {
            ...
            if ($term) {
                ...
                $where .= $wpdb->prepare(" AND {$wpdb->term_relationships}.term_taxonomy_id = %d",
        ...
        if (in_array($args['content'], ['post', 'page', 'attachment'], true)) {
            ...
            if ($args['start_date']) {
                $where .= $wpdb->prepare(" AND {$wpdb->posts}.post_date >= %s",
                    gmdate('Y-m-d', strtotime($args['start_date'])));
            }
            
            if ($args['end_date']) {
                $where .= $wpdb->prepare(" AND {$wpdb->posts}.post_date < %s",
                    gmdate('Y-m-d', strtotime('+1 month', strtotime($args['end_date']))));
            }
        }
        ...
        function wxr_term_meta($term)
        {
            ...
            $termmeta = $wpdb->get_results($wpdb->prepare("SELECT * FROM $wpdb->termmeta WHERE term_id = %d",
                $term->term_id));
                        ...
                        while ($next_posts = array_splice($post_ids, 0, 20)) {
                            ...
                            foreach ($posts as $post) {
                                ...
                                <item>
                                    ...
                                    <?php
                                        $postmeta = $wpdb->get_results($wpdb->prepare("SELECT * FROM $wpdb->postmeta WHERE post_id = %d",
                                            $post->ID));
                                        foreach ($postmeta as $meta) :
                                             ...
                                             *
                                             */
                                            if (apply_filters('wxr_export_skip_postmeta', false, $meta->meta_key,
                                                $meta)) {
                                                continue;
                                            }
                                            ?>
                                        ...
                                        $_comments = $wpdb->get_results($wpdb->prepare("SELECT * FROM $wpdb->comments WHERE comment_post_ID = %d AND comment_approved <> 'spam'",
                                            $post->ID));
                                        ...
                                        foreach ($comments as $c) :
                                            ...
                                            <wp:comment>
                                                ...
                                                <?php
                                                    $c_meta = $wpdb->get_results($wpdb->prepare("SELECT * FROM $wpdb->commentmeta WHERE comment_id = %d",
                                                        $c->comment_ID));
                                                    foreach ($c_meta as $meta) :
                                                         ...
                                                         *
                                                         */
                                                        if (apply_filters('wxr_export_skip_commentmeta', false,
                                                            $meta->meta_key,
                                                            $meta)) {
                                                            continue;
                                                        }
