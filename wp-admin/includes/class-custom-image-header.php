<?php
    ...
    class Custom_Image_Header
    {
        ...
        public function __construct($admin_header_callback, $admin_image_div_callback = '')
        {
            ...
            add_action('customize_save_after', [$this, 'customize_set_last_used']);
            add_action('wp_ajax_custom-header-crop', [$this, 'ajax_header_crop']);
            add_action('wp_ajax_custom-header-add', [$this, 'ajax_header_add']);
        ...
        public function init()
        {
            ...
            add_action("admin_print_scripts-{$page}", [$this, 'js_includes']);
            add_action("admin_print_styles-{$page}", [$this, 'css_includes']);
            add_action("admin_head-{$page}", [$this, 'help']);
            add_action("admin_head-{$page}", [$this, 'take_action'], 50);
            add_action("admin_head-{$page}", [$this, 'js'], 50);
        ...
        public function step()
        {
            ...
            $step = (int) $_GET['step'];
            if($step < 1 || 3 < $step || (2 === $step && ! wp_verify_nonce($_REQUEST['_wpnonce-custom-header-upload'], 'custom-header-upload')) || (3 === $step && ! wp_verify_nonce($_REQUEST['_wpnonce'], 'custom-header-crop-image')))
            {
                return 1;
            }
        ...
        final public function set_header_image($choice)
        {
            ...
            if(in_array($choice, ['remove-header', 'random-default-image', 'random-uploaded-image'], true))
            {
        ...
        public function step_2()
        {
            ...
            {
                wp_die('<h1>'.__('Something went wrong.').'</h1>'.'<p>'.__('The active theme does not support uploading a custom header image.').'</p>', 403);
            }
            ...
            if(! current_theme_supports('custom-header', 'flex-height') && ! current_theme_supports('custom-header', 'flex-width') && (int) get_theme_support('custom-header', 'width') === $width && (int) get_theme_support('custom-header', 'height') === $height)
            {
                // Add the metadata.
                if(file_exists($file))
                {
            ...
            <div class="wrap">
                ...
                <form method="post" action="<?php echo esc_url(add_query_arg('step', 3)); ?>">
                    ...
                    <p class="hide-if-js">
                        <strong><?php _e('You need JavaScript to choose a part of the image.'); ?></strong></p>
        ...
        public function step_1()
        {
            ...
            <div class="wrap">
                ...
                <?php
                    ...
                    {
                        $message = sprintf(/* translators: %s: URL to header image configuration in Customizer. */ __('You can now manage and live-preview Custom Header in the <a href="%s">Customizer</a>.'), admin_url('customize.php?autofocus[control]=header_image'));
                ...
                <table class="form-table" role="presentation">
                                ...
                                <?php
                                    ...
                                    else
                                    {
                                        ...
                                        <div id="headimg" style="<?php echo $header_image_style; ?>">
                                            ...
                                            ?>
                                            <h1><a id="name"
                                                   class="displaying-header-text" <?php echo $style; ?>
                                                   onclick="return false;"
                                                   href="<?php bloginfo('url'); ?>"
                                                   tabindex="-1"><?php bloginfo('name'); ?></a></h1>
                                            <div id="desc"
                                                 class="displaying-header-text" <?php echo $style; ?>><?php bloginfo('description'); ?></div>
                                        </div>
                                    <?php } ?>
                            </td>
                        </tr>
                    ...
                    <?php if(current_user_can('upload_files') && current_theme_supports('custom-header', 'uploads')) : ?>
                        <tr>
                            ...
                            <td>
                                <p><?php _e('You can select an image to be shown at the top of your site by uploading from your computer or choosing from your media library. After selecting an image you will be able to crop it.'); ?>
                                    <br/>
                                    <?php
                                        if(! current_theme_supports('custom-header', 'flex-height') && ! current_theme_supports('custom-header', 'flex-width'))
                                        {
                                            printf(/* translators: 1: Image width in pixels, 2: Image height in pixels. */ __('Images of exactly <strong>%1$d &times; %2$d pixels</strong> will be used as-is.').'<br />', get_theme_support('custom-header', 'width'), get_theme_support('custom-header', 'height'));
                                        }
                                        ...
                                        if(current_theme_supports('custom-header', 'flex-height') || current_theme_supports('custom-header', 'flex-width'))
                                        {
                                            ...
                                            {
                                                printf(/* translators: %s: Size in pixels. */ __('Suggested width is %s.').' ', sprintf(/* translators: %d: Custom header width. */ '<strong>'.__('%d pixels').'</strong>', get_theme_support('custom-header', 'width')));
                                            }

                                            if(current_theme_supports('custom-header', 'height'))
                                            {
                                                printf(/* translators: %s: Size in pixels. */ __('Suggested height is %s.').' ', sprintf(/* translators: %d: Custom header height. */ '<strong>'.__('%d pixels').'</strong>', get_theme_support('custom-header', 'height')));
                                            }
                                        }
                                    ?>
                                </p>
                                <form enctype="multipart/form-data"
                                      ...
                                      method="post"
                                      action="<?php echo esc_url(add_query_arg('step', 2)); ?>">
                                    <p>
                                ...
                                <td>
                                    <p>
                                        <label><input type="checkbox"
                                                      ...
                                                      id="display-header-text"<?php checked(display_header_text()); ?> /> <?php _e('Show header text with your image.'); ?>
                                        </label>
                                    </p>
                                </td>
                            </tr>
        ...
        public function step_3()
        {
            ...
            {
                wp_die('<h1>'.__('Something went wrong.').'</h1>'.'<p>'.__('The active theme does not support a flexible sized header image.').'</p>', 403);
            }
