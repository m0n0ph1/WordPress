    ...
    class WP_Debug_Data
    {
        ...
        public static function debug_data()
        {
            ...
            $info['wp-dropins'] = [
                'label' => __('Drop-ins'),
                'show_count' => true,
                'description' => sprintf(/* translators: %s: wp-content directory name. */ __('Drop-ins are single files, found in the %s directory, that replace or enhance WordPress features in ways that are not possible for traditional plugins.'), '<code>'.str_replace(ABSPATH, '', WP_CONTENT_DIR).'</code>'),
                'fields' => [],
            ];
            ...
            if(is_multisite())
            {
                ...
                $info['wp-core']['fields']['site_count'] = [
                    'label' => __('Site count'),
                    'value' => $site_count,
                ];
                $info['wp-core']['fields']['network_count'] = [
                    'label' => __('Network count'),
                    'value' => $network_query->found_networks,
                ];
            }
            $info['wp-core']['fields']['user_count'] = [
                'label' => __('User count'),
            ...
            $info['wp-media']['fields']['image_editor'] = [
            ...
            $info['wp-media']['fields']['imagick_module_version'] = [
            ...
            $info['wp-media']['fields']['imagemagick_version'] = [
            ...
            $info['wp-media']['fields']['imagick_version'] = [
            ...
            else
            {
                ...
                $info['wp-media']['fields']['post_max_size'] = [
                    'label' => __('Max size of post data allowed'),
                    'value' => $post_max_size,
                ];
                $info['wp-media']['fields']['upload_max_filesize'] = [
                    'label' => __('Max size of an uploaded file'),
                    'value' => $upload_max_filesize,
                ];
                $info['wp-media']['fields']['max_effective_size'] = [
            ...
            if('WP_Image_Editor_Imagick' === _wp_image_editor_choose() && isset($imagick) && $imagick instanceof Imagick)
            {
                ...
                $info['wp-media']['fields']['imagick_limits'] = [
                ...
                $info['wp-media']['fields']['imagemagick_file_formats'] = [
            ...
            $info['wp-media']['fields']['gd_version'] = [
                'label' => __('GD version'),
            ...
            {
                $info['wp-media']['fields']['gd_formats'] = [
            ...
            $info['wp-media']['fields']['ghostscript_version'] = [
            ...
            $php_version = sprintf('%s %s', $php_version_debug, ($php64bit ? __('(Supports 64bit values)') : __('(Does not support 64bit values)')));
            if($php64bit)
            {
                $php_version_debug .= ' 64bit';
            }
            $info['wp-server']['fields']['server_architecture'] = [
            ...
            $info['wp-server']['fields']['httpd_software'] = [
                'label' => __('Web server'),
            ...
            $info['wp-server']['fields']['php_version'] = [
                'label' => __('PHP version'),
                'value' => $php_version,
                'debug' => $php_version_debug,
            ];
            $info['wp-server']['fields']['php_sapi'] = [
                'label' => __('PHP SAPI'),
                'value' => PHP_SAPI,
                'debug' => PHP_SAPI,
            ];
            ...
            else
            {
                ...
                $info['wp-server']['fields']['max_input_time'] = [
                    'label' => __('Max input time'),
                    'value' => ini_get('max_input_time'),
                ];
                $info['wp-server']['fields']['upload_max_filesize'] = [
            ...
            {
                $curl = curl_version();
                $info['wp-server']['fields']['curl_version'] = [
            ...
            $info['wp-server']['fields']['imagick_availability'] = [
            ...
            if(is_file(ABSPATH.'.htaccess'))
            {
                ...
                $info['wp-server']['fields']['htaccess_extra_rules'] = [
            ...
            $info['wp-server']['fields']['current'] = [
                'label' => __('Current time'),
                'value' => $date->format(DateTime::ATOM),
            ];
            $info['wp-server']['fields']['utc-time'] = [
                'label' => __('Current UTC time'),
                'value' => $date->format(DateTime::RFC850),
            ];
            $info['wp-server']['fields']['server-time'] = [
            ...
            $info['wp-database']['fields']['extension'] = [
                'label' => __('Extension'),
                'value' => $extension,
            ];
            $info['wp-database']['fields']['server_version'] = [
                'label' => __('Server version'),
                'value' => $server,
            ];
            $info['wp-database']['fields']['client_version'] = [
                'label' => __('Client version'),
                'value' => $client_version,
            ];
            $info['wp-database']['fields']['database_user'] = [
                'label' => __('Database username'),
                'value' => $wpdb->dbuser,
                'private' => true,
            ];
            $info['wp-database']['fields']['database_host'] = [
                'label' => __('Database host'),
                'value' => $wpdb->dbhost,
                'private' => true,
            ];
            $info['wp-database']['fields']['database_name'] = [
                'label' => __('Database name'),
                'value' => $wpdb->dbname,
                'private' => true,
            ];
            $info['wp-database']['fields']['database_prefix'] = [
                'label' => __('Table prefix'),
                'value' => $wpdb->prefix,
                'private' => true,
            ];
            $info['wp-database']['fields']['database_charset'] = [
                'label' => __('Database charset'),
                'value' => $wpdb->charset,
                'private' => true,
            ];
            $info['wp-database']['fields']['database_collate'] = [
                'label' => __('Database collation'),
                'value' => $wpdb->collate,
                'private' => true,
            ];
            $info['wp-database']['fields']['max_allowed_packet'] = [
                'label' => __('Max allowed packet size'),
                'value' => self::get_mysql_var('max_allowed_packet'),
            ];
            $info['wp-database']['fields']['max_connections'] = [
            ...
            {
                $active_theme_parent_theme = sprintf(/* translators: 1: Theme name. 2: Theme slug. */ __('%1$s (%2$s)'), $active_theme->parent_theme, $active_theme->template);
                $active_theme_parent_theme_debug = sprintf('%s (%s)', $active_theme->parent_theme, $active_theme->template);
            }
            else
            {
            ...
            $info['wp-active-theme']['fields'] = [
                'name' => [
                    'label' => __('Name'),
                    'value' => sprintf(/* translators: 1: Theme name. 2: Theme slug. */ __('%1$s (%2$s)'), $active_theme->name, $active_theme->stylesheet),
                ],
                'version' => [
                    'label' => __('Version'),
            ...
            if($parent_theme)
            {
                ...
                $info['wp-parent-theme']['fields'] = [
                    'name' => [
                        'label' => __('Name'),
                        'value' => sprintf(/* translators: 1: Theme name. 2: Theme slug. */ __('%1$s (%2$s)'), $parent_theme->name, $parent_theme->stylesheet),
                    ],
                    'version' => [
                        'label' => __('Version'),
            ...
            foreach($all_themes as $theme_slug => $theme)
            {
                ...
                $info['wp-themes-inactive']['fields'][sanitize_text_field($theme->name)] = [
                    'label' => sprintf(/* translators: 1: Theme name. 2: Theme slug. */ __('%1$s (%2$s)'), $theme->name, $theme_slug),
        ...
        public static function format($info_array, $data_type)
        {
            ...
            foreach($info_array as $section => $details)
            {
                ...
                $return .= sprintf("### %s%s ###\n\n", $section_label, (isset($details['show_count']) && $details['show_count'] ? sprintf(' (%d)', count($details['fields'])) : ''));
