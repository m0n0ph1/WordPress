<?php
    ...
    function delete_theme($stylesheet, $redirect = '')
    {
        ...
        if(! $deleted)
        {
            return new WP_Error('could_not_remove_theme', /* translators: %s: Theme name. */ sprintf(__('Could not fully remove the theme %s.'), $stylesheet));
        }
    ...
    function wp_prepare_themes_for_js($themes = null)
    {
        ...
        foreach($themes as $theme)
        {
            ...
            else
            {
                 ...
                 */
                $auto_update_filter_payload = (object) [
                    'theme' => $slug,
            ...
            $prepared_themes[$slug] = [
                ...
                'actions' => [
                    ...
                    'autoupdate' => wp_is_auto_update_enabled_for_type('theme') && ! is_multisite() && current_user_can('update_themes') ? wp_nonce_url(admin_url('themes.php?action='.$auto_update_action.'&amp;stylesheet='.$encoded_slug), 'updates') : null,
                ],
            ...
            <div class="theme-wrap wp-clearfix" role="document">
                ...
                <div class="theme-about wp-clearfix">
                    <div class="theme-screenshots">
                        ...
                        <div class="screenshot"><img src="{{ data.screenshot[0] }}?ver={{ data.version }}" alt=""/>
                        </div>
                        <# } else { #>
                    ...
                    <div class="theme-info">
                        ...
                        <div class="theme-rating">
                            ...
                            <a class="num-ratings" target="_blank" href="{{ data.reviews_url }}">
                                <?php
                                    printf('%1$s <span class="screen-reader-text">%2$s</span>', /* translators: %s: Number of ratings. */ sprintf(__('(%s ratings)'), '{{ data.num_ratings }}'), /* translators: Hidden accessibility text. */ __('(opens in a new tab)'));
                                ?>
                            </a>
                        </div>
                        <# } #>
                        ...
                        <p class="parent-theme">
                            <?php
                                printf(/* translators: %s: Theme name. */ __('This is a child theme of %s.'), '<strong>{{{ data.parent }}}</strong>');
                            ?>
                        </p>
                        <# } #>
                        ...
                        <div class="notice notice-error notice-alt notice-large"><p>
                                ...
                                <?php
                                    printf(/* translators: %s: URL to the themes page (also it activates the theme). */ ' '.__('However, you can still <a href="%s">activate this theme</a>, and use the Site Editor to customize it.'), '{{{ data.actions.activate }}}');
                                ?>
                                <# } #>
                            </p></div>
                        <# } #>
                ...
                <div class="theme-actions">
                    <# if ( data.active ) { #>
                    <button type="button"
                            class="button button-primary customize-theme"><?php _e('Customize'); ?></button>
                    <# } else if ( 'installed' === data.type ) { #>
                    <?php if(current_user_can('delete_themes')) { ?>
                        <# if ( data.actions && data.actions['delete'] ) { #>
                        <a href="{{{ data.actions['delete'] }}}"
                           data-slug="{{ data.id }}"
                           class="button button-secondary delete-theme"><?php _e('Delete'); ?></a>
                        <# } #>
                    <?php } ?>
                    ...
                    <button type="button"
                            ...
                            data-slug="{{ data.id }}"><?php _e('Live Preview'); ?></button>
                    <# } else { #>
                    ...
                    <# if ( data.compatibleWP && data.compatiblePHP ) { #>
                    <button type="button"
                            class="button theme-install"
                            data-slug="{{ data.id }}"><?php _e('Install'); ?></button>
                    <button type="button"
                            class="button button-primary theme-install preview"
                            data-slug="{{ data.id }}"><?php _e('Install &amp; Preview'); ?></button>
                    <# } else { #>
                    <button type="button" class="button disabled"><?php _ex('Cannot Install', 'theme'); ?></button>
                    <button type="button"
                            class="button button-primary disabled"><?php _e('Install &amp; Preview'); ?></button>
                    <# } #>
                    <# } #>
                </div>
            </div>
    ...
    function paused_themes_notice()
    {
        ...
        $message = sprintf('<p><strong>%s</strong><br>%s</p><p><a href="%s">%s</a></p>', __('One or more themes failed to load properly.'), __('You can find more details and make changes on the Themes screen.'), esc_url(admin_url('themes.php')), __('Go to the Themes screen'));
