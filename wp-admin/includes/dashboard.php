    ...
    function wp_dashboard_setup()
    {
        ...
        if (is_blog_admin() && current_user_can(get_post_type_object('post')->cap->create_posts)) {
            $quick_draft_title = sprintf('<span class="hide-if-no-js">%1$s</span> <span class="hide-if-js">%2$s</span>',
                __('Quick Draft'), __('Your Recent Drafts'));
            ...
            <ul>
                <?php
                    ...
                    foreach (['post', 'page'] as $post_type) {
                        ...
                        if ($num_posts && $num_posts->publish) {
                            ...
                            if ($post_type_object && current_user_can($post_type_object->cap->edit_posts)) {
                                printf('<li class="%1$s-count"><a href="edit.php?post_type=%1$s">%2$s</a></li>',
                                    $post_type,
                                    $text);
                            } else {
                    ...
                    if ($num_comm && ($num_comm->approved || $num_comm->moderated)) {
                        ...
                        /* translators: %s: Number of comments. */
                        $text = sprintf(_n('%s Comment in moderation', '%s Comments in moderation',
                            $num_comm->moderated),
    ...
    function wp_dashboard_recent_posts($args)
    {
        ...
        if ($posts->have_posts()) {
            ...
            while ($posts->have_posts()) {
                ...
                $recent_post_link = current_user_can('edit_post',
                    get_the_ID()) ? get_edit_post_link() : get_permalink();
...
//
    ...
    function wp_dashboard_browser_nag()
    {
         ...
         *
         */
        echo apply_filters('browse-happy-notice', $notice,
            $response); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
    }
