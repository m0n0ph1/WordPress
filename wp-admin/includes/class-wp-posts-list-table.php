    ...
    class WP_Posts_List_Table extends WP_List_Table
    {
        ...
        public function __construct($args = [])
        {
            ...
            $this->user_posts_count = (int) $wpdb->get_var(
				...
				AND post_author = %d", $post_type, get_current_user_id()
                )
            );
            ...
            if('post' === $post_type && $sticky_posts)
            {
                ...
                $this->sticky_posts_count = (int) $wpdb->get_var(
					...
					AND ID IN ($sticky_posts)", $post_type
                    )
                );
            }
        }
        ...
        public function get_columns()
        {
            ...
            if(
                post_type_supports($post_type, 'comments') && ! in_array($post_status, [
                    'pending',
                    'draft',
                    'future',
                ],                                                       true)
            ...
            {
                $posts_columns['comments'] = sprintf('<span class="vers comment-grey-bubble" title="%1$s" aria-hidden="true"></span><span class="screen-reader-text">%2$s</span>', esc_attr__('Comments'), /* translators: Hidden accessibility text. */ __('Comments'));
            }
        ...
        private function _display_rows_hierarchical($pages, $pagenum = 1, $per_page = 20)
        {
            ...
            if(empty($_REQUEST['s']))
            {
                ...
                foreach($pages as $page)
                {
                    ...
                    {
                        $page->post_parent = 0;
                        $wpdb->update($wpdb->posts, ['post_parent' => 0], ['ID' => $page->ID]);
        ...
        public function single_row($post, $level = 0)
        {
            ...
            <tr id="post-<?php echo $post->ID; ?>"
                class="<?php echo implode(' ', get_post_class($classes, $post->ID)); ?>">
        ...
        public function column_cb($item)
        {
            ...
            if(apply_filters('wp_list_table_show_post_checkbox', $show, $post)) :
                ...
                <div class="locked-indicator">
                    ...
                    <span class="screen-reader-text">
				<?php
                    printf(/* translators: Hidden accessibility text. %s: Post title. */ __('&#8220;%s&#8221; is locked'), _draft_or_post_title());
                ?>
				</span>
                </div>
            <?php
        ...
        public function column_date($post)
        {
            ...
            {
                $t_time = sprintf(/* translators: 1: Post date, 2: Post time. */ __('%1$s at %2$s'), /* translators: Post date format. See https://www.php.net/manual/datetime.format.php */ get_the_time(__('Y/m/d'), $post), /* translators: Post time format. See https://www.php.net/manual/datetime.format.php */ get_the_time(__('g:i a'), $post));
        ...
        protected function get_edit_link($args, $link_text, $css_class = '')
        {
            ...
            if(! empty($css_class))
            {
                $class_html = sprintf(' class="%s"', esc_attr($css_class));
                        ...
                        while($bulk < 2) :
                                    ...
                                    <div class="inline-edit-wrapper"
                                        ...
                                        <fieldset class="inline-edit-col-left">
                                            <legend class="inline-edit-legend"
                                                    id="<?php echo $bulk ? 'bulk' : 'quick'; ?>-edit-legend"><?php echo $bulk ? __('Bulk Edit') : __('Quick Edit'); ?></legend>
                            ...
                            ?>
                                                        ...
                                                        <?php if(current_user_can($taxonomy->cap->assign_terms)) : ?>
                                                            ...
                                                            <div class="inline-edit-tags-wrap">
                                                                <label class="inline-edit-tags">
                                                                    ...
                                                                    <textarea
                                                                            data-wp-taxonomy="<?php echo $taxonomy_name; ?>"
                                                                            cols="22"
                                                                            rows="1"
                                                                ...
                                                                <p class="howto"
                                                                   id="inline-edit-<?php echo esc_attr($taxonomy->name); ?>-desc"><?php echo esc_html($taxonomy->labels->separate_items_with_commas); ?></p>
                                                            </div>
                                        ...
                                        <div class="submit inline-edit-save">
                                            ...
                                            <input type="hidden"
                                                   name="screen"
                                                   value="<?php echo esc_attr($screen->id); ?>"/>
        ...
        protected function get_views()
        {
            ...
            if($this->user_posts_count && $this->user_posts_count !== $total_posts)
            {
                ...
                $mine_inner_html = sprintf(/* translators: %s: Number of posts. */ _nx('Mine <span class="count">(%s)</span>', 'Mine <span class="count">(%s)</span>', $this->user_posts_count, 'posts'), number_format_i18n($this->user_posts_count));
                $mine = [
            ...
            $all_inner_html = sprintf(/* translators: %s: Number of posts. */ _nx('All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', $total_posts, 'posts'), number_format_i18n($total_posts));
            $status_links['all'] = [
            ...
            if(! empty($this->sticky_posts_count))
            {
                ...
                $sticky_inner_html = sprintf(/* translators: %s: Number of posts. */ _nx('Sticky <span class="count">(%s)</span>', 'Sticky <span class="count">(%s)</span>', $this->sticky_posts_count, 'posts'), number_format_i18n($this->sticky_posts_count));
                $sticky_link = [
                    'sticky' => [
        ...
        protected function extra_tablenav($which)
        {
            ...
            <div class="alignleft actions">
                <?php
                    ...
                    if($this->is_trash && $this->has_items() && current_user_can(get_post_type_object($this->screen->post_type)->cap->edit_others_posts))
                    {
            ...
            <select name="post_format" id="filter-by-format">
                ...
                <?php
                    foreach($used_post_formats as $used_post_format)
                    {
                        ...
                        ?>
                        <option<?php selected($displayed_post_format, $slug); ?>
                                value="<?php echo esc_attr($slug); ?>"><?php echo esc_html($pretty_name); ?></option>
                        <?php
                    }
                ?>
            </select>
        ...
        protected function get_sortable_columns()
        {
            ...
            if('page' === $post_type)
            {
                ...
                $sortables = [
                    'title' => ['title', false, __('Title'), $title_orderby_text, 'asc'],
                    'parent' => ['parent', false],
                    'comments' => ['comment_count', false, __('Comments'), __('Table ordered by Comments.')],
                    'date' => ['date', true, __('Date'), __('Table ordered by Date.')],
                ];
            }
            else
            {
                $sortables = [
                    'title' => ['title', false, __('Title'), __('Table ordered by Title.')],
                    'parent' => ['parent', false],
                    'comments' => ['comment_count', false, __('Comments'), __('Table ordered by Comments.')],
        ...
        public function column_title($post)
        {
            ...
            echo "</strong>\n";
            if('excerpt' === $mode && ! is_post_type_hierarchical($this->screen->post_type) && current_user_can('read_post', $post->ID))
            {
        ...
        protected function handle_row_actions($item, $column_name, $primary)
        {
            ...
            if($can_edit_post && 'trash' !== $post->post_status)
            {
                $actions['edit'] = sprintf('<a href="%s" aria-label="%s">%s</a>', get_edit_post_link($post->ID), /* translators: %s: Post title. */ esc_attr(sprintf(__('Edit &#8220;%s&#8221;'), $title)), __('Edit'));
                if('wp_block' !== $post->post_type)
                {
                    $actions['inline hide-if-no-js'] = sprintf('<button type="button" class="button-link editinline" aria-label="%s" aria-expanded="false">%s</button>', /* translators: %s: Post title. */ esc_attr(sprintf(__('Quick edit &#8220;%s&#8221; inline'), $title)), __('Quick&nbsp;Edit'));
                }
            }
            if(current_user_can('delete_post', $post->ID))
            {
                ...
                {
                    $actions['untrash'] = sprintf('<a href="%s" aria-label="%s">%s</a>', wp_nonce_url(admin_url(sprintf($post_type_object->_edit_link.'&amp;action=untrash', $post->ID)), 'untrash-post_'.$post->ID), /* translators: %s: Post title. */ esc_attr(sprintf(__('Restore &#8220;%s&#8221; from the Trash'), $title)), __('Restore'));
                }
                elseif(EMPTY_TRASH_DAYS)
                {
                    $actions['trash'] = sprintf('<a href="%s" class="submitdelete" aria-label="%s">%s</a>', get_delete_post_link($post->ID), /* translators: %s: Post title. */ esc_attr(sprintf(__('Move &#8220;%s&#8221; to the Trash'), $title)), _x('Trash', 'verb'));
                }
                if('trash' === $post->post_status || ! EMPTY_TRASH_DAYS)
                {
                    $actions['delete'] = sprintf('<a href="%s" class="submitdelete" aria-label="%s">%s</a>', get_delete_post_link($post->ID, '', true), /* translators: %s: Post title. */ esc_attr(sprintf(__('Delete &#8220;%s&#8221; permanently'), $title)), __('Delete Permanently'));
                }
            }
            if(is_post_type_viewable($post_type_object))
            {
                if(in_array($post->post_status, ['pending', 'draft', 'future'], true))
                {
                    if($can_edit_post)
                    {
                        ...
                        $actions['view'] = sprintf('<a href="%s" rel="bookmark" aria-label="%s">%s</a>', esc_url($preview_link), /* translators: %s: Post title. */ esc_attr(sprintf(__('Preview &#8220;%s&#8221;'), $title)), __('Preview'));
                    }
                }
                elseif('trash' !== $post->post_status)
                {
                    $actions['view'] = sprintf('<a href="%s" rel="bookmark" aria-label="%s">%s</a>', get_permalink($post->ID), /* translators: %s: Post title. */ esc_attr(sprintf(__('View &#8220;%s&#8221;'), $title)), __('View'));
                }
            }
            if('wp_block' === $post->post_type)
            {
                $actions['export'] = sprintf('<button type="button" class="wp-list-reusable-blocks__export button-link" data-id="%s" aria-label="%s">%s</button>', $post->ID, /* translators: %s: Post title. */ esc_attr(sprintf(__('Export &#8220;%s&#8221; as JSON'), $title)), __('Export as JSON'));
            }
            if(is_post_type_hierarchical($post->post_type))
            {
                /**
