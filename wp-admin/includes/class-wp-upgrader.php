<?php
    ...
    class WP_Upgrader
    {
        ...
        public function generic_strings()
        {
            ...
            $this->strings['fs_no_content_dir'] = sprintf(__('Unable to locate WordPress content directory (%s).'),
                'wp-content');
            ...
            $this->strings['temp_backup_mkdir_failed'] = sprintf(__('Could not create the %s directory.'),
                'upgrade-temp-backup');
            /* translators: %s: upgrade-temp-backup */
            $this->strings['temp_backup_move_failed'] = sprintf(__('Could not move the old version to the %s directory.'),
                'upgrade-temp-backup');
        ...
        public function fs_connect($directories = [], $allow_relaxed_file_ownership = false)
        {
            ...
            $credentials = $this->skin->request_filesystem_credentials(false, $directories[0],
        ...
        public function install_package($args = [])
        {
            ...
            if ($clear_destination) {
                 ...
                 *
                 */
                $removed = apply_filters('upgrader_clear_destination', $removed, $local_destination,
                    $remote_destination,
                    $args['hook_extra']);
            ...
            if ($args['clear_working']
                ...
                if (!$wp_filesystem->exists($remote_destination)) {
                    if (!$wp_filesystem->mkdir($remote_destination, FS_CHMOD_DIR)) {
                        return new WP_Error('mkdir_failed_destination', $this->strings['mkdir_failed'],
                            $remote_destination);
                    }
                }
