...
<?php
    while (have_posts()) :
        ...
        <?php if ((function_exists('get_post_format') && 'gallery' === get_post_format($post->ID)) || in_category(_x('gallery',
            'gallery category slug', 'twentyten'))) : ?>
                    ...
                    if ($images) :
                        ...
                        <div class="gallery-thumb">
                            <a class="size-thumbnail"
                               href="<?php the_permalink(); ?>"><?php echo wp_get_attachment_image($image,
                                    'thumbnail'); ?></a>
                        </div><!-- .gallery-thumb -->
                        <p><em>
                                <?php
                                    ...
                                    /* translators: 1: HTML tag attributes, 2: Image count. */
                                        _n('This gallery contains <a %1$s>%2$s photo</a>.',
                                            'This gallery contains <a %1$s>%2$s photos</a>.', $total_images,
                                            'twentyten'),
                                        /* translators: %s: Post title. */
                                        'href="' . esc_url(get_permalink()) . '" title="' . esc_attr(sprintf(__('Permalink to %s',
                                            'twentyten'), the_title_attribute('echo=0'))) . '" rel="bookmark"',
            ...
            <div class="entry-utility">
                <?php
                    ...
                    if (function_exists('get_post_format') && 'gallery' === get_post_format($post->ID)) :
                        ?>
                        <a href="<?php echo esc_url(get_post_format_link('gallery')); ?>"
                           title="<?php esc_attr_e('View Galleries', 'twentyten'); ?>"><?php _e('More Galleries',
                                'twentyten'); ?></a>
                        <span class="meta-sep">|</span>
                    <?php elseif ($gallery && in_category($gallery->term_id)) : ?>
                        <a href="<?php echo esc_url(get_category_link($gallery)); ?>"
                           title="<?php esc_attr_e('View posts in the Gallery category',
                               'twentyten'); ?>"><?php _e('More Galleries', 'twentyten'); ?></a>
                        <span class="meta-sep">|</span>
                    <?php endif; ?>
                <span class="comments-link"><?php comments_popup_link(__('Leave a comment', 'twentyten'),
                        __('1 Comment', 'twentyten'), __('% Comments', 'twentyten')); ?></span>
    ...
    <?php elseif ((function_exists('get_post_format') && 'aside' === get_post_format($post->ID)) || in_category(_x('asides',
            'asides category slug', 'twentyten'))) : ?>
        <div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
        ...
        <div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
            ...
            <div class="entry-utility">
                ...
                <?php
					...
					<?php
                        /* translators: 1: CSS classes, 2: Category list. */
                        printf(__('<span class="%1$s">Tagged</span> %2$s', 'twentyten'),
