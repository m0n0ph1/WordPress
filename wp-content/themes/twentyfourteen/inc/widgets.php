<?php
    ...
    class Twenty_Fourteen_Ephemera_Widget extends WP_Widget
    {
        ...
        public function __construct()
        {
            parent::__construct(
                ...
                [
                    ...
                    'description' => __('Use this widget to list your recent Aside, Quote, Video, Audio, Image, Gallery, and Link posts.',
                        'twentyfourteen'),
            ...
            if ($ephemera->have_posts()) :
                ...
                <h1 class="widget-title <?php echo esc_attr($format); ?>">
                    <a class="entry-format"
                       href="<?php echo esc_url(get_post_format_link($format)); ?>"><?php echo esc_html($title); ?></a>
                </h1>
                <ol>
                                            ...
                                            if (has_post_format('gallery')) :
                                                ...
                                                if (post_password_required()) :
                                                    ...
                                                    elseif ($total_images > 0) :
                                                        $image = reset($images);
                                                        $post_thumbnail = wp_get_attachment_image($image,
                                                            'post-thumbnail');
                                                    endif;
                                                    ...
                                                    <p class="wp-caption-text">
                                                        <?php
                                                            ...
                                                            /* translators: 1: Post permalink, 2: Number of images in the gallery. */
                                                                _n('This gallery contains <a href="%1$s" rel="bookmark">%2$s photo</a>.',
                                            ...
                                            <?php
                                                ...
                                                if (!post_password_required() && (comments_open() || get_comments_number())) :
                                                    ...
                                                    <span class="comments-link"><?php comments_popup_link(__('Leave a comment',
                                                            'twentyfourteen'), __('1 Comment', 'twentyfourteen'),
                                                            __('% Comments', 'twentyfourteen')); ?></span>
                                                <?php endif; ?>
                                        </div><!-- .entry-meta -->
                                    </header><!-- .entry-header -->
        ...
        public function form($instance)
        {
            ...
            <p><label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php _e('Number of posts to show:',
                        'twentyfourteen'); ?></label>
            ...
            <p><label for="<?php echo esc_attr($this->get_field_id('format')); ?>"><?php _e('Post format to show:',
                    'twentyfourteen'); ?></label>
            <select id="<?php echo esc_attr($this->get_field_id('format')); ?>"
                ...
                <?php foreach ($this->formats as $slug) : ?>
                    <option value="<?php echo esc_attr($slug); ?>"<?php selected($format,
                        $slug); ?>><?php echo esc_html(get_post_format_string($slug)); ?></option>
                <?php endforeach; ?>
            </select>
            <?php
        }
